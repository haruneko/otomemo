...
function ChordCell({
  ...
  onCancel: () => void;
  children?: ReactNode; // arp グリフ（向き記号）。ストローク時は無地＝children なし。
}) {
  const hd = useHoldDrag({ axis: "y", onFire, onDrag, onCommit, onCancel });
  const style = hv != null ? ({ "--hv": hv } as CSSProperties) : undefined;
  return (
    <button type="button" aria-label={ariaLabel} className={className} style={style} onClick={onTap} {...hd}>
      {children}
    </button>
  );
}
...
export function ChordPatternEditor({
  ...
  // Task1c：arp の向き記号（右手ヒットのグリフ＋響き折りたたみサマリ）。
  const arpArrow = v.arpDir === "down" ? "↓" : v.arpDir === "updown" ? "↑↓" : "↑";
  // 「響き」折りたたみ見出しの現在値サマリ（例：アルペジオ·top E6·close·↑↓·1oct）。
  const voicingSummary = [
    isArp ? "アルペジオ" : "ストローク",
    `top ${pitchName(top)}`,
    v.openClose === "open" ? "open" : "close",
    ...(isArp ? [arpArrow, `${v.arpOctaves ?? 1}oct`] : v.powerChord ? ["パワー"] : []),
  ].join("·");
  // 奏法バッジ（読み取り専用・S3／格下げ）＝auto は音色から解決した旨を添える。
  const styleLabel = guitarResolved
    ? v.style === "auto"
      ? "ギター（音色から）"
      : "ギター"
    : v.style === "auto"
  ...
  return (
    <div className="cp-editor">
      ...
      <div className="cp-grid" role="grid" aria-label="two-hand-grid" ref={scrollerRef}>
        ...
        {/* 上帯＝右手（1レーン固定・度数レーン化しない＝2026-07-04裁定）。arp 時はヒットに向きグリフ＋斜め階段。 */}
        <div className="cp-hand cp-rh-lane" role="row" aria-label="right-hand">
          ...
          {Array.from({ length: pattern.steps }, (_, s) => {
            ...
            return (
              ...
              >
                {head && isArp ? arpArrow : null}
              </ChordCell>
            );
          })}
        </div>
        {/* S3 D/U ストリップ（guitar 解決時のみ・モックB）：右手の直下（同一列）。hit のあるセルに D/U（明示=実線／自動=薄）・タップで反転。 */}
        {guitarResolved && (
          <div className="cp-hand cp-du-row" role="row" aria-label="du-strip">
            <div className="cp-hand-label">D/U</div>
            {Array.from({ length: pattern.steps }, (_, s) => {
              const head = startAt(s);
              if (!head) return <span key={s} className="cp-cell du empty" aria-hidden="true" />;
              ...
              return (
                <button
                  ...
                  type="button"
                  aria-label={`dir-${s}`}
                  className={"cp-cell du" + (explicit ? " on" : " auto") + (d === "U" ? " up" : " down")}
                  onClick={() => toggleDir(s)}
                >
                  {d}
                </button>
            ...
            <div className="cp-lh-lanes" role="grid" aria-label="lh-pad">
              {LH_LANES.map((lane) => (
                <div className="cp-hand cp-lh-lane" role="row" key={lane.d}>
                  <div className="cp-hand-label">{lane.label}</div>
                  {Array.from({ length: pattern.steps }, (_, s) => {
      ...
      <div className="cp-perf-row">
        <span className="cp-perf-badge" aria-label="voicing-style-summary">奏法：{styleLabel}</span>
        {guitarResolved && (
          <span className="cp-unit">
            <span className="cp-vlbl">速さ</span>
      ...
      {/* Task1c「響き」＝折りたたみへ降格（既定=閉）＋現在値サマリ。展開で7ノブ（打ち方/トップ/広がり/向き/駆け上がり幅/区切り/高さ）。 */}
      <div className="cp-voicing" aria-label="voicing">
        <button type="button" className="cp-voicing-toggle" aria-label="voicing-toggle" aria-expanded={voicingOpen} onClick={() => setVoicingOpen((o) => !o)}>
          <span className="cp-voicing-caret">{voicingOpen ? "▼ 響き" : "▶ 響き"}</span>
          {!voicingOpen && <span className="cp-voicing-summary">{voicingSummary}</span>}
        </button>
        {voicingOpen && (
          <div className="cp-voicing-body">
            ...
            <div className="cp-vrow">
              {isArp ? (
                <>
                  ...
                  {/* 区切り＝ドロップダウンからセグメントへ（他ノブと統一・Task1c 細部是正）。横スクロールで7段を収める。 */}
                  <span className="cp-unit cp-unit-wide">
                    <span className="cp-vlbl">区切り</span>
                    <div className="seg seg-chord cp-reset-seg" role="group" aria-label="arp-reset">
                      {ARP_RESET_OPTS.map((o) => (
                        <button key={o.v} type="button" aria-label={`arp-reset-${o.v}`} className={(v.arpReset ?? 0) === o.v ? "on" : ""} onClick={() => setV({ arpReset: o.v || undefined })}>{o.label}</button>
                      ))}
                    </div>
                  </span>
                </>
