...
import { LyricSplitComposer } from "./LyricSplitComposer";
  ...
  return (
    <div className="proll-wrap">
            ...
            <div className="proll-lyric-panel" aria-label="lyric-panel">
              ...
              {status?.kind === "字余り" && (
                <div className="proll-lyric-split">
                  ...
                  >
                    {splitState === "busy" ? "割り方を出しています…" : `音符を割って合わせる（字余り${status.count}）`}
                  </button>
                  {splitState === "error" && <span className="muted">割り方が出せませんでした</span>}
                  {splitData && (
                    <LyricSplitComposer
                      origNotes={notes}
                      moras={readMoras}
      ...
      {/* W-K2：歌詞があれば韻律チェックのトグル＋軽い凡例（既定ON・機械は候補まで＝どぎつくしない）。
          句の面が配線された画面ではこの行は出さない＝歌詞パネルの奥へ移した（§31-11 の16 (c)・暫定実装）。
          配線していない呼び側は**行の見た目は従来どおり常設**。ただし ON/OFF の初期値は覚え（`cm.lyricFit`）を
          読むので、配線画面で OFF にすると**この行も次から OFF で開く**＝完全な「後退ゼロ」ではない
          （2026-07-30b 監査の指摘2-3）。覚えを画面ごとに分ける必要が出たらキーを2つに割る。 */}
      {hasLyric && !wired && (
