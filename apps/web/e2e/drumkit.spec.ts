...
test("drum kit maps GM notes; hihat close/open distinguished (U12/#79)", async ({
  ...
  try {
    ...
    // 各GM番号が正しい楽器・note に載る
    expect(map).toMatch(/drum 36 -> Standard Kick.* @note 60/); // kick=原音高
    expect(map).toMatch(/drum 38 -> Standard Snare.* @note 60/); // snare=原音高
    ...
    expect(map).toMatch(/drum 49 -> Crash Cymbal.* @note 49/); // crash(効果音Car-Crash誤マッチ無し)
  } finally {
