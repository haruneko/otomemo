...
import { chordChips, applyChordTrial, adoptedChordContent, chordName, dedupeChordSubs, type ChordSub, type ChordTrial } from "../src/deskChords";
...
describe("dedupeChordSubs（P3-3：代替候補の重複表示を畳む）", () => {
  it("同じ (root,quality) は先勝ちで1つに畳む・順序維持", () => {
    const subs: ChordSub[] = [
      { root: 9, quality: "m" }, // Am
      { root: 5, quality: "" }, // F
      { root: 9, quality: "m" }, // Am（重複）
      { root: 5, quality: "m" }, // Fm（quality 違い＝別物・残る）
    ];
    expect(dedupeChordSubs(subs)).toEqual([
      { root: 9, quality: "m" },
      { root: 5, quality: "" },
      { root: 5, quality: "m" },
    ]);
  ...
  it("root は 12 で正規化して重複判定（21≡9）・空配列はそのまま", () => {
    expect(dedupeChordSubs([{ root: 9, quality: "m" }, { root: 21, quality: "m" }])).toEqual([{ root: 9, quality: "m" }]);
    expect(dedupeChordSubs([])).toEqual([]);
  });
});
