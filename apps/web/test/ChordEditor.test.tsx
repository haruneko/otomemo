...
describe("ChordEditor", () => {
  it("adds a chord with ＋コード", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith([{ root: "C", quality: "", start: 0, dur: 4 }]);
  });

  it("changes a chord's quality", async () => {
    const onChange = vi.fn();
    render(
      <ChordEditor chords={[{ root: "C", quality: "", start: 0, dur: 4 }]} onChange={onChange} />,
    );
    await userEvent.selectOptions(screen.getByLabelText("quality-0"), "m7");
    expect(onChange).toHaveBeenCalledWith([{ root: "C", quality: "m7", start: 0, dur: 4 }]);
  });
