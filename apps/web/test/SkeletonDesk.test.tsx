...
const playHandle = { stop: vi.fn(), pause: vi.fn(), resume: vi.fn(), setLensGain: vi.fn(), reschedule: vi.fn(), setLoopRange: vi.fn() };
vi.mock("../src/audio", () => ({
  previewNote: vi.fn(),
...
describe("SkeletonDesk D4（④出口＝吹く→試着→置く・分岐スタック）", () => {
  ...
  it("分岐スタック：getRelations の realized_from×melody 件数が「作ったメロ N」に出る（bass は数えない）", async () => {
    getComposition.mockResolvedValue(tree4);
    getRelations.mockResolvedValue([
...
describe("SkeletonDesk D5（ステージレール＋レンズのステージ相対）", () => {
  ...
  it("ステージレール4つが出て既定は③骨格・レンズ2択ラベルは［骨格だけ｜フル］", async () => {
    getComposition.mockResolvedValue(tree);
    renderDesk();
    await screen.findByText(/Aメロ/);
    ...
    // 既定③＝レンズラベルは骨格だけ/フル。
    expect(screen.getByLabelText("lens-fold").textContent).toBe("骨格だけ");
    expect(screen.getByLabelText("lens-real").textContent).toBe("フル");
  });

  it("②コードへ切替→レンズラベルが［コードだけ｜フル］に読み替わる（aria-label は A/B ゲートで固定）", async () => {
    getComposition.mockResolvedValue(tree);
    renderDesk();
    await screen.findByText(/Aメロ/);
    ...
    expect(screen.getByLabelText("lens-fold").textContent).toBe("コードだけ"); // A群ラベルが読み替わる
    expect(screen.getByLabelText("lens-real").textContent).toBe("フル"); // B群ラベルが読み替わる
    // ①ビートへ再切替＝また読み替わる。
    fireEvent.click(screen.getByLabelText("stage-beat"));
    expect(screen.getByLabelText("lens-fold").textContent).toBe("ドラムだけ");
    expect(screen.getByLabelText("lens-real").textContent).toBe("伴奏");
  });

  it("再生中のステージ切替：reloop＝その場組み直し（reschedule）で playing 維持・begin/stop を呼ばない（頭に戻らない）", async () => {
    getComposition.mockResolvedValue(tree);
    renderDesk();
    await screen.findByText(/Aメロ/);
    // 再生開始（初期 loopOn は mount で立つ）。begin→playNotes→state playing。
    fireEvent.click(screen.getByLabelText("desk-play"));
    // Icon 化（絵文字→SVG）後：再生中は pause アイコン（<rect>2つ）／停止中は play アイコン（<path>のみ・rect無し）。
    await waitFor(() => expect(screen.getByLabelText("desk-play").querySelector("rect")).toBeTruthy());
    const beginCalls = (playNotes as ReturnType<typeof vi.fn>).mock.calls.length;
    playHandle.reschedule.mockClear();
    // ステージ切替＝reloop（内容が変わる）＝その場組み直し。
    fireEvent.click(screen.getByLabelText("stage-beat"));
    // reloop は handle.reschedule を呼ぶ（stop→begin しない＝playNotes 再呼び出し無し）。playing/loop は維持。
    await waitFor(() => expect(playHandle.reschedule).toHaveBeenCalled());
    expect((playNotes as ReturnType<typeof vi.fn>).mock.calls.length).toBe(beginCalls); // begin を回さない＝頭に戻らない
    expect(playHandle.stop).not.toHaveBeenCalled(); // 音が途切れない
    expect(screen.getByLabelText("desk-play").querySelector("rect")).toBeTruthy(); // playing 維持（pause アイコン）
  });
...
describe("SkeletonDesk D6（stale＝要確認・変化した瞬間を聴く・オオカミ少年化しない）", () => {
  ...
  it("②で G→B を採用→bar2 接点1つだけ stale（要チェック×1）→タップで差替後の2声（4度）ダイアッドが発火→確認で消える", async () => {
    getComposition.mockResolvedValue(tree);
...
// --- C 他ネタへ潜る導線（①ドラムブロック／分岐一覧「作ったメロ N」→そのネタを開く・onOpenNeta） ---
describe("SkeletonDesk C（潜り導線＝onOpenNeta）", () => {
  const material = { bars: 2, tones: [{ start: 0, pitch: 60 }, { start: 4, pitch: 64 }] };
  const drumNeta = mkNeta("dr1", "rhythm", { title: "ビート", content: { rhythm: { steps: 8, beatsPerStep: 0.5, lanes: [{ midi: 36, hits: [0, 4] }] } } });
  const withDrum = {
    neta: mkNeta("s1", "section", { title: "Aメロ" }),
    children: [
      { position: 0, ord: 0, node: { neta: drumNeta, children: [] } },
      { position: 0, ord: 0, node: { neta: mkNeta("sk1", "skeleton", { content: material, key: 2, mode: "major" }), children: [] } },
    ],
  };

  it("①ドラムブロックのタップ→onOpenNeta(そのドラムネタ) が呼ばれる", async () => {
    const onOpenNeta = vi.fn();
    getComposition.mockResolvedValue(withDrum);
    render(<SkeletonDesk sectionId="s1" sectionKey={0} sectionMode="major" meter="4/4" tempo={120} skelNetaId="sk1" skelPosition={0} skelOrd={0} onClose={() => {}} onOpenNeta={onOpenNeta} />);
    await screen.findByText(/Aメロ/);
    fireEvent.click(screen.getByLabelText("beat-block-0"));
    expect(onOpenNeta).toHaveBeenCalledTimes(1);
    expect(onOpenNeta.mock.calls[0]![0]).toEqual(expect.objectContaining({ id: "dr1", kind: "rhythm" }));
  });

  it("onOpenNeta 未指定なら①ブロックは従来＝表示のみ（tappable でない・click しても何も起きない）", async () => {
    getComposition.mockResolvedValue(withDrum);
    render(<SkeletonDesk sectionId="s1" sectionKey={0} sectionMode="major" meter="4/4" tempo={120} skelNetaId="sk1" skelPosition={0} skelOrd={0} onClose={() => {}} />);
    await screen.findByText(/Aメロ/);
    const block = screen.getByLabelText("beat-block-0");
    expect(block.className).not.toContain("tappable");
    expect(block.getAttribute("role")).toBeNull();
  });

  it("分岐一覧の各メロタップ→onOpenNeta(そのメロネタ) が呼ばれる", async () => {
    const onOpenNeta = vi.fn();
    getComposition.mockResolvedValue(withDrum);
    getRelations.mockResolvedValue([
      { type: "realized_from", neta: mkNeta("m1", "melody", { title: "吹メロ1" }) },
    ...
    render(<SkeletonDesk sectionId="s1" sectionKey={0} sectionMode="major" meter="4/4" tempo={120} skelNetaId="sk1" skelPosition={0} skelOrd={0} onClose={() => {}} onOpenNeta={onOpenNeta} />);
    await screen.findByText(/Aメロ/);
    fireEvent.click(await screen.findByLabelText("realized-stack")); // 一覧を開く
    const list = await screen.findByLabelText("realized-list");
    const items = list.querySelectorAll(".desk-stack-item");
    expect(items.length).toBe(2);
    fireEvent.click(items[1]!); // 2つ目（吹メロ2）
    expect(onOpenNeta).toHaveBeenCalledTimes(1);
    expect(onOpenNeta.mock.calls[0]![0]).toEqual(expect.objectContaining({ id: "m2", kind: "melody" }));
  });
});

// --- P3-4 保存失敗の可視化（無音の catch をやめる） ---
describe("SkeletonDesk P3-4（保存失敗の可視化＋タップ再試行）", () => {
  const material = { bars: 2, tones: [{ start: 0, pitch: 60 }, { start: 4, pitch: 64 }] };
  const tree = {
    neta: mkNeta("s1", "section", { title: "Aメロ" }),
    children: [{ position: 0, ord: 0, node: { neta: mkNeta("sk1", "skeleton", { content: material, key: 2, mode: "major" }), children: [] } }],
  ...
  it("保存失敗で警告が出る→タップ再試行で成功すれば消える", async () => {
    getComposition.mockResolvedValue(tree);
    updateNeta.mockRejectedValueOnce(new Error("net down")); // 初回保存＝失敗
    render(<SkeletonDesk sectionId="s1" sectionKey={0} sectionMode="major" meter="4/4" tempo={120} skelNetaId="sk1" skelPosition={0} skelOrd={0} onClose={() => {}} />);
    await screen.findByText(/Aメロ/);
    // 編集→debounce 保存が失敗→警告表示。
    fireEvent.click(screen.getByLabelText("skel-bars-inc"));
    const err = await screen.findByLabelText("desk-save-error");
    expect(err.textContent).toContain("保存に失敗");
    // 以後は成功（beforeEach で resolve 済）。タップ再試行で updateNeta 再発行→警告が消える。
    updateNeta.mockResolvedValue({});
    fireEvent.click(err);
