...
import { ChordPatternEditor, voicingRollRects } from "../src/components/ChordPatternEditor";
...
// Task1k（2026-07-24）：概形ロールをパッド整列の下段ピアノロールへ作り直す。
// 純関数 voicingRollRects＝列ジオメトリ(1step=17px)共有・音程で縦配置。エディタは cp-grid 内の帯へ移し
// 横スクロール/列を共有＝パッドと縦に揃う（旧 MiniRoll 引き伸ばしを撤去）。content 契約は不変。
describe("ChordPatternEditor Task1k 概形＝パッド整列ピアノロール", () => {
  it("voicingRollRects＝step 整列（start=1拍→x=68・dur=1拍→w=67）・stripW=steps*17−1", () => {
    const { rects, stripW } = voicingRollRects([{ pitch: 60, start: 1, dur: 1 }], 16);
    expect(rects[0]!.x).toBe(4 * 17); // start=1拍=step4（1step=0.25拍）→ 68px
    expect(rects[0]!.w).toBe(4 * 17 - 1); // dur=1拍=4step → 67px（末尾gap分を引く）
    expect(stripW).toBe(16 * 17 - 1); // 最終セル右端に flush（271px）
  });

  it("voicingRollRects＝音程で縦配置（高音ほど小さい y＝上）", () => {
    const { rects, lo, hi } = voicingRollRects(
      [{ pitch: 60, start: 0, dur: 1 }, { pitch: 72, start: 0, dur: 1 }],
      16,
    );
    expect(lo).toBe(60);
    expect(hi).toBe(72);
    expect(rects[1]!.y).toBeLessThan(rects[0]!.y); // pitch72（高音）の方が上＝y 小
  });

  it("voicingRollRects＝単音/同高は中央寄せ（frac=0.5＝上下同じ y）・空は rects なし", () => {
    const flat = voicingRollRects([{ pitch: 64, start: 0, dur: 1 }, { pitch: 64, start: 4, dur: 1 }], 16);
    expect(flat.rects[0]!.y).toBe(flat.rects[1]!.y);
    expect(voicingRollRects([], 16).rects).toEqual([]);
  });

  it("エディタ＝概形帯（voicing-roll）が cp-grid 内・cp-roll-strip を含む／旧 mini-preview は非存在", () => {
    const { container } = render(<ChordPatternEditor pattern={pat()} onChange={vi.fn()} />);
    const roll = screen.getByLabelText("voicing-roll");
    expect(roll).toBeTruthy();
    expect(screen.getByLabelText("two-hand-grid").contains(roll)).toBe(true); // 同一スクロール容器内＝列/横スクロール共有
    expect(container.querySelector(".cp-roll-strip")).toBeTruthy();
    expect(screen.queryByLabelText("mini-preview")).toBeNull(); // 旧 MiniRoll 引き伸ばしは撤去
  });

  it("右手 hits 空でも左手 lh があれば概形が出る（両手の片方だけでも描く）", () => {
    render(<ChordPatternEditor pattern={pat({ hits: [], lh: { mode: "custom", hits: [{ step: 0, deg: "R", dur: 4 }] } })} onChange={vi.fn()} />);
    expect(screen.getByLabelText("voicing-roll")).toBeTruthy();
  });

  it("右手 hits も左手 lh も空＝概形は出ない（描くものが無い）", () => {
    render(<ChordPatternEditor pattern={pat({ hits: [] })} onChange={vi.fn()} />);
    expect(screen.queryByLabelText("voicing-roll")).toBeNull();
  });
});
