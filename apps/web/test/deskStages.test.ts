...
describe("stageLabels（レンズ2択のラベルが focusStage で読み替わる）", () => {
  it("①beat=［ドラムだけ｜伴奏］・②chord=［コードだけ｜フル］・③④=［骨格だけ｜フル］", () => {
    expect(stageLabels("beat")).toEqual(["ドラムだけ", "伴奏"]);
    expect(stageLabels("chord")).toEqual(["コードだけ", "フル"]);
    expect(stageLabels("skeleton")).toEqual(["骨格だけ", "フル"]);
    expect(stageLabels("surface")).toEqual(["骨格だけ", "フル"]);
  });
});
...
describe("③④ bit一致：skeleton/surface の [...a,...b] は現行 deskLensNotes と音符列 deepEqual（LENS印含む）", () => {
  it("skeleton：a=fold群(LENS_FOLD)・b=real群(LENS_REAL)・合算が deskLensNotes と一致", () => {
    ...
    expect(s.labels).toEqual(["骨格だけ", "フル"]);
...
describe("①beat：ドラムだけ＝ドラムのみ／伴奏＝フル", () => {
  const s = stageLensSets("beat", args);
  it("labels=［ドラムだけ｜伴奏］", () => {
    expect(s.labels).toEqual(["ドラムだけ", "伴奏"]);
  });
  it("a（ドラムだけ）＝全て drum・全て LENS_FOLD", () => {
  ...
  it("b（伴奏）＝フル(real)＝コード楽器/ドラム＋骨格線を含み全て LENS_REAL", () => {
    for (const n of s.b) expect(n.lens).toBe(LENS_REAL);
...
describe("②chord：コードだけ＝素の三和音（chordsToNotes）／フル＝編成合成", () => {
  const s = stageLensSets("chord", args);
  it("labels=［コードだけ｜フル］", () => {
    expect(s.labels).toEqual(["コードだけ", "フル"]);
  });
  it("a（コードだけ）＝effChords 由来のコードトーン（part:'chord'・LENS_FOLD）・コード数×構成音", () => {
  ...
  it("b（フル）＝編成合成(real)＝コード楽器/ドラムを含み全て LENS_REAL", () => {
    for (const n of s.b) expect(n.lens).toBe(LENS_REAL);
    expect(s.b.some((n) => n.part === "chord")).toBe(true);
  });
});
