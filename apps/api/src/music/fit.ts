...
import { type Chord, chordPcs, normRoot, scalePcs, rankKeys, KEY_NAMES } from "./theory";
...
export function detectKeyFromNotes(notes: Note[]): { key: number; mode: "major" | "minor" } {
  const valid = (notes ?? []).filter((n) => typeof n.pitch === "number");
  if (!valid.length) return { key: 0, mode: "major" };
