...
export function genMelody(
  ...
  if (opts?.motifModel && bpb === 4 && (chords?.length ?? 0) > 0 && bars >= 1) {
    ...
    const mNotes = genMotifMelody(chordPcsPerBar, sp, opts.motifModel.rhythm, opts.motifModel.move, { seed: seed ?? 1, tonicPc, fifthPc: (tonicPc + 7) % 12, ending: "close", skelModel: opts.skelModel ?? loadSkeletonModel(), appoggiatura: opts.appoggiatura ?? 0.5 }); // 既定=同梱学習骨格＋倚音0.5（中景の表情・強拍CT99→81%・実曲57%へ向け耳で微調整）
