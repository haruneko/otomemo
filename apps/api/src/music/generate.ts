...
import { compTypeById, pickCompType, compHitsForBar, type CompType } from "./chordLibrary"; // 伴奏パターン型辞書（chordLibrary・S2・2026-07-22）
...
export function genChordPattern(
  frame?: Frame | null,
  seed?: number | null,
  opts?: { style?: "keyboard" | "guitar"; strumMs?: number; pattern?: string } | null,
): GenResult {
  const f = normalizeFrame(frame);
  ...
  const compType: CompType | null = opts?.pattern != null && info.grouping !== "compound" && info.beatsPerBar === 4
    ? (compTypeById(opts.pattern) ?? pickCompType(opts.pattern, f.section?.role, f.tempo, seed ?? 5))
    : null;
  if (compType) {
    // 型の RH 16セルを各小節へ敷き hits 化（vel 層込み）。mode/voicing は型の既定。LH は data-only＝配線しない。
    ...
    const content = { mode: compType.mode, voicing, steps, hits };
