...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    "sing_neta",
    {
      ...
      description:
        "歌詞(syllable)付きの melody ネタを VOICEVOX 歌唱エンジンで実際に歌わせ、wav を生成してネタに render 資産として紐付ける（asset id を返す）。先に set_lyric でモーラを載せておくこと。1フレーズ≒1秒で同期実行。speaker で声色(frame_decode id)を選べる（既定=波音リツ3009）。音域外はクランプ・休符/タイは自動。歌い回しの良否は耳で（機械は足場・仕上げは人間）。",
      inputSchema: {
