...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    "suggest_lyric_rhythm",
    {
      title: "歌詞→リズム型候補",
      description:
        "「この歌詞、どんなリズムで歌えばノる？」「メロを作る前に歌詞の譜割り（音の詰め方）を先に見たい」のように、歌詞のリズム型を先に確かめたいとき使う。歌詞(かな)をモーラに分けて『リズム型』候補を複数出す（ピッチは付けない＝割付の型のみ・**提案であって保存物ではない**）。候補=基本(1モーラ1音符)/細分(字余り・早口)/句末伸ばし(字足らず・メリスマ)。特殊拍の扱い＝この提案では長音ーは直前へ延長(tie)・促音っは詰め(rest)＝**音符を立てない**（撥音んは独立音符・拗音きゃは1モーラ）。※オーナー裁定(design §31-8)は「っ」「ー」にも**音符を立てる**＝この型候補だけが従来の数え方で残っている（保存物ではないので据え置き）。句頭が助詞/接続詞/感動詞なら弱起(pickup)を提案。漢字は先に読み(かな)へ。",
  ...
  server.registerTool(
    ...
    async ({ frame, chords, seed, style, repetition, rangeSteps, motifBars, density, swing, expression, phrasing, runs, push, foreground, breathe, humanize, form, bass, counter, drums, backbeat, drumLock, converse, hook, articulation, inflect, motifMode, finest, flow, pickup, arc, skeletonNetaId, rhythmParts, lyrics, readLyrics, standSpecialMoras, corpus, corpusStrength, rhythmicContrast, cadenceSoprano, vlWeight, vlAtCadenceOnly }) => {
      ...
      if (lyrics && lyrics.trim()) {
        ...
        const plan = planLyricMelody(rawLines, { bars: barsN, beatsPerBar: barLen, readings, standSpecialMoras });
        ...
        (resL as typeof resL & { accentSource?: string; lyricPlan?: unknown }).lyricPlan = {
          ...
          readingSource, standSpecialMoras: standSpecialMoras === true, // どちらの数え方で音数が決まったか＝耳で確かめる側の手がかり
        };
        return ok(resL);
      }
