...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    ...
    async ({ frame, chords, seed, drums, kickLock, snareGap, approach, style, fill, slashBass, skeletonNetaId }) => {
      ...
      const res = genBass(frame, chords, seed, drums, { kickLock, snareGap, approach, skeleton, style, fill, slashBass });
