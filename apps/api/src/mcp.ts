...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      // 2026-09-16 外したつまみ（錨・リフ文法・和音追従・変奏）が来たら黙って捨てず meta.warnings で告げる（schema は passthrough＝未知キーを落とさない）。
      const res = withRemovedRiffKnobWarning(genBass(frame, chords, seed, drums, { kickLock, snareGap, approach, skeleton, style, fill, slashBass, swing, humanize, relative, respondToCues }), args, "bass");
