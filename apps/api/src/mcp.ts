...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    "gen_melody",
    { title: "メロディを生成", description: "コードトーン拘束のメロを生成（拍頭=コードトーン）。chords を渡せば合わせる。style でコーパス(library)の歩幅統計にバイアス。repetition=動機反復の強さ(0-1)、rangeSteps=音域(音階ステップ・6度≈6)で利用時制約。density=細かさ(0=疎〜1=細かい)、swing=跳ね(0=ストレート〜1=シャッフル)、expression=表情(0=素直〜1=強拍に倚音/掛留)、phrasing=句割り(symmetric/asymmetric)。", inputSchema: { frame: frameSchema, chords: chordsSchema.optional(), seed: z.number().int().optional(), style: z.string().optional().describe("コーパスstyle(irish/game等)。投入済みなら歩幅をその統計へ寄せる"), repetition: z.number().min(0).max(1).optional().describe("動機反復の強さ 0=反復なし〜1=強反復(既定0.85=やや強め)"), rangeSteps: z.number().int().min(2).max(20).optional().describe("骨格の音域(音階ステップ)。6度差に抑えるなら6"), motifBars: z.number().int().min(1).max(4).optional().describe("モチーフ/フレーズ長(小節)。1=短く反復多め/2=既定/4=長く展開的"), density: z.number().min(0).max(1).optional().describe("細かさ 0=疎(白玉寄り)〜1=細かい(16分多め)。未指定=コーパス分布"), swing: z.number().min(0).max(1).optional().describe("跳ね 0=ストレート〜1=シャッフル(8分裏が3連2/3へ)"), expression: z.number().min(0).max(1).optional().describe("表情 0=素直(強拍ほぼコード音)〜1=もたれ(強拍に倚音/掛留＝歌い回し)。未指定=素直"), phrasing: z.enum(["symmetric", "asymmetric"]).optional().describe("句割り。symmetric=2小節句(問い→答え・前楽節末が属音で開く)/asymmetric=不等分割(3+3+2等の呼吸)。未指定=句末着地なし(従来)") } },
    async ({ frame, chords, seed, style, repetition, rangeSteps, motifBars, density, swing, expression, phrasing }) => {
      const corpusModel = learnMotifModelFromLibrary(core, style); // P1：らしさ順ランクの軸（＝生成bias と同じ学習モデル）
      const res = genMelodyCandidates(frame, chords, seed, { useV2: true, stepWeights: learnStepWeightsFromLibrary(core, style) ?? undefined, motifModel: corpusModel ?? undefined, repetition, rangeSteps, motifBars, corpusModel, density, swing, expression, phrasing });
