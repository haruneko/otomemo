...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    ...
    async ({ frame, chords, seed, style, repetition, rangeSteps, motifBars, density, swing, expression, phrasing, runs, push, foreground, breathe, humanize, form, bass, counter, drums, backbeat, drumLock, converse, hook, articulation, inflect, motifMode, finest, flow, pickup, arc, skelColor, skeletonNetaId, rhythmParts }) => {
      ...
      const res = genMelodyCandidates(frame, chords, seed, { useV2: true, motifModel: corpusModel ?? undefined, repetition, rangeSteps, motifBars, corpusModel, density, swing, expression, phrasing, runs, push, foreground, breathe, humanize, form, bass, counter, drums, backbeat, drumLock, converse, hook, articulation, inflect, motifMode, finest, flow, pickup, arc, skelColor, skeleton, rhythmParts: rp });
