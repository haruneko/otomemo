...
import { GUITAR_GRAMMAR_IDS } from "@cm/music-core"; // M5：gen_chord_pattern の guitarRiff 選択肢を辞書から作る
import { BASS_GRAMMARS } from "./music/bassLibrary"; // M3-3c＝リフ文法辞書（inputSchema の選択肢を辞書から作る＝schema と実態がずれない）
...
import { accentsFromSyllables, extractReadings } from "./accent"; // W-K1 アクセント自動注入＋句の読み（pyopenjtalk・どちらも spawn）
...
// #100/#101: surface="chat" は **共通 verbs だけ**公開（legacy を隠す＝モデルが旧ツールを掴まない）。"full"(既定)＝共通+legacy（test互換）。本数は増える＝2026-07-27現在 chat27/legacy44/full75。
// （2026-07-27：gen_melody を legacy専用ブロックから共通ブロックへ移設＝design #13d(c)の「既存chat verb」前提を実態化。旧位置に置いたままだと chat面のツール一覧に出ず、CHAT_VERB_NAMES へ足すだけでは治らない＝BUG#1型の根）
// リフ文法の選択肢＝**辞書から作る**（手で並べると辞書が増えたとき schema が実態とずれる＝M3-3c）。
const BASS_GRAMMAR_IDS = BASS_GRAMMARS.map((g) => g.id) as [string, ...string[]];
// ギターのリフ文法（M5）も辞書から（music-core `GUITAR_GRAMMAR_IDS`）。
const GUITAR_GRAMMAR_ID_ENUM = [...GUITAR_GRAMMAR_IDS] as [string, ...string[]];
...
export function buildMcpServer(core: Core, opts: { surface?: "chat" | "full" } = {}): McpServer {
  ...
  server.registerTool(
    "gen_bass",
    { title: "ベースを生成", description: "強拍ルート/弱拍5度のベースライン（C2基準低域・コードに合う）。drums(gen_drums の content)を渡すとドラムに噛む：kickLock=キック骨格の採用率(負=逆相・キック裏8分)、snareGap=スネア(2,4拍)頭で音価を切りbackbeatを抜く、approach=コードチェンジ直前を半音/全音接近→次ルート着地。全て既定0=従来。6/8はkickLock/approach対象外。**style**=ベース定型型ID(RK-8ROOT/BL-WHOLE/CP-OCT8/FK-ONE/ED-OFFBEAT/VR-8DRIVE 等)またはジャンル名(rock/ballad/citypop/funk/edm/vocarock＝frame.section.role＋tempo域で候補型を絞り選択)＝度数×16分グリッドの型を実音化。型格子を正準に鳴らしkickLockとは二重適用しない(排他)。**fill**=0..1(小さい=下降で落ち着かせる/大きい=上昇で盛り上げる)またはフィル型ID(FL-WALKUP等)＝末尾1つ手前の小節に駆け上がり/下がりを挿入。style/fill未指定=従来bit一致。6/8はstyle/fill対象外。skeletonNetaId=骨格ネタのベース明示区間で表面化＝書いた区間(クリシェ/ペダル)だけベース音を上書き・省略区間はコード導出のまま(design #20)。**relative**=true で style 型経路を実音化せず**相対パターン**（度数×16分グリッド）content で出力＝配置先の進行/調に当てて再生時に解決し、採用後は**ベースエディタのパターンとして再編集できる**（「鳴らし方」がネタに常在＝修理#2/H2）。相対はstyle指定時のみ有効・skeleton明示ベース/6-8は絶対のまま(escape hatch)・未指定=絶対notes=従来bit一致。", inputSchema: { frame: frameSchema, chords: chordsSchema.optional(), seed: z.number().int().optional(), drums: drumsSchema, kickLock: z.number().min(-1).max(1).optional().describe("キック骨格 -1..1。正=キックstepにオンセットを乗せる率(小節頭は常に弾く)、負=逆相(キックに無い8分裏へ)。0=従来fig経路"), snareGap: z.number().min(0).max(1).optional().describe("スネア頭で音価を切る強さ 0..1（onset不変・最小dur16分）＝2・4に穴を空けてスネアを抜く"), approach: z.number().min(0).max(1).optional().describe("コードチェンジ直前の接近音化率 0..1（弱拍・短音のみ＝半音上下/全音下→次ルート着地）"), style: z.string().optional().describe("ベース定型型ID or ジャンル名(rock/ballad/citypop/funk/edm/vocarock)。度数×16分グリッドの型を実音化＝型格子が正準(kickLockと排他)。未指定=従来。**JZ-WALK**=ウォーキングベース(M3・phrase_maker walking v2の候補生成＋v3の規則3本＝禁則音程(トライトーン/長7度/オクターブ超)・跳躍後の順次回復・同方向連続跳躍の禁止を制約に。区間頭はルート・チェンジ直前は次ルートへ±2・ガイドトーン(3度/7度)を通す弧)。1拍1歩、6/8は付点拍＋ピックアップの4歩。**耳未判定**(規則は耳の言葉から出ているが効いたことは未確認)＝名指しした時だけ立つ試作。コードが要る"), fill: z.union([z.number().min(0).max(1), z.string()]).optional().describe("フィル 0..1(小=下降/大=上昇) or 型ID(FL-WALKUP等)。末尾1つ手前の小節に駆け上がり/下がりを挿入。未指定=なし(従来)"), slashBass: z.boolean().optional().describe("分数コード/転回の低音伝播 true=chord.bass(分数コード G/B や IAC 第1転回の指定低音)をアンカー(小節頭/チェンジ頭)のベース音に使う。間の5度/octは和音root基準のまま。既定OFF=chord.bassを無視=従来bit一致(web譜と対称化)"), swing: z.number().min(0).max(1).optional().describe("跳ね 0=ストレート〜1=シャッフル(8分裏が3連2/3へ)＝content.feel に載る(notesはストレート・再生/書き出しの applyFeel がベース part で跳ねさせる)。未指定=なし(feelキー無し=従来bit一致)"), humanize: z.number().min(0).max(1).optional().describe("人間味(グルーヴ) 0=機械的〜1=微小タイミング揺れ＝content.feel に載る(feel層・ベース part プロファイル)。未指定=なし(feelキー無し=従来bit一致)"), relative: z.boolean().optional().describe("true=style 型経路を相対パターン(mode:relative・度数×16分グリッド)で出力＝実音化せず配置先の進行/調に当てて解決し、ベースエディタのパターンとして再編集可(修理#2/H2)。style指定時のみ有効(skeleton明示ベース/6-8/style未指定は絶対のままフォールバック=返りに relativeFallback 理由)。未指定=絶対notes=従来bit一致"), skeletonNetaId: z.string().optional().describe("骨格ネタ(kind=skeleton)のid。骨格の明示ベース区間(bass)だけをベース音として上書き＝表面化(design #20)。骨格休符(pitch:null)区間はベースも鳴らさない。明示ゼロ/未指定=従来コード導出とbit一致。返りに skeletonNetaId が入る＝capture 後 link(ベース→骨格,\"realized_from\")"), respondToCues: z.boolean().optional().describe("カスケード合図(frame.section.cues)への応答 既定true。false=このトラックはcuesを一切読まない(fill位置/land着地とも従来経路のまま)＝そのトラックだけ合図に乗らない自由"), anchorLock: z.boolean().optional().describe("錨と間の分業(M3・phrase_maker移植) true=drumsのキックstepに錨を置く＝**コードの変わり目のキックは必ずルート**・同じコードが続く間のキックは体の音が構成音/5度/b7ならそのまま(2026-09-15裁定・anchorStrictnessで全キックルートの源流互換へ戻せる)。錨の**間**のリフ本体は1音も書き換えない。kickLock(確率でキック共有率を近づける)と違って構造的＝seedに依らず骨が安定する。体(錨を打つ先の格子)はstyle型の16分格子、style未指定ならルートペダル＋ペンタ答句のリフ文法。**kickLockとは排他(anchorLockが勝つ)・styleとは併用**。drumsが要る・4/4系のみ。未指定=false=従来bit一致。経路が立たない時は返りの **meta.warnings** に日本語の理由が載る(ドラム無し/複合拍子/ドラム1小節長の不一致/格子が16分に写せない/**ドラムにキックが無い**/**骨格でベースを明示している**＝いずれも当てずに従来どおり生成)。**approach とは併用可**(錨は接近音化しない)・**fill とは併用可だがフィル小節は音形ごと差し替わるので錨が残らない**(その事実も meta.warnings に数つきで載る)"), anchorRestOnSyncopatedKick: z.boolean().optional().describe("anchorLock時の案B true=**拍頭でない無音キックではベースを休む**(ベースが元々鳴っていないシンコペのキックにだけ錨を挿さない＝ドラムのキックが単独で鳴る。変わり目のキックは必ず挿す)。既定=anchorStrictnessがchord-change(既定)ならtrue・every-kickならfalse。明示のtrue/falseが勝つ"), anchorStrictness: z.enum(["chord-change", "every-kick"]).optional().describe("anchorLock時の錨の厳しさ。chord-change(既定)=コードの変わり目のキックだけルート必須・滞在中のキックは体の音がそのコードの構成音/5度/b7なら上書きしない(答句が生き残る)。every-kick=全キックに必ずルート(phrase_maker源流と同じ)。anchorLock無しで渡すと効かない旨を meta.warnings に"), riffVariation: z.number().min(0).max(1).optional().describe("錨(anchorLock)の文法の体にだけ効く変奏量(design.md 追補 (k-7))。0=従来(未指定と同じ・bit一致)／0.5=2回目以降の2小節の答句をリフ語彙の階段で1段ずらす／1=さらに途中で答句を断片化して間を作り・最後の2小節で答句を1拍前へ広げ・装飾(ベース=直前ペダルのオクターブ)。それ以外は最寄りへ丸めて告げる。変奏の中身は仮(耳で決める)。content に bassRiff{grammar,variation}。錨・後段に打ち消されて従来と同じ音になったら meta.warnings"), riffVariationSteps: z.boolean().optional().describe("true=変奏なし/中/多めの3件を items に並べる(label=変奏なし（従来）/変奏 中/変奏 多め)。riffVariation より優先。錨の文法の体が立たない時は1件＋理由"), anchorGrammar: z.enum(BASS_GRAMMAR_IDS).optional().describe("anchorLock の体に敷くリフ文法(M3・phrase_maker riff.py の3文法)。pedal_answer=ルートペダル＋ペンタ答句(既定)／gallop_pedal=ギャロップ(8分＋16分×2)＋ブルーノート＋半音クライム／octave_call_response=オクターブの呼応(弦跳びstab)。**style を指定した時は style 型の格子が体**＝この値は使われない。未知IDは既定へ落として返りの anchorGrammarFallback に理由"), chordFollow: z.boolean().optional().describe("コード追従の5ガード(M3・phrase_maker移植) true=**リズムは1音も動かさず音高だけ**を進行へ写し直す：①拍頭は必ずコードトーン②それ以外はそのコードのスケール(層B 25クオリティ×教科書スケール)③各コード区間の最後の自由音は次のコードのルートへの導音(半音優先・必ず±2で解決)④低域窓の内側。錨(anchorLock)が置いた音は触らない。**approach ノブとは排他**(両方来たらchordFollowが勝つ＝二重に接近音を作らない)＝approachの上位互換。コードが要る・4/4系のみ。未指定=false=従来bit一致。立たない時は返りの **meta.warnings** に日本語の理由が載る(コードが無い/複合拍子)") } },
    async ({ frame, chords, seed, drums, kickLock, snareGap, approach, style, fill, slashBass, swing, humanize, relative, skeletonNetaId, respondToCues, anchorLock, anchorRestOnSyncopatedKick, anchorGrammar, anchorStrictness, chordFollow, riffVariation, riffVariationSteps }) => {
      ...
      const res = genBass(frame, chords, seed, drums, { kickLock, snareGap, approach, skeleton, style, fill, slashBass, swing, humanize, relative, respondToCues, anchorLock, anchorRestOnSyncopatedKick, anchorGrammar, anchorStrictness, chordFollow, ...(riffVariation != null ? { riffVariation } : {}), ...(riffVariationSteps === true ? { riffVariationSteps: true } : {}) });
  ...
  server.registerTool(
    "gen_chord_pattern",
    { title: "コード楽器パターンを生成", description: "コンピング/アルペジオの伴奏パターン（コード楽器・CP4）を候補生成する。**進行に解決する相対型**＝度数抽象のパターンだけ返し(音は出さない)、実音化は web `resolveChordPattern` が section の進行に当てて行う（現行分業を維持）。content=ChordPatternContent（mode:strum(和音ブロック)/arp(構成音巡回)・voicing(tones/openClose/octave)・steps(16分グリッド)・hits{step,dur,vel?}）。密度は frame.mood/tempo から自動(sparse=小節頭/busy=八分/既定=拍頭)。決定的(seed)。**pattern**=伴奏パターン型辞書(chordLibrary・S2)＝型ID直指定(鍵盤 PB-WHOLE/PB-ARP8/CP-SYNC16/CP-16CUT/DN-OFFBEAT/AN-VERSE/GS-STRIDE/JZ-CHARL 等・ギター GT-FOLK8/GT-DOWN8/GT-DU16/GT-FUNK16/GT-SKANK/GT-POWER16 等・オルガン OG-PAD/OG-PAD2/OG-STAB/OG-SOUL/OG-PUNCH＝アレンジS1 2026-08-02・ジャンル名では選ばれず型ID直指定 or おまかせ候補で出る)または ジャンル名(ballad/rock/citypop/dance/anison/gospel/jazz/folk/funk/reggae/pop/metal＝frame.section.role＋tempo域で候補型を絞り決定的1つ)。型の16分グリッド(vel層＝アクセント112/弱64/ギターのアップ78/ゴースト40)を**サイクル(型の bars・既定1小節)単位**で敷き、セクション末で切り詰める(はみ出す hit は落とし・跨ぐ hit は dur を詰める＝2小節型×7小節＝3周＋前半1小節。アレンジS1 contract④)。白玉パッド型は content.followChords=true が載る＝実音化側(web)がコード境界で再ボイシングする印(contract③)。4/4系のみ・域外/未解決は従来経路。**style**=ボイシング奏法 keyboard(既定・鍵盤的クローズド積み)/guitar(ギター的＝最低声=根音・3度1個・根音/5度オクターブ重複)。ギター型は自動で style=guitar＋strumMs 相場が載る。**strumMs**=弦順ロールの1弦あたり時差(ms)＝guitar×strum のとき和音内各声をダウン=低→高に決定的にずらす(実音化は web・研究doc 2026-07-22)。pattern/style/strumMs 未指定=従来 bit 一致(voicing にキーを生やさない)。返り kind=\"chord_pattern\"。", inputSchema: { frame: frameSchema, seed: z.number().int().optional(), pattern: z.string().optional().describe("伴奏パターン型ID(PB-WHOLE/CP-SYNC16/GT-FOLK8 等)またはジャンル名(ballad/rock/citypop/dance/anison/gospel/jazz/folk/funk/reggae/pop/metal)。frame.section.role＋tempo域で候補を絞り決定的に選択。4/4系のみ。未指定/未解決=従来"), style: z.enum(["keyboard", "guitar"]).optional().describe("ボイシング奏法。keyboard=鍵盤的クローズド積み(既定)/guitar=ギター的ボイシング(最低声=根音・3度1個・根音/5度重複)。未指定=keyboard=従来(pattern がギター型なら自動 guitar)"), strumMs: z.number().min(0).max(60).optional().describe("弦順ロールの1弦あたり時差(ms・0〜60)。guitar×strum で各声を低→高に決定的にずらす(実音化は web)。相場=10ms/弦・バラードは20〜35。0/未指定=時差なし=従来bit一致(pattern がギター型なら型の相場が載る)"), variety: z.number().int().min(1).max(8).optional().describe("候補数(スライスC「聴いて選ぶ」)。>=2 かつ pattern がジャンル名/おまかせ(omakase・型IDでない)のとき、GENRE_TABLE の role/tempo 適合から**別々の型**を最大 n 件 items へ並べる(各 label=型ID＋場面タグ)。型ID直指定は単数固定。未指定/1=従来の単数=bit一致。"), swing: z.number().min(0).max(1).optional().describe("跳ね 0=ストレート〜1=シャッフル(8分裏が3連2/3へ)＝content.feel に載る(hitsはストレート・再生/書き出しの applyFeel がコード part で跳ねさせる)。未指定=なし(feelキー無し=従来bit一致)"), humanize: z.number().min(0).max(1).optional().describe("人間味(グルーヴ) 0=機械的〜1=微小タイミング揺れ＝content.feel に載る(feel層・コード part プロファイル)。未指定=なし(feelキー無し=従来bit一致)"), guitarRiff: z.enum(GUITAR_GRAMMAR_ID_ENUM).optional().describe("ギターのリフ文法(M5・phrase_maker guitar gen2 移植)。power_chug=パワーコードの刻み(ブリッジミュート＋単音の答え)／pedal_answer=低音弦ペダル＋ペンタの答句／gallop=ギャロップ＋ブルーノート＋半音クライム。**相対形**＝hits に voice(mono/power/power8＝ヒット単位のボイシング)と riff 注記(kind/deg/role/anchor/strong)が載り、音高は配置先の進行に当てて web が出す(強拍はコードトーン・弱拍の単音はそのコードのスケール音・dim/aug では5度を落とす)。4/4のみ。pattern/style より優先。未指定=従来bit一致。立たない時は meta.warnings に理由"), anchorLock: z.boolean().optional().describe("guitarRiff 時のみ true=drums のキック step に**必ず** POWER8(R+5+8)の刻みを置く(既存の単音はロックが勝ってパワーコードへ・休符には挿入・ghost/dead は不可侵)。drums が要る。立たない時(ドラム無し/キック無し/1小節長の不一致/格子不一致/guitarRiff 未指定)は meta.warnings に理由"), guitarShape: z.boolean().optional().describe("guitarRiff 時のみ true=音高を『手の形』ソルバ(フォーム=状態・層状Viterbi・移動時間ゲート)で出す。**耳未判定の試作**(重みは otomemo 仮置き)。未指定=コード追従(源流一致)"), keyStab: z.boolean().optional().describe("鍵盤の隙間刺し(M6a・phrase_maker legacy rock_piano／gesture build_rock 移植)。true=drums のキック以外の打点(スネア)に和音を刺す＝キックの隙間＋バックビート。キックしか無ければ2拍裏・4拍裏。相対形(keyboard strum の hits)。4/4のみ・drums が要る。guitarRiff が勝つ。立たない時は meta.warnings に理由。未指定=従来bit一致"), guitarPalmGate: z.number().optional().describe("guitarRiff 時のみ＝刻みの短さ(音価の係数 palmGate・0.1〜1・大きいほど長く鳴る)。未指定=文法の源流値(power_chug 0.80/pedal_answer 0.85/gallop 0.65＝8分の刻みが8分音符の約34%)。範囲外は丸めて meta.warnings"), guitarGhostVel: z.number().optional().describe("guitarRiff 時のみ＝弱音(ゴースト)の vel 1〜127。未指定=46(源流値)。範囲外は丸めて meta.warnings"), riffVariation: z.number().min(0).max(1).optional().describe("guitarRiff 時のみ＝繰り返しの変奏量(design.md 追補 (k-7))。0=従来(未指定と同じ・bit一致)／0.5=2回目以降の2小節の単音の答句をリフ語彙の階段で1段ずらす／1=さらに途中で答句を断片化して間を作り・最後の2小節で答句の直前1拍の刻みを単音の答句へ広げ・ゴーストの付け外し。それ以外は最寄りへ丸めて告げる。変奏の中身は仮(耳で決める)。content.guitarRiff.variation に印。キックの刻み・和音追従に打ち消されて従来と同じ音になったら meta.warnings"), riffVariationSteps: z.boolean().optional().describe("guitarRiff 時のみ true=変奏なし/中/多めの3件を items に並べる(label=変奏なし（従来）/変奏 中/変奏 多め)。riffVariation より優先"), drums: drumsSchema, chords: chordsSchema.optional().describe("進行。guitarRiff 時に渡すと api が web と同じ実音化で検算し、代用したコード表・押さえられない音・手の形の緩和を meta.warnings で告げる(content は相対形のまま)") } },
    async ({ frame, seed, pattern, style, strumMs, variety, swing, humanize, guitarRiff, anchorLock, guitarShape, keyStab, guitarPalmGate, guitarGhostVel, riffVariation, riffVariationSteps, drums, chords }) => {
      const gtr = guitarRiff != null || anchorLock != null || guitarShape != null || keyStab != null || guitarPalmGate != null || guitarGhostVel != null || riffVariation != null || riffVariationSteps != null;
      return ok(genChordPattern(frame, seed, pattern != null || style != null || strumMs != null || variety != null || swing != null || humanize != null || gtr ? { pattern, style, strumMs, variety, swing, humanize, ...(gtr ? { guitarRiff, anchorLock, guitarShape, drums, chords, ...(keyStab != null ? { keyStab } : {}), ...(guitarPalmGate != null ? { guitarPalmGate } : {}), ...(guitarGhostVel != null ? { guitarGhostVel } : {}), ...(riffVariation != null ? { riffVariation } : {}), ...(riffVariationSteps === true ? { riffVariationSteps: true } : {}) } : {}) } : undefined));
    },
  );
  server.registerTool(
    "gen_drums",
    { title: "ドラムを生成", description: "GMバックビート（16分グリッド hits=step index・4/4で16step/6-8で12step・seedで小変化）。content.rhythm.beatsPerStep で拍換算。**style**=定型ビート型ID(beat8.syncopated/four.rock/beat16.ghost/six8.ballad 等)またはジャンル名(jpop/rock/dance/ballad/funk＝frame.section.role＋tempo域で候補型を絞り選択)。**fill**=0..1(小さい=軽い節目/大きい=大遷移フィル)またはフィル型ID(fill.tom.asc.half 等)＝frame.bars本の末尾遷移小節へフィル挿入＋着地(次小節頭crash+kick)・他小節不変。**ビルドアップ型**(build.tight.4bar=J-popプリコーラス/build.standard.8bar=汎用/build.big.16bar=大サビ前)＝密度倍加(4→8→16分)＋vel単調漸増＋末尾無音ギャップ＋ビルド区間キック抜き→着地でドロップ復帰（要 frame.bars≥テンプレ小節+1）。型はstraight格子でスイング/timingはfeel層へ委譲。**fillStyle**=grid(既定・定型ビート格子)/physical(M2・phrase_maker固定10型フィルをnoteレベルでfillNotesへ)/body(M3・身体シミュレータ＝型を引かずDPで解く。骨=いま鳴っているグルーヴを引用)。style/fill 未指定=従来 bit 一致。6/8は6/8対応型のみ(ビルド型は4/4)。**humanize/swing**=ノリ(演奏レイヤー)＝content.feelに載る(hitsはストレート・未指定=既定なし・fillStyle:\"body\"のみ既定humanize=0.25)。", inputSchema: { frame: frameSchema, seed: z.number().int().optional(), style: z.string().optional().describe("定型ビート型ID or ジャンル名(jpop/rock/dance/ballad/funk)。未指定=従来"), fill: z.union([z.number().min(0).max(1), z.string()]).optional().describe("フィル 0..1(強さ) or 型ID(fill.*/build.* ビルドアップ)。指定でセクション末小節にフィル＋着地。未指定=なし(従来)"), fillStyle: z.enum(["grid", "physical", "body"]).optional().describe("フィルの実装。grid=定型ビート格子(既定・従来)/physical=phrase_maker物理フィル(M2)＝note レベル(三連/32分/フラム保持)をfillNotesに載せN小節へ展開。型はcue.aimプール選抜(fillKindで明示可)/body=身体シミュレータ(M3)＝型を引かずDPで手の運びを毎回解く。未指定=grid=従来bit一致"), fillKind: z.string().optional().describe("physical時のフィル型を明示(tom_descent/triplet_cascade/snare_roll/snare_roll_32/herta/gallop/buildup/flam_accents/sixteenth_groove/offbeat_syncopated)。未指定=cue.aimプール/seedで選抜"), fillLength: z.union([z.number(), z.enum(["beat", "2beat", "half_bar", "bar"])]).optional().describe("フィルの長さ(拍数の数値 or beat/2beat/half_bar/bar)。未指定=型と強度で既定(physical/body共通)"), fillBeat: z.number().optional().describe("フィルの開始拍(小節内。未指定=末尾から逆算・physical/body共通)"), bodyDepth: z.number().min(0).max(1).optional().describe("body専用：手が刻みからどれだけ逸脱するか(0=グルーヴ寄り〜1=大暴れ)。未指定=既定"), bodyDensity: z.number().min(0).max(2).optional().describe("body専用：打点の密度(GMD prior下では評価されない=source仕様)。未指定=既定"), bodyCrescendo: z.number().min(0).max(1).optional().describe("body専用：終盤へのvel漸増の強さ。未指定=既定"), bodyTailAnchor: z.number().min(0).max(1).optional().describe("body専用：着地(crash+kick)へ両手を収束させる強さ。未指定=既定"), bodyDrummer: z.string().optional().describe("body専用：手癖prior(GMD_PRIORSのキー、\"none\"で純物理)。未指定=既定drummer"), humanize: z.number().min(0).max(1).optional().describe("人間味(グルーヴ) 0=機械的〜1=微小タイミング揺れ＝content.feelに載る(演奏レイヤー・hitsは不変)。未指定=なし(fillStyle:\"body\"のみ既定0.25)。0明示で切れる"), swing: z.number().min(0).max(1).optional().describe("跳ね 0=ストレート〜1=シャッフル＝content.feelに載る。未指定=なし(従来bit一致)") } },
    async ({ frame, seed, style, fill, fillStyle, fillKind, fillLength, fillBeat, bodyDepth, bodyDensity, bodyCrescendo, bodyTailAnchor, bodyDrummer, humanize, swing }) => {
      const res = genDrums(
        frame, seed,
        style != null || fill != null || fillStyle != null || fillKind != null || fillLength != null || fillBeat != null
          || bodyDepth != null || bodyDensity != null || bodyCrescendo != null || bodyTailAnchor != null || bodyDrummer != null
          || humanize != null || swing != null
          ? { style, fill, fillStyle, fillKind, fillLength, fillBeat, bodyDepth, bodyDensity, bodyCrescendo, bodyTailAnchor, bodyDrummer, humanize, swing }
          : undefined,
      );
  ...
  server.registerTool(
    "suggest_lyric_rhythm",
    {
      title: "歌詞→リズム型候補",
      description:
        "「この歌詞、どんなリズムで歌えばノる？」「メロを作る前に歌詞の譜割り（音の詰め方）を先に見たい」のように、歌詞のリズム型を先に確かめたいとき使う。歌詞(かな)をモーラに分けて『リズム型』候補を複数出す（ピッチは付けない＝割付の型のみ・**提案であって保存物ではない**）。候補=基本(1モーラ1音符)/細分(字余り・早口)/句末伸ばし(字足らず・メリスマ)。特殊拍の扱い＝この提案では長音ーは直前へ延長(tie)・促音っは詰め(rest)＝**音符を立てない**（撥音んは独立音符・拗音きゃは1モーラ）＝オーナー裁定で確定(design §31-8)。句頭が助詞/接続詞/感動詞なら弱起(pickup)を提案。漢字は先に読み(かな)へ。",
  ...
  server.registerTool(
    ...
    async ({ frame, chords, seed, style, repetition, rangeSteps, motifBars, density, swing, expression, phrasing, runs, push, foreground, breathe, humanize, form, bass, counter, drums, backbeat, drumLock, converse, hook, articulation, inflect, motifMode, finest, flow, pickup, arc, skeletonNetaId, rhythmParts, lyrics, readLyrics, corpus, corpusStrength, rhythmicContrast, cadenceSoprano, vlWeight, vlAtCadenceOnly }) => {
      ...
      if (lyrics && lyrics.trim()) {
        ...
        // スライス7（design §31-10）＝**読みは呼び側（ここ）で先に解く**＝planLyricMelody は同期のまま。
        // 既定OFF（readLyrics 未指定）＝表記の字をそのまま数える従来のまま＝bit一致。取れなければ黙って従来へ落ちる
        // （読みが無いより生成が止まるほうが害＝§31-3 と同じ流儀）。返りの lyricPlan.readingSource に出所を出す。
        let readings: string[] | undefined;
        let readingSource: "none" | "pyopenjtalk" | "failed" = "none";
        if (readLyrics) {
          try {
            const rs = await extractReadings(rawLines.map((l) => l.trim()));
            readings = rs.map((r) => (r.error ? "" : r.moras.map((m) => m.kana).join("")));
            readingSource = readings.some((k) => k.length > 0) ? "pyopenjtalk" : "failed";
            if (readingSource === "failed") readings = undefined;
          } catch {
            readingSource = "failed"; // venv 未導入・時間切れ等＝従来の数え方で続ける
          }
        }
        const plan = planLyricMelody(rawLines, { bars: barsN, beatsPerBar: barLen, readings });
        ...
        (resL as typeof resL & { accentSource?: string; lyricPlan?: unknown }).lyricPlan = {
          syllableCount: plan.syllables.length, lines: plan.lines, warnings: plan.warnings,
          readingSource, // 読みの出所＝耳で確かめる側の手がかり（数え方は §31-8 で従来一本に確定）
        };
        return ok(resL);
      }
