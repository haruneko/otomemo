...
describe("genMotifMelodyV2（A2レシピ＝骨格＋選別＋輪郭駆動＋発展＋弧）", () => {
  ...
  it("㉟ 長尺(16小節)sentence：断片化はカデンツ直前ブロックに集中＝中間は反復（対策6・畳み掛け→解放）", () => {
    // 16小節=6ブロック(mb=2)。bi/seq/seq/seq/frag/cad。継続断片(bar12-13=frag)が中間(bar8-9=seq)より密＝
    // 提示は反復で伸ばし畳み掛けは最後だけ（旧＝中間全部fragで加速しっぱなし）。
    const R16 = [...ROOTS, ...ROOTS], Q16 = [...QUALS, ...QUALS], p16 = R16.map((r, i) => chordPcs(r, Q16[i]!));
    let fragSum = 0, midSum = 0;
    for (let seed = 1; seed <= 24; seed++) {
      const notes = genMotifMelodyV2(p16, R16, Q16, sp, motif16, { seed, tonicPc: 0, minor: false, form: "sentence" });
      const d = new Array(16).fill(0); notes.forEach((n) => { const b = Math.floor(n.start / 4); if (b >= 0 && b < 16) d[b]++; });
      fragSum += d[12]! + d[13]!; // カデンツ直前ブロック(frag)
      midSum += d[8]! + d[9]!;    // 中間ブロック(seq=反復・畳み掛けない)
    }
    expect(fragSum, `frag直前(${fragSum}) > 中間seq(${midSum})＝畳み掛けは末尾集中`).toBeGreaterThan(midSum);
  });
  ...
  it("F1 finest＝最小音符上限：未指定=従来bit一致／eighth=16分裏onsetゼロ（高BPM対策・オーナーFB）", () => {
    const genFin = (seed: number, finest?: "quarter" | "eighth") =>
      genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed, tonicPc: 0, minor: false, finest });
    const is16 = (t: number) => Math.abs(((t * 4) % 2) - 1) < 0.1;
    for (let seed = 1; seed <= 20; seed++) {
      expect(JSON.stringify(genFin(seed)), `seed=${seed} 未指定=bit一致`).toBe(JSON.stringify(gen(seed)));
    }
    let off8 = 0, off4 = 0, tot = 0;
    for (let seed = 1; seed <= 40; seed++) {
      for (const n of genFin(seed, "eighth")) { tot++; if (is16(n.start)) off8++; }
      for (const n of genFin(seed, "quarter")) if (is16(n.start) || Math.abs(((n.start * 4) % 4) - 1) < 0.1 || Math.abs(((n.start * 4) % 4) - 3) < 0.1) off4++; // 4分外(8分/16分)
    }
    expect(off8, "eighth＝16分裏onsetゼロ").toBe(0);
    expect(off4, "quarter＝8分/16分onsetゼロ").toBe(0);
  });
  ...
  it("P2 往復再現（本丸）：seedMotif=オーナー例(A A B C A/B G G)→提示ブロックで反復音が同pitch保存・輪郭符号一致", () => {
    // move は半音差（エンジン共通単位）：0,0,+2,+1,-3（ラ ラ シ ド ラ）／0,-4,0（シ ソ ソ）＝反復音を含む動機。
    // renderPreserve が半音→段換算＝deg [0,0,1,2,0]→E,E,F,G,E（同度=反復音・順次上行・回帰）。
    const seedMotif = { ons: [0, 0.5, 1, 1.5, 2, 4, 4.5, 5], mv: [0, 0, 2, 1, -3, 0, -4, 0], run: Array(8).fill(false) };
...
// ── Stage1 句フレージング（flow=連結/長音・pickup=弱起・arc=山なり弧）2026-07-11・オーナーFB「塊がぶつ切れ」 ──
describe("句フレージング（flow / pickup / arc・既定=bit一致・後段onset不変）", () => {
  const genO = (seed: number, o: Record<string, unknown>) =>
    genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed, tonicPc: 0, minor: false, ...o }).sort((a, b) => a.start - b.start);
  const base = (seed: number) => genO(seed, {});

  it("flow 未指定＝dur含め従来bit一致（回帰ゼロ）", () => {
    for (let s = 1; s <= 8; s++) expect(genO(s, { flow: undefined })).toEqual(base(s));
  });
  it("flow>0＝1.6拍の上限を破って白玉が出る＋≥8拍の連続歌唱塊が現れる（穴が埋まる）", () => {
    let maxDur = 0, seg8 = 0, segTot = 0;
    for (let s = 1; s <= 12; s++) {
      const notes = genO(s, { flow: 0.7 });
      for (const n of notes) maxDur = Math.max(maxDur, n.dur);
      let segStart = notes[0]!.start, prevEnd = notes[0]!.start + notes[0]!.dur;
      for (let i = 1; i < notes.length; i++) {
        if (notes[i]!.start - prevEnd > 0.5) { if (prevEnd - segStart >= 8) seg8++; segTot++; segStart = notes[i]!.start; }
        prevEnd = Math.max(prevEnd, notes[i]!.start + notes[i]!.dur);
      }
      if (prevEnd - segStart >= 8) seg8++; segTot++;
    }
    expect(maxDur).toBeGreaterThan(2);          // 既定の1.6上限を突破＝長音/白玉
    expect(seg8 / segTot).toBeGreaterThan(0.15); // ≥8拍塊が15%超（既定は0%）
  });
  it("flow＋phrases＝句末に必ず息を残す（全レガート化しない・オーナーFB「息継ぎが無いのもダメ」）", () => {
    // sentence[2,2,4] 相当の句末（bar2/bar4/最終）。flow を強めても句末境界に息(>0.5拍gap)が残ること。
    const phrases = [
      { startBeat: 0, beats: 8, cadenceDegree: 5 },
      { startBeat: 8, beats: 8, cadenceDegree: 1 },
      { startBeat: 16, beats: 16, cadenceDegree: 1 },
    ];
    for (let s = 1; s <= 12; s++) {
      const notes = genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed: s, tonicPc: 0, minor: false, flow: 0.6, pickup: 0.5, phrases }).sort((a, b) => a.start - b.start);
      let breaths = 0, prevEnd = notes[0]!.start + notes[0]!.dur;
      for (let i = 1; i < notes.length; i++) { if (notes[i]!.start - prevEnd > 0.5) breaths++; prevEnd = Math.max(prevEnd, notes[i]!.start + notes[i]!.dur); }
      expect(breaths, `seed=${s} は息継ぎゼロ（全レガート）`).toBeGreaterThanOrEqual(1); // どのseedも最低1つは息がある
    }
  });
  it("flow＋phrases＝句末の歌い終わり位置が句ごと・seedごとにばらつく（対策2-C・終止の単峰解消）", () => {
    const phrases = [
      { startBeat: 0, beats: 8, cadenceDegree: 5 },
      { startBeat: 8, beats: 8, cadenceDegree: 1 },
      { startBeat: 16, beats: 16, cadenceDegree: 1 },
    ];
    const hist: Record<string, number> = {}; let total = 0;
    for (let s = 1; s <= 24; s++) {
      const notes = genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed: s, tonicPc: 0, minor: false, flow: 0.6, pickup: 0.5, phrases }).sort((a, b) => a.start - b.start);
      let prevEnd = notes[0]!.start + notes[0]!.dur;
      for (let i = 1; i < notes.length; i++) {
        if (notes[i]!.start - prevEnd > 0.5) { const p = (Math.round(prevEnd * 2) / 2).toString(); hist[p] = (hist[p] ?? 0) + 1; total++; } // 歌い終わり拍(0.5刻み)
        prevEnd = Math.max(prevEnd, notes[i]!.start + notes[i]!.dur);
      }
    }
    const top = Math.max(...Object.values(hist));
    expect(Object.keys(hist).length).toBeGreaterThan(6); // 着地位置が多様
    expect(top / total).toBeLessThan(0.4); // 最頻の着地でも占有<40%＝単峰でない
  });
  it("flow は onset(start)集合を変えない＝反復・和声後処理と無干渉（durのみ延長）", () => {
    for (let s = 1; s <= 6; s++) {
      const a = base(s).map((n) => n.start), b = genO(s, { flow: 1 }).map((n) => n.start);
      expect(b).toEqual(a);
    }
  });

  it("pickup 未指定＝従来bit一致（回帰ゼロ）", () => {
    for (let s = 1; s <= 8; s++) expect(genO(s, { pickup: undefined })).toEqual(base(s));
  });
  it("pickup>0＝弱起（小節境界直前から始まる音）が増える", () => {
    const anacrusis = (o: Record<string, unknown>) => {
      let c = 0;
      for (let s = 1; s <= 12; s++) for (const n of genO(s, o)) { const b = n.start % 4; if (b > 3.05 && b < 3.95) c++; }
      return c;
    };
    expect(anacrusis({ pickup: 0.6 })).toBeGreaterThan(anacrusis({}));
  });

  it("arc 未指定＝従来bit一致（回帰ゼロ）", () => {
    for (let s = 1; s <= 8; s++) expect(genO(s, { arc: undefined })).toEqual(base(s));
  });
  it("arc='arch'＝決定的・音域維持（山なり弧・合法性不変）", () => {
    for (let s = 1; s <= 6; s++) {
      const a = genO(s, { arc: "arch" }), b = genO(s, { arc: "arch" });
      expect(a).toEqual(b); // 決定的
      for (const n of a) { expect(n.pitch).toBeGreaterThanOrEqual(sp[0]!); expect(n.pitch).toBeLessThanOrEqual(sp[sp.length - 1]!); }
    }
  });
});

// ── swing×16分の衝突ガード（2026-07-11・オーナーFB「JRockのリズムが崩れる」）──
describe("swing×16分連＝フラム回避（8分裏の直後に16分がある所は跳ねない）", () => {
  const genSR = (seed: number, o: Record<string, unknown>) =>
    genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed, tonicPc: 0, minor: false, ...o }).sort((a, b) => a.start - b.start);
  it("swing強×runs（16分連）でも極短音(<0.15拍)・近接onset(<0.13拍)が出ない＝フラム状にならない", () => {
    let tiny = 0, collide = 0, total = 0;
    for (let s = 1; s <= 20; s++) {
      const notes = genSR(s, { density: 0.7, swing: 0.9, runs: 0.5 });
      total += notes.length;
      for (let i = 0; i < notes.length; i++) {
        if (notes[i]!.dur < 0.15) tiny++;
        if (i > 0) { const g = notes[i]!.start - notes[i - 1]!.start; if (g > 0.01 && g < 0.13) collide++; }
      }
    }
    expect(tiny, "極短音（フラム）が残っている").toBe(0);
    expect(collide, "0.13拍未満の近接onset（衝突）が残っている").toBe(0);
  });
  it("16分が続かない8分裏は従来どおり跳ねる（swing が無効化されていない）", () => {
    let swung = 0;
    for (let s = 1; s <= 20; s++) for (const n of genSR(s, { density: 0.5, swing: 0.9 })) { const f = ((n.start % 1) + 1) % 1; if (f > 0.6 && f < 0.7) swung++; }
    expect(swung, "跳ね（x.65付近のonset）が全く出ていない").toBeGreaterThan(20);
  });
  it("swing 未指定＝従来 bit一致（回帰ゼロ）", () => {
    for (let s = 1; s <= 8; s++) {
      const a = genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed: s, tonicPc: 0, minor: false, density: 0.7, runs: 0.5 }).sort((x, y) => x.start - y.start);
      const b = genSR(s, { density: 0.7, runs: 0.5, swing: undefined });
      expect(b).toEqual(a);
    }
  });
});
