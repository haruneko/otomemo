...
describe("genMotifMelodyV2（A2レシピ＝骨格＋選別＋輪郭駆動＋発展＋弧）", () => {
  ...
  it("㉛ humanize=1＝velが付き(強弱)・start/dur は不変（timing揺れは feel 層へ移行）・決定的", () => {
    // 2026-07-11 feel層分離：humanize の velocity はデータ層に残す／タイミング揺れは applyFeel(content.feel.humanize)へ。
    for (const seed of [3, 7, 14]) {
      ...
      // start/dur/pitch は base と同一＝ストレート格子を歪めない（vel だけ足す）。
      const geom = (ns: typeof base) => ns.map((n) => ({ pitch: n.pitch, start: n.start, dur: n.dur }));
      expect(geom(h), `seed=${seed}: humanize は timing を動かさない`).toEqual(geom(base));
    }
...
describe("swing は生成側で焼かない＝feel層へ移行（2026-07-11・design.md「フィール層分離」）", () => {
  const genV2 = (seed: number, o: Record<string, unknown>) =>
    genMotifMelodyV2(pcsPerBar, ROOTS, QUALS, sp, motif16, { seed, tonicPc: 0, minor: false, ...o }).sort((a, b) => a.start - b.start);
  it("swing 指定は genMotifMelodyV2 の出力を変えない（notes は常にストレート・swing は content.feel へ）", () => {
    // 旧「x.5だけ後段書き換え＋衝突band-aid」は撤去。フラム衝突の根絶とスイングの音は applyFeel が担当（feel.test.ts）。
    for (let s = 1; s <= 8; s++) {
      const straight = genV2(s, { density: 0.7, runs: 0.5 });
      const swung = genV2(s, { density: 0.7, runs: 0.5, swing: 0.9 });
      expect(swung, `seed=${s}: swing が V2 出力を歪めている（焼き込みが残存）`).toEqual(straight);
    }
  });
});
