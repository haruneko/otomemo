...
describe("(c) kickLock＝キック骨格（正）／逆相（負）", () => {
  it("kickLock=1: 小節頭+キックstepにオンセット・レガートdur・小節頭=ルート", () => {
    const f: Frame = { bars: 2, meter: "4/4" };
    const d = mkDrums([0, 6, 10], [4, 12]); // キック＝拍0,1.5,2.5
    const out = notesOf(genBass(f, C1, 42, d, { kickLock: 1 }));
    expect(out.map((n) => n.start)).toEqual([0, 1.5, 2.5, 4, 5.5, 6.5]);
    expect(out.map((n) => n.dur)).toEqual([1.5, 1, 1.5, 1.5, 1, 1.5]); // 次オンセット/小節末まで
    expect(out[0]!.pitch).toBe(36); // 小節頭アンカー=ルートC2
    expect(out[3]!.pitch).toBe(36);
  ...
  it("kickLock経路の音域窓は 33..55（従来36..47の張り付きを拡張）", () => {
...
describe("(d) approach＝コードチェンジ直前の接近音", () => {
  ...
  it("approach=1: チェンジ直前の弱拍オンセットが次ルート(F=41)への半音/全音接近・チェンジ頭は41着地", () => {
    const d = mkDrums([0, 8, 14], [4, 12]); // 小節内オンセット＝{0,2,3.5}
    ...
    expect(head2.pitch).toBe(41); // beat1=ターゲット（次ルート）
  });
...
describe("(e) 5度は原則上（root+7 実音）", () => {
  it("root=G(43) でも5度は50（下の38は出ない）・語彙は R/5度上/オクターブ上のみ", () => {
    ...
    for (const seed of SEEDS) {
      ...
      for (const n of out) {
        expect([43, 50, 55], `pitch=${n.pitch}#${seed}`).toContain(n.pitch); // 転回(38)無し
        expect(n.pitch).toBeGreaterThanOrEqual(43);
      }
      if (out.some((n) => n.pitch === 50)) sawFifth = true;
    }
    expect(sawFifth).toBe(true); // 5度が実際に「上(50)」で出る
  });
});
...
describe("API 配線（/music/gen_bass・/gen/section のドラム→ベース結線）", () => {
  ...
  it("/music/gen_bass: drums+ノブを透過（kickLock=1 でキック骨格になる）", async () => {
    ...
    expect(notes.map((n) => n.start)).toEqual([0, 1.5, 2.5, 4, 5.5, 6.5]);
  });
  it("/music/gen_bass: drums 無しは従来と一致（回帰）", async () => {
    ...
    expect(r.statusCode).toBe(200);
    expect(J(r.json())).toBe(J(genBass({ bars: 2, meter: "4/4" }, [{ root: 0, quality: "", start: 0, dur: 8 }])));
  });
