...
describe("completeMelody（部分メロ→モチーフ発展で全小節補完）", () => {
  ...
  it("⑤b preserve既定：反復音を含む partial は後続ブロックでも反復音が同pitchで保存される（Phase2案B）", () => {
    // ラーラシドラ相当（反復音頭）を partial に。tail の発展ブロックで反復ペアが保存されるか。
    const repPartial = [
      { pitch: 64, start: 0, dur: 0.5 }, { pitch: 64, start: 0.5, dur: 0.5 }, // ラーラ（同pitch）
      { pitch: 65, start: 1, dur: 0.5 }, { pitch: 67, start: 1.5, dur: 0.5 }, { pitch: 64, start: 2, dur: 2 },
    ];
    let tailRepBlocks = 0;
    for (const seed of [3, 7, 14]) {
      const out = complete(seed, 8, repPartial).filter((n) => n.start >= 4).sort((a, b) => a.start - b.start); // tail（partial以降）
      // tail に「連続同pitch」ペアが実在＝反復音が発展ブロックで保存
      for (let i = 1; i < out.length; i++) if (out[i]!.pitch === out[i - 1]!.pitch) { tailRepBlocks++; break; }
    }
    expect(tailRepBlocks, "反復音partialのtailで反復が保存されるseedが実在").toBeGreaterThan(0);
  });
