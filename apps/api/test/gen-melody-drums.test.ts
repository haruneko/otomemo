...
// S3d：HTTP は候補へ対位法レポート(meta)を加算する（読み取り専用・ノート不変）。ノブ透過の bit一致は
// ノート内容で見るので meta を剥がして比較する。
...
describe("API 配線（/music/gen_melody・/gen/section の rhythm→melody 結線）", () => {
  ...
  it("/music/gen_melody: drums＋3ノブを透過（direct 呼び出しと一致）・係数0/drums無しは従来と一致", async () => {
    ...
    expect(J(stripMeta(r.json()))).toBe(J(genMelody(FRAME, CHORDS, 14, { useV2: true, drums: DRUMS_RAW, drumLock: 0.5, backbeat: 0.5, converse: 0.5 })));
    ...
    expect(J(stripMeta(r0.json())), "drums有り係数未指定＝従来").toBe(base);
  });
