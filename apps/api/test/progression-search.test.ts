...
describe("findProgressions（進行の連想retrieval）", () => {
  it("タグで足切り（切ない の進行だけ）", () => {
    core.createNeta({ kind: "chord_progression", title: "暗い", content: chords([[9, "m"], [5, ""], [7, ""], [0, ""]]), tags: ["切ない"] });
    core.createNeta({ kind: "chord_progression", title: "明るい", content: chords([[0, ""], [5, ""], [7, ""]]), tags: ["明るい"] });
  ...
  it("構造類似（like に近い進行が上位）", () => {
    core.createNeta({ kind: "chord_progression", title: "カノン寄り", key: 0, content: chords([[0, ""], [7, ""], [9, "m"], [4, "m"]]), tags: [] });
    core.createNeta({ kind: "chord_progression", title: "別物", key: 0, content: chords([[1, "dim"], [6, "aug"]]), tags: [] });
  ...
  it("人気度タグ（ヒット/定番）が同点を押し上げる", () => {
    core.createNeta({ kind: "chord_progression", title: "普通", content: chords([[0, ""]]), tags: ["明るい"] });
    core.createNeta({ kind: "chord_progression", title: "ヒット曲", content: chords([[0, ""]]), tags: ["明るい", "ヒット"] });
    const hits = findProgressions(core, { tags: ["明るい"] });
    expect(hits[0].title).toBe("ヒット曲"); // 人気度ぶん上
  });

  it("該当なしは空（捏造しない）", () => {
    core.createNeta({ kind: "chord_progression", title: "x", content: chords([[0, ""]]), tags: ["明るい"] });
