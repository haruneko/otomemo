...
describe("(D) next_chord 結線（注入無し=bit一致・注入時=コーパス順）", () => {
  ...
  it("直前2コードの trigram が bigram より優先（無ければ bigram フォールバック）", () => {
    const prog2 = [{ degree: 5, quality: "" }, { degree: 7, quality: "" }]; // IV→V の後（V=D機能→候補=T:I/iii/vi）
    const bigram = new Map<string, [string, number][]>([["7q", [["0q", 100]]]]); // V→I を強く
    const trigram = new Map<string, [string, number][]>([["5q>7q", [["9qm", 100]]]]); // IV→V→vi(偽終止) を強く
    expect(nextChordCandidates(prog2, { mode: "major", top: 4, transitions: { bigram, trigram } })[0]!.degree).toBe(9); // trigram 優先＝vi
    expect(nextChordCandidates(prog2, { mode: "major", top: 4, transitions: { bigram, trigram: new Map() } })[0]!.degree).toBe(0); // trigram 無し＝bigram＝I
  });
});
