...
// S1（ピアノ伴奏取り込み・設計書 §5 S1）＝既知の和音の割り当て・届かない組・重みの感度。
// 期待値は phrase_maker handmodel.best_assignment を直接呼んだ値（2026-09-17・読むだけ）。
describe("S1 既知の和音（Python と一致）", () => {
  const cases: [number[], Hand, PyBA][] = [
    [[53, 57, 60, 65], "R", { cost: 0.75, assign: [[1, 53], [2, 57], [3, 60], [5, 65]], wrist_x: 59.8, span: 12 }],
    [[53, 57, 60, 65], "L", { cost: 0.75, assign: [[5, 53], [3, 57], [2, 60], [1, 65]], wrist_x: 57.7, span: 12 }],
    [[52, 55, 59, 64], "R", { cost: 4.75, assign: [[1, 52], [2, 55], [3, 59], [5, 64]], wrist_x: 58.55, span: 12 }],
    [[52, 55, 59, 64], "L", { cost: 2.75, assign: [[5, 52], [3, 55], [2, 59], [1, 64]], wrist_x: 56.45, span: 12 }],
    [[60, 72], "R", { cost: 0.45, assign: [[1, 60], [4, 72]], wrist_x: 68.1, span: 12 }],
    [[48, 52, 55, 60, 64], "R", null],
    [[48, 52, 55, 60, 64], "L", null],
  ];
  for (const [p, h, out] of cases) it(`${p.join(" ")} ${h}`, () => expect(ba(bestAssignment(p, h))).toEqual(out));
  it("届かない組（2オクターブ超）は null", () => {
    expect(bestAssignment([48, 74], "R")).toBeNull();
    expect(canGrab([48, 74], "L")).toBe(false);
  });
});

describe("S1 診断：重みを振ると出力が変わる（検査係でない反証）", () => {
  it("薬指の罰を重くすると F A C（C F A）の右手の割り当てが替わる／小指の罰は F A C F の費用に効く", () => {
    const weak = WEAK as Map<number, number>;
    const b1 = bestAssignment([60, 65, 69], "R");
    const b2 = bestAssignment([53, 57, 60, 65], "R");
    try {
      weak.set(4, 10);
      const a1 = bestAssignment([60, 65, 69], "R");
      expect(b1!.assign.some(([f]) => f === 4)).toBe(true);
      expect(a1!.assign.some(([f]) => f === 4)).toBe(false);
      weak.set(4, 0.45); weak.set(5, 10);
      expect(bestAssignment([53, 57, 60, 65], "R")!.cost).not.toBe(b2!.cost);
    } finally { weak.set(4, 0.45); weak.set(5, 0.75); }
  });
});
