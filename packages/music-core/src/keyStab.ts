...
/** 刺しの音価（16分 step）。源流 legacy＝0.4 拍（≒1.6 step）を整数格子へ＝2 step。次の刺し・次のキック・セクション末で詰める。 */
export const KEY_STAB_DUR_STEPS = 2;

/**
 * 小節ごとに同じ刺しを敷いた ChordHit 列（相対形＝音高は実音化側）。
 * vel は anchor だけ `anchorVel` を載せ、colour は載せない（＝既定の強さ）。
 * `kick`（1小節の step）を渡すと、**鳴っている区間**が次のキックの頭（小節線の向こうの次小節のキックも含む）に掛からないよう音価を詰める
 *   （2026-09-13 M6a 監査 軽微3＝スネア 15 の 2 step が次小節頭のキックまで鳴っていた。「キックに重ならない」は打点でなく鳴る区間で見る）。
 *   ただし最小 1 step（刺しの直後の step がキックなら 1 step＝16分1つ分は鳴らす）。源流 legacy の 0.4 拍も掛かるが、趣旨を優先した。
 */
export function keyStabHits(
  slots: readonly KeyStabSlot[], bars: number, anchorVel: number, stepsPerBar = 16, kick?: readonly number[],
): { step: number; dur: number; vel?: number }[] {
  const out: { step: number; dur: number; vel?: number }[] = [];
  const total = bars * stepsPerBar;
  const kicks = kick ? [...new Set(kick)].sort((a, b) => a - b) : [];
  // step より後の最初のキック（次小節ぶんまで見る）。キックが無ければ Infinity。
  const nextKickAfter = (step: number): number => {
    const b = Math.floor(step / stepsPerBar), local = step - b * stepsPerBar;
    const k = kicks.find((x) => x > local);
    return k != null ? b * stepsPerBar + k : kicks.length ? (b + 1) * stepsPerBar + kicks[0]! : Infinity;
  };
  for (let b = 0; b < bars; b++) {
    for (let i = 0; i < slots.length; i++) {
      ...
      const dur = Math.max(1, Math.min(KEY_STAB_DUR_STEPS, next - step, total - step, nextKickAfter(step) - step));
