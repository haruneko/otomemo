...
~~正典＝本doc。~~ **（2026-07-29 訂正）正典は `docs/design.md` #13c「句頭子音カウントイン」へ昇格済み**（実装済み＝`vocalSourceSchedule` の一般式化・`leadRest` の api SSOT 化。web/api 実コードで確認）。本docは実測と設計根拠のアーカイブ。上流＝`docs/design.md` #13c（仮歌＝メロの楽器）／#25（弱起再生契約）。
