/* render.mjs — v7 の全枚を desc 非表示設定でスクショする（設計論を絵に出さない）。
 *
 * 実行: node render.mjs           … 配布用（demo枚は撮らない）→ shots/v7_<番号>_<枚id>.png
...
const leaked = await p.evaluate(() => {
  ...
  document.querySelectorAll(".frame").forEach((f) => {
    ...
    const t = f.innerText;
    ["裁定", "欠陥", "§", "R-1", "R-2", "v4", "v5", "v6", "却下", "廃止", "工程"].forEach((wd) => {
...
for (const f of frames) {
  ...
  await f.screenshot({ path: path.join(OUT, `v7_${String(++shot).padStart(2, "0")}_${id}.png`) });
  console.log("撮影:", id);
}
