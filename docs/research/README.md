...
- [2026-07-09-melody-theory-gaps-and-plan](2026-07-09-melody-theory-gaps-and-plan.md) — **メロ生成＝理論不足9項目の総点検＋実装計画（別モデル実行用・自己完結）**。発見=legacy⇔V2の機能逆転（倚音/句構造/骨格休符は完成済資産がV2未配線）・16分0%の犯人は語彙でなく選別抑圧(n16Pen/runPen)・同音0%は mkMotif の m=0 潰し1行。計画=Step1 expression(強拍非和声・S)→Step2 P0-b句構造V2配線+骨格休符(S)→Step3 カデンツ選択器→Step4 16分runs/push(本丸1・耳必須)→Step5 motif-driven前景(本丸2)。全ノブ既定=従来bit一致・design#12-M先行・テスト赤先行
