...
- [2026-07-10-68-sixteenth-runs-plan](2026-07-10-68-sixteenth-runs-plan.md) — **6/8で16分が出ない問題の修正方針（方針のみ・実装未着手）**。原因=4/4の「選別抑圧」と違い6/8は**グリッド(8分6枠)と語彙(RHYTHM68_DATA)のレベルで16分が不在**＋run旗全false固定＝runsノブno-op。方針=runs>0時のみ compound を16分12枠へ切替（12枠設計語彙 RHYTHM68X_DATA 新設・run旗解禁・score対応）＝既定bit一致。表面パス案は不採用（runsの効く層が拍子で変わるのは歪）。受入条件5点＋SDD手順あり。スコープ外=swing/push等の6/8対応・コーパス12枠統計(後続任意)
