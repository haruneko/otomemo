...
### ⑤-補（2026-07-10 実装で確定した事項・design「gen_melody×ドラム結線」）
- **残論点1（push × drumLock の合成規則）＝「音単位の排他（実効 max）」に確定**：drumLock 段が push 段より先に走り、食った音は拍頭から外れるので後段 push の対象（拍頭±0.06 マッチ）から**構造的に**外れる＝一音は最大1回・0.25拍しか前借りされない（二重前借りは実装上不可能・契約テストで担保）。両ノブ併用可＝対象拍の和集合。max/排他の場合分けコードは不要になった。
- **残論点2（B 合わせる vs 外す）＝正係数（乗せ）のみ実装**：`backbeat` 0..1＝スネア実在位置 +12/キック +6 の velocity（onset/pitch/dur 不変・humanize の後段・基底 vel??100）。**負係数（外して呼吸）は未実装のまま残論点**（ノブの符号 or 別ノブは耳較正後に）。web 既定＝0.3（弱く）。
- **残論点3（密度の測り方）＝kick+snare+0.3×hihat／小節で実装**（hihat=midi42/44/46 合算・重み0.3 は仮のまま＝実測較正は残）。C は**ブロック単位**（phrases 指定時は句＝句境界を跨がない）で平均→全小節の中央値比 rel→`scale=clamp(1−converse×(rel−1)×0.3, 0.7, 1.3)`。実現＝モチーフ写像への**決定的 onset 追加/削除**（密→弱位置間引き・疎→最大ギャップ中点へ8分挿入・先頭/末尾保持）＝基底モチーフ共有で A/A'' 同一性を維持。一様密度（genDrums の1小節パターン）は rel=1＝**無変化＝bit一致**＝C が効くのは濃淡のある（多小節/実録り）ドラムのみ。
- **残論点6（compound）＝3段とも 6/8 除外で確定**（B は velocity のみで技術的には可能だが、本項の「A/B も当面4/4のみ」に従い保守側）。
- **既定係数（web SectionEditor）**＝rhythm レーンが在る時のみ `backbeat=0.3`・**drumLock/converse は 0＝耳較正待ち**（残論点1の「食いすぎ＝走る」閾値・残論点5の定量ガード較正は実機の耳で）。
- 鉄則の実装形＝drums 無し or 全係数0 で従来と bit 一致（api 契約テスト gen-melody-drums.test.ts・段は独立 seed 派生＝drumLock は seed+61・B/C は rng 不使用の決定則）。メロ×ベースの密度相補は実装せず**ドラム版C一本に統合**（[melody-bass-counterpoint](2026-07-10-melody-bass-counterpoint.md) ⑤-補どおり・二重適用なし）。
