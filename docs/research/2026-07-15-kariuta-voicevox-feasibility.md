...
## 7. 歌唱バンド（使える音域）の実測 — 2026-07-15 追記（B2 修正の根拠）

「♪歌う」実機バグ B2（オクターブ折りの設計誤り）の修正にあたり、**歌声モデル（query=6000 波音リツ）が
「要求した MIDI キーどおりに歌えるか」を実合成で測定**した（`sing_frame_audio_query` の返す `f0` の
持続部の中央値を要求ピッチと突き合わせ・単音「ラ」0.8s＋前後休符）。誤差＝実音高−要求音高（半音）。

| MIDI key | 音名 | 誤差(半音) | 判定 |
|---|---|---|---|
| 45–49 | A2–C#3 | +16〜+28 | ✗ 破綻（低すぎて上へ大暴走） |
| 50 | D3 | +5.9 | ✗ ずれる |
| 51 | D#3 | +16.5 | ✗ 破綻（不安定） |
| **52** | **E3** | **+0.2** | **✓ 正確（実用下限）** |
| 53–79 | F3–G5 | ±0.5 以内 | ✓ 正確 |
| 80 | G#5 | −16〜−21 | ✗ **再現するオクターブ落ちバグ**（80だけ） |
| 81 | A5 | −2 | △ 境界（やや低い） |
| 82–84 | A#5–C6 | −5.5〜−15.8 | ✗ 破綻 |

**結論＝実用歌唱バンド = MIDI [52, 79]（E3〜G5）**。旧実装の `RANGE=[48,72]` は
**下限が既に壊れており（48–51は正確に歌えない）・上限が狭すぎた**（オーナー検体 62–73 の 73 だけが
音ごと折り返しで −12 され旋律の輪郭を破壊）。

**B2 の修正**（`apps/api/src/sing.ts`）：
- 音ごとのオクターブ折り返し（`foldPitch`）を**廃止**。
- 代わりに**メロ全体を k×12 半音シフト**して [52,79] に最も収める k を選ぶ（範囲外音数が最小／同数タイは
  平均をバンド中央へ寄せる）＝**隣接音程列（輪郭）が不変**。
- 全体シフト後もなお外れる音だけ最終手段でクランプし、`shift`・`clamped` を返り値に出して**黙って変えない**
  （`singNeta` がログに残す）。
- オーナー検体 62–73 は [52,79] に収まるため **shift=0・無変換**＝輪郭そのまま歌える。

> 測定スクリプト：本タスクのスクラッチパッド `b3/`（`probe_range2.mjs`／`probe_range3.mjs`・engine 0.25.2 実走）。
> 80 の落ちは単音でも再現＝エンジン側の癖。安全側で上限を 79 に決め打ち。
