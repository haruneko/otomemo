...
### 6.1 occurrence指標の訂正（重要）
「主モチーフ4.5-5.1回＝出過ぎ」は**指標の交絡**＝同音(0音程)連打の「0,0,0」署名を数えてた。
**非自明(動きのある3音程)署名だけ数える**と：実曲2.8回 / 基盤2.0 / 注入2=2.3＝**実曲水準＝motif感は出てる**。
＝occurrence は「非自明署名」で測るのが正準。残る soft な乖離は**同音率(27-34 vs 実曲23・seed振れ大)**のみ＝耳判定領域。
