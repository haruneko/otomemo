...
## 推奨PoC順（費用対効果）
1. **GrooVAE で gen_drums の humanize 後段**（TSネイティブ・即・著作権クリーン・効果明確）。
2. **VOICEVOX で mora_count 実測化＋歌詞 check-repair**（日本語の核・Python不要）。
3. **CA2 ローカルサーバPoC**：「メロ/コード固定→伴奏補完」（重いが理想形・要Python sidecar設計）。
