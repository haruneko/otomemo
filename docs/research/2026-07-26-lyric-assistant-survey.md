...
| 制約充足による日本語作曲Orpheus | 詞を先に決め、韻律を制約として動的計画法で旋律を最適化生成する | https://link.springer.com/chapter/10.1007/978-3-642-04052-8_47 | Otomemoは既にこの設計（Orpheus型二段DP＝設計B）を検討・**不採用と裁定済み**（[lyrics-first-melody-B](2026-07-15-lyrics-first-melody-B.md)＝硬い制約は自作曲を生成禁止にするほど厳しすぎると実測で判明、[lyrics-first-melody-C-audit](2026-07-15-lyrics-first-melody-C-audit.md)）。**既決事項＝再検討不要** |
