...
## ③ 「崩す」を安全弁として強化（①②は2026-06-29 実装済）
1. ✅**崩し強度ノブ**：`genFromEssence(..., {strength∈[0..1]})`。0=従来(輪郭厳密保存・後方互換)／中=歩幅プール拡張([1,2,2,3])／強=広い歩幅([1,2,3,4])＋**向きの確率的揺らぎ(flipP=strength·0.5)**＝面影だけ。**リズム指紋(オンセット)は常に保持**＝「同雰囲気」を残す。ラベルも「連想/崩した/大きく崩した」。
2. ✅**複数参照ブレンド**：`genFromEssence(..., {blendWith:[...melodies]})`＝位置ごとに各参照の輪郭から抽選＝出力が単一源に辿れない（類似の希薄化＝著作権・凡庸さ両対策）。
   - 露出：http `/music gen_from_essence`(strength/blendWith)＋**MCP `reshape mode:"deform"`**（Chat「これ崩して/似た雰囲気で別案」）。実機(http)で strength0=寄せ／strength1=別物(ノリ保持)／blend=身振り混合 を確認。**残＝リズム指紋自体の変形(augmentation/diminution)は未**（今はピッチ/輪郭のみ崩す）。
