/* v8 marks.js — 印の規則（唯一の定義）。v7から: DEFAULT_OPTS を イ+B に変更
 * （差し戻し5の暫定統一。案イ=v6で見せた印を消さない側・案B=裁定前に新類型の黄を
 * 増やさない側）。音数系のドット化は既定経路に通さない（改訂: 批判4）＝
 * onsuDot() は枚2「印の決まり」の案くらべ区画専用。
 *
...
(function (g) {
  ...
  var DEFAULT_OPTS = { flatPair: "イ", yellowUse: "B" };
  ...
  /**
   * 音数ドット案（案くらべ区画専用・既定経路には通さない）。
   * 「音数の食い違い（あとN音・字余りN）を行末ドットにも出すか」は枚2「印の決まり」の
   * 裁定対象（計画§1-4・§5裁定待ち2）。既定の行末ドット（computeMarks 由来）には混ぜない。
   * @param moraCount 読みのモーラ数
   * @param noteCount 音符数
   * @returns null | {color:"y", why}
   */
  function onsuDot(moraCount, noteCount) {
    if (moraCount === noteCount) return null;
    if (moraCount > noteCount) return { color: "y", why: "字余り " + (moraCount - noteCount) };
    return { color: "y", why: "あと" + (noteCount - moraCount) + "音" };
  }
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = g.V8MARKS;
