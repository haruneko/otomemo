...
- **現状の実プロセスは2つ**：`cm-api`(:8787 単一オリジン＋reap/scheduler＝生成/継続調査/MIDI取込を消化) / `cm-search`(:8788 意味検索)。
  ⚠️ `deploy/systemd/` にはまだ `cm-worker.service`（旧ジョブワーカー・2026-07-05撤去済）と `cm-music-mcp`(:8790・S2廃止)への
  参照が残る＝**死にユニット**。install.sh も cm-worker を入れようとする（存在しない実行体）→ **剪定が必要**（backlog「systemd 死にユニット剪定」）。
- **自動再起動**(`Restart=on-failure`)・**日次バックアップ**(`cm-backup.timer`→`scripts/backup.sh`)。
