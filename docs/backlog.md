...
- **小節単位の別案（io-map 裁定#7・繰り越し・2026-09-10 受け入れ監査 中⑤で「backlog に無い」と指摘され起票）**：「気に入らない小節だけ再生成」＝**別案の粒度**（seed の当て方）であって per-note 編集レーンではない＝シーケンサー化に当たらない（正典＝`docs/drafts/2026-09-09-phrasemaker-port-master-plan.md` §5-1 の4行目・design.md:507 の隣）。**M2 で足すはずが未消化**のまま phrase_maker 取り込みアークへ繰り越されている。着手時 Task 化。
- ✅**真因調査済み・裁定済み（2026-09-15）＝research `2026-09-15-anchor-rigidity-guitar-motion-rootcause.md`**：主因は錨でなく体（固定表・変奏なし）。裁定＝錨は変わり目だけ必須／刻みの音価はつまみ／変奏は候補を段で。**案4（体を規則生成へ作り替え＝ウォーキング方式）は次の白紙設計の候補**。以下2項は経緯として残す。
- **［耳判定 2026-09-13］ベースの錨は「やりすぎ・固い」＝要らない**（原文「必ずベースルートになるのは保守的と言うか固くない？」）。錨（anchorLock）は既定 OFF のまま。**真因を見てから扱いを決める**＝「全キックに必ずルート」という構造的な契約が固さの源か／源流でも同じ固さか（phrase_maker の耳GO「ロックとても良い」は ensemble 全体での評価＝ベース単体の評価ではない可能性）／傾きとして弱める形が要るか。**M5 ギターのキックロックも同じ発想**なので、ギターの「動きがない」と併せて見る。
- **［耳判定 2026-09-13］ギターのリフは「パームミュートみたいで動きがない。繰り返しを使ってもっと動かしたい」＝微妙**。候補の仮説（未検証）＝①発散のつまみ `diverge`（phrase_maker `bass_rock_riff/diverge.py` 系・M5 で未移植）②キックロックが刻みを固定している③文法3型の語彙が薄い。**反復と変化（リフの変奏）**は research `2026-08-02-riff-structure-and-variation.md` に既存の調査あり。真因調査を先に。
- **［オーナーの好み 2026-09-13］6拍子のドラム（自作・オーナー提示）**：1文字＝8分・`.`＝休符・`x`＝打つ・2小節（6/8×2）。
  ```
  sd: .x..x. ..x...
  bd: x..x.. x...x.
  ```
  スネアとバスドラ。ヘミオラ型への感想（「すごくよくはないけど、まあまあかも」）に添えて出てきた。**自作ネタなのでリテラル保存は可**（他者コーパスではない）。drumLibrary の world68／6/8 系へ型として足すかは次に触る時に確認。
- **低域窓 `[BASS_LO,BASS_HI]=[33,48]` は 16 半音しかなく、錨（anchorLock）の「最寄りのレジスタ」が 12 ルート中 8（pc=1..8）で候補1個へ縮退する**（2026-09-10 監査 中③・実測）。規則の実装漏れではなく窓の幅の帰結＝**doc は実態に合わせて直した**（design 追補 (k)）。窓を広げる／錨だけ別窓にするのは**出音が変わる＝耳の裁定が要る**ので保留。
