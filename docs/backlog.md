...
- **アプリ内クレジット画面**：`NOTICE.md` の内容（GMD の CC BY 4.0 表示）を実機の画面からも辿れるようにする。
  ...
  配布物の画面から見えるほうが素直。
- **身体シミュレータの三連スロット**：`bodyFill.ts` のスロットは 0.25qb 固定＝三連格子（shuffle 型）には
  フィルを付けない。実フィルの約35%が三連という調査（phrase_maker 計画 §既知の未達）があるので、
  三連スロットを持つ増分が要る。いまは「黙って歪めず、付けない」で退避している。
