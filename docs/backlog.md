...
- **[M] モバイルのワークスペース整備**（design「狭い＝mainpane全画面＋レールはドロワー/タブ」未実装の顕在化）：①🏠後もレールが上に積まれる（暫定=🏠で自動畳み済）②💬バブルが最下段カードに被る（page scroll構造由来でproject-screen単体では解けず）③エディタ全画面の戻るボタンが☰を覆う。まとめて構造から直す＝**アプリ全体のモバイルレイアウト刷新**（範囲広・別腰）。
