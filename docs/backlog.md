...
旧: 2026-08-02c（**作詞補助アークのスライス6/7が入った**＝複製が出自を残す（`copy_of`＝新→元・vary の variant_of とは別種）／詞先メロが**句を持って生まれる**。**詞先の音符数が変わる新挙動（読みを取る・「っ」「ー」に音符を立てる）は既定OFFの opt-in**＝下に新設「作詞補助アークの耳確認リスト」＝耳で確かめてから既定を反転する。正準＝design §31-10 スライス6/7）
...
正準＝design §31-10 スライス7／§31-8。**詞先メロ（`gen_melody` に `lyrics` を渡す経路）の「音数の決まり方」を2つ切り替えられるようにしたが、どちらも既定OFF**＝今の音は従来のまま。耳で確かめて既定を反転する（反転＝コード1行＋テストの期待値、戻すのも同じ）。
- **①読みを取る（`readLyrics:true`）**：表記（漢字仮名交じり）から pyopenjtalk で読みを取ってモーラを数える。既定OFF＝表記の字をそのまま数える＝**漢字は1字1音に化けて音数がずれる**（例「君の名前を」＝5音／読みなら7音）。聴くところ＝**歌詞のとおりの音数で歌えているか**。既定にしてよいか（＝表記のまま渡せる道具にするか）。
- **②「っ」「ー」に音符を立てる（`standSpecialMoras:true`）**：§31-8 のオーナー裁定どおり全モーラが音符を持つ（「がっこうへ」4音→5音・「そーらへ」3音→4音）。聴くところ＝**促音・長音に音符が立ったメロが歌として自然か**＋**促音「っ」を VOICEVOX にどの lyric で送るか**（`sing.ts` は syllable をそのまま送る＝「っ」が歌えない場合の音の扱いはここで決める＝§31-8 の実装時の確認1点）。
- 見る場所＝返りの `lyricPlan.readingSource`（none/pyopenjtalk/failed）と `lyricPlan.standSpecialMoras`＝**どの数え方で音数が決まったか**が候補と一緒に出る。
