...
最終更新: 2026-08-02e（**句が意味検索の索引に入った**＝A5小工事完了＝`_content_text` が kind を問わず `content.lyric.phrases[].text` を拾う＝詞先メロ（title も text も無い）を書いた言葉で探せる。ダミーで実測1位。⚠ 副産物の発見＝`neta_embedding` は `neta.updated` キーの派生キャッシュ＝**`restart.sh --search` は既存ネタを再索引しない**（既存の句持ち10件は未反映・下の該当節））
