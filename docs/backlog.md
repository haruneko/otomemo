...
- **P2（残・効果中/前向き防止）**：今後の study/audio_analyze も**音源を asset に保存＋処理後に params の audio_b64 を落とす**（再蓄積の恒久防止）。触る＝`study-runner.ts`・`audio-analyze.ts`（asset化＋strip）・reaper（結果ネタへ source リンク）。design #16 のジョブ契約を先に改訂→TDD。※P1で一覧払い出しは既に軽い＝緊急ではない（再蓄積してもDB肥大のみ）。
