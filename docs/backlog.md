...
最終更新: 2026-08-02c（**作詞補助アークのスライス6/7が入った**＝複製が出自を残す（`copy_of`＝新→元・vary の variant_of とは別種）／詞先メロが**句を持って生まれる**。**詞先の音符数が変わる新挙動（読みを取る・「っ」「ー」に音符を立てる）は既定OFFの opt-in**＝下に新設「作詞補助アークの耳確認リスト」＝耳で確かめてから既定を反転する。正準＝design §31-10 スライス6/7）
...
## 楽器アレンジの打ち込み助け（2026-08-02・スコープ確定／実装は「頭を空にしてから」で保留）
...
- ~~**実装計画は保留**（オーナー「一度頭を空にしてからやりたい」＝白紙で計画したい）。着手時は計画doc（`docs/drafts/`）→Task化。~~ → ✅**白紙計画 済（2026-08-02b）**＝`docs/drafts/2026-08-02-arrange-midi-help-cleanroom-plan.md`（コード非開示Fable3案→検分→統合・推し＝「語彙帳＋配札」・未決7点）＝**オーナー裁定待ち**。オーナーの生の課題（2026-08-02）＝手癖で似たニュアンスの別パターンが出せない・楽器固有リフ・オルガン/ピアノの奏法バッキング。下の暫定スライス分割は白紙計画で上書きされうる＝裁定後に整理。
- **スライス分割（暫定・着手時Task化）**：
  - **(近) 背景パートの候補を厚くする**＝既存verb（`gen_chord_pattern`/`gen_drums`/`gen_bass`）に「候補を一段厚く（質を上げる）」を足す。**まず打ち込みで一番だるい1種**（ドラム／ギターのコンピング等）を選び縦スライス。既存の質ノブ（feel/humanize/voicing/型辞書）が足場＝拡張の作法は確立済（下の「伴奏・演奏アーク」参照）。
  - **(後) ソロ/リードの専用verb**（`gen_solo`/`lead` 等・**未実装**）＝新規・別スライス。型辞書へ押し込まず「進行に解決する線の候補」を出す＝`gen_section_inst` descの「旋律的セクションライン＝スコープ外(後続)」の後継。**per-note編集レーンは付けない**（詰めはDAW）。
