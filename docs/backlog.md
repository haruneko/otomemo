...
- **P2（残・効果特大/コスト中）**：オーディオを `job.params` に永続保存しない。既存 `asset`（ファイル実体保存）へ寄せ params には参照(asset_id)だけ／少なくとも **done 時に params の audio_b64 を NULL 化**。触る＝`study-runner.ts`・`audio-analyze.ts`・投入経路（MCP `start_study`/`analyze_audio`）。design #16 のジョブ契約を先に改訂→TDD。
- **P3（残・効果大/書込リスク）**：既存肥大の回収＝done/failed の study・audio_analyze の `params` を NULL 更新→`VACUUM`（freelist 22.6MB も回収）。**必ずバックアップ後・api停止中に**。一度きりの保守スクリプト（`scripts/`）。DB 127MB→30MB未満の見込み。
