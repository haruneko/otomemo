...
- **systemd 自動起動**：母艦再起動でスタックが落ちる（手起動が要る）。`deploy/systemd/install.sh` で
  cm-api/search を --user systemd に入れて enable。グローバル汚染を避けたいので最後でよい（ユーザー方針）。未インストール。
- **systemd 死にユニット剪定**（2026-07-07 摘出）：`deploy/systemd/` に `cm-worker.service`（旧ジョブワーカー・2026-07-05撤去済）と
  `cm-music-mcp`(:8790・S2廃止) 参照が残り、`install.sh` も cm-worker を入れようとする（実行体不在）。生存は cm-api＋cm-search の2つ＝
  install.sh/ユニットから cm-worker/cm-music-mcp を除去。deploy.md は是正済。
