...
- **バッチジョブの true cancel（#100④-S6 の続き）**：`DELETE /job/:id` は行を消すが、**実行中の
  research/audio_analyze 消費者プロセスは走り切る**（結果は捨てられるだけ・実害小）。チャットのターンは
  `turn/stop` で実際に殺せる。バッチも中断したいなら api の consumer 側に abort シグナル（running 中に
  削除されたら処理を打ち切る）を配線する。頻度が低ければ後回しでよい。
