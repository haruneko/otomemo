...
最終更新: 2026-08-02（**楽器アレンジの打ち込み助けをスコープ確定**＝Otomemo内・別プロジェクト/別リポにしない。要件「達成度の線引き」＋design「前景/背景の判別軸」に⚠オーナー未レビューで記載（ソロも背景＝per-note編集なし・詰めはDAW／前景=主旋律だけ）。**実装計画はオーナー「一度頭を空にしてからやりたい」で保留**＝下に新設セクション「楽器アレンジの打ち込み助け」）
...
- **実装計画は保留**（オーナー「一度頭を空にしてからやりたい」＝白紙で計画したい）。着手時は計画doc（`docs/drafts/`）→Task化。
- **スライス分割（暫定・着手時Task化）**：
