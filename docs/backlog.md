...
- **BUG#2【中・要design判断】曲名アナリーゼの期待ズレ**：ユーザーは「この曲アナリーゼして」で本物MIR
  （取込パネル🎵の audio_analyze＝Demucs+librosa+BTC）を期待するが、常駐チャットに音声解析ツールは無い。
  Web/記憶で key・コード表を作り「分析結果が揃いました」と提示＝**実測でない二次情報を"解析済"に見せる**。
  対策案(a) チャットが「音声解析は取込🎵から。ここは一般情報のみ」と明示／(b) URL を渡されたら audio_analyze
  ジョブへ橋渡し(create_job)。COMPOSE_PLAYBOOK か chat面ツールの調整。
