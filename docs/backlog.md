...
- **CoWガードの残経路**：共有 section 内の compose 辺操作（ブロック削除・候補/ピッカー配置＝落ちサビの「辺を外す」を素手でやる経路）は useCowGuard を通らない（S2は updateNeta 直接経路までをスコープ化）。分家導線（FormStrip「分家にする」）があるので実害は限定的だが、安全弁の完全化は次スライスで。
...
- **allin1 追い焚きジョブ**（機能ラベル付き構成・条件付きGO）：10-14分/曲＋局所パッチ3点＝既定パイプラインに入れず**任意の非同期エンリッチ**（解析済みネタに後から構成ラベルを足す）。venv-f1(1.4GB・パッチ済)が検収用に残置＝実装の種。
