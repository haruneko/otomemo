...
- **worker の claude_prompt 完全撤去（#100 の最終形）**：⑤で consult は撤去済。① NetaList の「AI生成」ボタン
  も ✅ 決定的 `/gen/section` へ移設済（2026-07-05・GN-08修正）。残＝② NetaDialog の scheduled research を
  常駐 claude/Claude コマンドへ移して claude_prompt を消す。移したら brainstorm/suggest/gen_lyric/fetch 等の
  旧 LLM ハンドラも一掃。
