...
- **worker の claude_prompt 完全撤去（#100 の最終形）**：⑤で consult 撤去済。① NetaList「AI生成」→ ✅ 決定的
  `/gen/section` へ移設（2026-07-05・GN-08）。② scheduled research → ✅ **api 内 claude 実行器**へ移設
  （2026-07-05・`research-runner.ts`＝schedule→job→**api consumer(claude -p)**→reaper→referenceネタ→トレイ）。
  残＝worker の research/collect/brainstorm/suggest/gen_lyric/fetch 等 旧 LLM ハンドラの**撤去**（api が research を
  引き取り worker は import_midi 等のみ＝ほぼ空。worker プロセス自体の要否も再判断）。
