...
- **エディタの Undo/Redo**（現状 capture取消=削除のみ／ロール等はundo無し）：**全エディタ共通で `content` スナップショット履歴を NetaDialog 層に1つ持つ**と melody/chord/bass/rhythm/section 全部に一発で効く（各 onChange 前の content を push・深さ上限・redoスタック・コマンドパターン不要＝簡易）。**スマホUX＝画面の ↩︎/↪︎ ボタンが本命**（親指届く下部/ヘッダ）。ジェスチャ(2本指/シェイク)はロール横スクロール・タップ配置と衝突＆非発見的で避ける。1操作=1ステップ・空時disable・「取り消しました」トースト。※backlog「ネタの版管理(chat書込のサーバ側undo)」とは別レイヤ(ローカル編集履歴)・両立。
- **ノート編集の拡充**（今はクリック追加/削除のみ＝`PianoRoll` addAt/removeNote）：**ドラッグ移動・範囲選択・コピペ・小節複製**。スマホは長押し→選択モード等。
