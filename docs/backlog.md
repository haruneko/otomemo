...
- **Chat のターン永続化＋ストリーム復帰（2026-07-05 保留・オーナー「直すのはやっとこう」）**：チャットを閉じても
  サーバ側の claude ターンは裏で続く（＝良い・機材の capture 等はDBに残る）が、**再オープンで途中ストリームが復帰
  しない／最後のテキスト返信がクライアント側保存のため履歴に残らない**ことがある。→ ①ターンの結果を **サーバ側で
  chat_message に永続化**（/turn 完走時に addChatMessage）②再オープン時に **進行中バナー＋完了後リロード**。
  実害は限定的（書込ネタは残る）だが長文相談で締めの返信が消えるのは惜しい。
