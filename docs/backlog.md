...
- **［黙って落ちる・2026-09-16 発見］型 ID を間違えると通知なしで既定の型に差し替わる**：`/music/gen_chord_pattern` は未知の `pattern` で既定へ、`/music/gen_bass`・`/music/gen_drums` は未知の `style` で既定へ、`meta.warnings` を出さずに落ちる（耳の基準線の試聴音声づくりで判明・その時は出力を1本ずつ照合して混入なし）。画面やチャットから指定を誤ると気づけない＝「通知が嘘をつく／黙る」の形。直し方＝未知 ID は `meta.warnings` で落ち先を告げる（既存の作法）。 ✅ 2026-09-17 修正（849939f）＝「〈部位〉の型『…』が見つからないので、型を使わず従来どおり生成しました」を HTTP/MCP/`/gen/section`/web に。
