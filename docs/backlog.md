...
- **メロ生成エンジンの大物負債（骨格層の動機そのもの・S1-S3安定後に着手）** → **~~#10（死にプロト＋Note型）~~✅完了(2026-07-11)・#11（経路撤去＋ノブ再編・J1〜J4に再分割）**：genMelody(generate.ts)に4経路がearly-return並存（補完/V2/motifModel/旧経路）・opts=40個がMCP/HTTP/web UIに伝播→**層別（骨格生成/表面化/装飾）ノブ再編＋旧経路撤去**。倚音/掛留は実質三重実装（applyExpression／V2 melodyCells／motifModel）＝V2を正に一元化。
  ...
  - **残＝#16 ③motifModel(genMotifMelody)撤去＋appoggiatura ノブ削除**（テスト移植を伴う＝melody-cells.test.ts の genMotifMelody describe :115-／generate.ts ③分岐）。本番は全経路useV2:true＝③は既にテスト専用（不到達）。
