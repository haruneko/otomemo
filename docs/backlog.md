...
- **R0§6の遷移統計テーブル化**：~~（phrase_pattern / note_transition / chord_transition）辞書→遷移統計の器が未着手~~ → **(B) note_transition＝骨格n-gram＋M9変換文法は ✅WP-0(#21・cf06399)で器実装済**（`corpus_note_transition`/`corpus_skeleton_prior`/`corpus_motif_transform`＋読み出し純関数 `corpusStats.ts`・生成側結線は WP-M1/M2）。**残＝(A) phrase_pattern の literal 句・(C/D) chord_transition**。
  - **→ 2026-07-21 設計着地（正典＝design.md「コーパス遷移統計テーブル 第2弾」#21拡張）**：素材ブロックを切り分けた＝**(D) 進行遷移と (A) pop 句は在DBだけで再構築できる（生ソース不要）**。在DB `neta`(chord_progression, library) 210件が正規化済（度数×正準quality×count）＝遷移集計に足る。「U-FRET生データ消失」が塞ぐのは①新規曲追加②生バーライン厳密復元であって在DB集計ではない。**設計＝新表 `corpus_chord_transition`(count重みn-gram・追加のみ)＋在DB集計スクリプト＋読み出し純関数＋`next_chord`/`genChords` へ count 重み結線（既定OFFで seed bit 一致）**。**着手可＝WP化して走らせる**（M2 を待たず最短で無ランク next_chord を是正）。
