...
最終更新: 2026-08-02h（**「っ」「ー」に音符を立てる旧裁定をオーナーが撤回＝立てない（従来）で確定**。原文「**C1 促音伸ばし音はいらない、打ち込むときに工夫するので。**」＝長音ーは直前へ延長・促音っは詰め＝機械は音符を増やさない。同日入れた opt-in `standSpecialMoras`（`planLyricMelody` の旗・MCP `gen_melody` のパラメータ・返りの値・関連テスト）は**使う者ゼロの死にノブなので撤去**＝従来経路一本。促音を VOICEVOX にどの文字で送るかの確認事項も消滅。requirements/design（§31-8・§31-5・§31-10 スライス7・#13d 表）を現況主義で書き換え。**作詞補助の耳確認は①読みを取る（`readLyrics`）を既定にするかだけが残**。正準＝design §31-8）
旧: 2026-08-02g（**B1/B2/B3 をオーナー実機スクショで追認＝作詞補助アークの画面の未決がゼロ**。原文「B1-3 は良い→良いです」。B1＝通しで読む面の姿＋(い-c) 遅延生成の粒度（design §31-11 の17 を閉じ）／B2＝歌詞パネルの (b)(c)(d)（同 16 を (a)(b)(c)(d) 全部で閉じ＝**「オーナー不在中に推しで進めた＝未レビュー・1件ずつ戻せる」の札を撤去**）／B3＝SplitRoll の色＝**黄で確定・青系は却下**。**いずれも有力仮説で先行実装した形がそのまま正＝作り直しゼロ**。正準＝design §31-5／§31-9／§31-10 スライス5／§31-11 の15・16・17。**残る作詞補助の宿題は耳/手だけ**＝下の「作詞補助アーク（2026-08-02）の耳確認リスト」）
旧: 2026-08-02f（**パターンライブラリ L4トラックA が実って締め＝残は耳のみ**＝新21型＋co-tag17件＋drum roles の辞書化（51a5ac2・独立監査で受け入れ＝OK21/落とす0）→実DB再seed 118件（冪等）→**18セル実測＝ballad/vocarock/edm×verse/chorus×3パート全て4件以上を達成**。GENRE_TABLE据置＝生成器出音bit不変・api 1522緑・tsc clean。正準＝design「Task2/L4 トラックA」実施決定ブロック。**残＝下に新設「パターンライブラリ L4 の耳確認リスト」＝ポップス/ボカロック優先（オーナー裁定）**）
旧: 2026-08-02e（**句が意味検索の索引に入った**＝A5小工事完了＝`_content_text` が kind を問わず `content.lyric.phrases[].text` を拾う＝詞先メロ（title も text も無い）を書いた言葉で探せる。ダミーで実測1位。⚠ 副産物の発見＝`neta_embedding` は `neta.updated` キーの派生キャッシュ＝**`restart.sh --search` は既存ネタを再索引しない**（既存の句持ち10件は未反映・下の該当節））
旧: 2026-08-02d（**アレンジ打ち込み助けの計画が確定**＝オーナー裁定で統合案「語彙帳＋配札」採用・最初のスライス＝写像規則。枝7点も裁定（ソロは初期スコープ外／軸指定で再配札／試聴音源は今のまま／**自作採取は不採用・他者実データの統計で育てる＝⚠統計のみ**／進行変更は自動で当て直し／**手癖の鏡は作らない**／一括下敷きは後段）。正準＝design「楽器アレンジの打ち込み助け＝計画確定」節・白紙計画は `docs/archive/` へ移送＝**次は Task 化して S1 着手**）
旧: 2026-08-02c（**作詞補助アークのスライス6/7が入った**＝複製が出自を残す（`copy_of`＝新→元・vary の variant_of とは別種）／詞先メロが**句を持って生まれる**。**詞先の音符数が変わる新挙動（読みを取る・「っ」「ー」に音符を立てる）は既定OFFの opt-in**＝下に新設「作詞補助アークの耳確認リスト」＝耳で確かめてから既定を反転する（→ **後者は 2026-08-02h の裁定で撤去**）。正準＝design §31-10 スライス6/7）
旧: 2026-08-02b（**アレンジ白紙計画＋件1代替案モックを裁定材料化**＝①白紙計画＝コード非開示Fable3案→検分→統合→`docs/drafts/2026-08-02-arrange-midi-help-cleanroom-plan.md`（推し=統合案「語彙帳＋配札」・最初のスライス=写像規則・未決7点）＋研究2本を research/ へ（オルガン/ピアノ奏法語彙・リフ＋変奏手法）②件1（詞モード）＝代替案4候補＋360pxモック＝drafts の lyric-editor-screen-decisions 末尾増補（**事実訂正＝現ラベルは「歌詞」**・3cd5bf3で変更済み＝design記述ドリフト）③スライス6/7実装は別エージェント走行中。**どちらも裁定待ち＝次のオーナー実機/裁定回で**）
旧: 2026-08-02（**楽器アレンジの打ち込み助けをスコープ確定**＝Otomemo内・別プロジェクト/別リポにしない。要件「達成度の線引き」＋design「前景/背景の判別軸」に⚠オーナー未レビューで記載（ソロも背景＝per-note編集なし・詰めはDAW／前景=主旋律だけ）。**実装計画はオーナー「一度頭を空にしてからやりたい」で保留**＝下に新設セクション「楽器アレンジの打ち込み助け」）
旧: 2026-07-30（**作詞補助アークの画面まわりが前進**＝空メロの見た目 (い-c) 遅延生成の裁定＋歌詞パネル（常時1行＋展開）を実装。歌詞まわりの常設 約220px→52px・ロールが折り返し内。件2/3/4 は**オーナー不在中に推しで進めた＝未レビュー**、件1（詞モードの見せ方）だけ裁定待ち＝実機3分が前提。**新しい積み＝下の「詞先で生まれるメロは歌詞で検索できない」**（索引に句が入っていない・直しは数行だが意味索引の再構築が要る＝スライス5と同じ回に））
旧: 2026-07-22c（**次アーク＝パターンライブラリ化を計画化**＝オーナー方針「パターンはネタ帳にライブラリ扱いで登録・タグでフォルダ・たくさん準備・Section選択画面と共用」。正典＝`docs/research/2026-07-22-pattern-library-arc-plan.md`（スライスL0-L5・未決5点・引き継ぎ手順）。**修理#3大手術はS1-S9全完了**（〜18351d0・T1/T2/M1-M4✅）だが**M5 UI一貫性監査と耳確認は本アークと合流するため保留中**＝完了宣言はまだしない。品質真因調査（Fable・pattern-quality-root-cause）は走行中＝doc有無を次セッションで確認）
旧: 2026-07-22b（**伴奏・演奏アーク完走**＝研究3本(ピアノ/ギター奏法語彙・スタイルエンジン)→ベースslashBass→S1 voiceGuitar+じゃら〜ん→S2型辞書26型→UI三層A/B/D+モック→S3左手内蔵+ギターD/U→C聴いて選ぶトレイ→S4 feel添付。a82d028〜c52ca10・全既定OFF=bit一致・api1396/web989緑・dist焼き+restart済。**耳確認リストを下に新設**。ストラム時間展開(#29残)を✅化）
...
## 楽器アレンジの打ち込み助け（2026-08-02・スコープ確定＋**計画確定**＝オーナー裁定済み／着手は Task 化から）
正準＝requirements「達成度の線引き」の楽器アレンジ項＋design「前景/背景の判別軸」（どちらも⚠オーナー未レビュー）＋memory `project-arrange-layer-midi-help-scope`。**確定した枠**：MIDI打ち込み時の手伝い（背景パート＝コンピング/ドラム/ベース＋器楽ソロ/副次リード）は **Otomemo内**（別プロジェクト/別リポにしない＝生成・和声基盤 `apps/api/src/music/`＋`apps/web/src/music.ts` を共有）。候補の質は上げてよい（#149「自作と遜色なく差し替えできるレベル」が許容）が、線は"質の天井"でなく**適用主導権＋編集単位**＝提案のみ・自動適用しない／音符を1個ずつ直すレーンを足さない（＝シーケンサー化しない）／ニュアンス・音符の詰め・完パケはDAW(ABILITY)。**ソロも背景扱い**＝前景（音符が編集対象）は"その曲の主旋律"だけ（判定は"主旋律か"・歌詞の有無ではない＝インスト主旋律・詞先の空メロも前景）。
- ~~**実装計画は保留**（オーナー「一度頭を空にしてからやりたい」）／白紙計画は裁定待ち~~ → ✅**計画確定（2026-08-02 オーナー裁定）＝正準は design「楽器アレンジの打ち込み助け＝計画確定」節**（「前景/背景の判別軸」の直後）。材料の白紙計画は決着ヘッダを付けて `docs/archive/2026-08-02-arrange-midi-help-cleanroom-plan.md` へ移送済み。**着手＝計画doc不要でそのまま Task 化**。
- **採用＝統合案「語彙帳＋配札」**＝①語彙帳（コード非依存の奏法テンプレ棚）②写像試聴（曲のコード/キー/テンポへ当てて主旋律と一緒に鳴らす）③配札・ピン（ランキングしない）④軸指定の再配札（薄く/低く等の注文チップ）⑤リフ雛形（和声4型×楽器イディオム）。オーナーの生の課題（2026-08-02）＝手癖で似たニュアンスの別パターンが出せない・楽器固有リフ・オルガン/ピアノの奏法バッキング。
- **スライス分割（確定・着手時Task化）**：
  - **S1 写像＋試聴＋採用（ピアノ/オルガンから）**＝テンプレ→曲のコードへ写像→主旋律と一緒にループ試聴→パターン単位採用→MIDI書き出し。**分数コード・テンション・コード2拍替わり・テンプレ長とセクション長の不一致**をここで潰す＝**全案共通の要石**（`resolveChordPattern` の延長線が足場／オルガンの CC11/Leslie は content スキーマ拡張の要設計＝修理#3送り分「CC11可否要調査」と合流）。
  - **S2 配札＋軸指定の再配札**＝候補を数枚配る→ピン/捨て→ピンを種に再配札＋注文チップ（`vary`/`reshape`・候補トレイ `useMelodyGen` が足場）。
  - **S3 リフ雛形**＝和声4型（移調/固定/ペダル/微調整）×楽器イディオム（`gen_riff` verb が既存＝距離は着手時に実コードで精査）。
  - **横断＝統計較正**＝他者コーパス（POP909等）の統計を写像規則・候補分布へ効かせる。**⚠統計のみ＝リテラルなパターン/旋律は保存しない**（リズム分布/ボイシング傾向/遷移統計まで）。
  - **後段**＝セクション一括下敷き（全パート整合の一括提案）／**ソロ・副次リードの専用verb**（`gen_solo`/`lead`・未実装＝初期スコープ外の裁定。型辞書へ押し込まず「進行に解決する線の候補」＝`gen_section_inst` desc の後継・**per-note編集レーンは付けない**）。
- **落とした要素（裁定）**：**自作MIDIからの採取／採用品の棚還元＝不採用**（原文「手癖を超えたいのに私のライブラリを使うのは変。900POPSとかの実データ使いたい」）。**手癖の鏡（プロファイル/使用履歴メーター/遠い順ソート）＝作らない**（原文「そも弾いたり打ち込んだりの口ないから手癖に縛られる意味がわからない」）。**試聴音源への投資はしない**＝SpessaSynth/SF3 の検討は保留のまま（下記の該当節も据置）。**進行変更時は自動で当て直し**。
- **既に"仕上げ寄り"な足場**（＝ゼロからではない）：グルーヴ/ストラム時差/ゴーストノート/humanize/guitarボイシングは実装済。rootless/drop2の高度ボイシングはdesign「後段＝人の仕上げ領域」の先例あり＝踏み込むなら個別裁定。
- **［2026-09-16］リフ撤去の後始末2件（③決着・④一覧化中）**（`docs/archive/2026-09-16-riff-arc-revert-review.md` §7 ③④・撤去そのものは裁定済み・完了）：
  ③ ~~phrase_maker 取り込みアークを完走扱いにし残りを打ち切るか~~ → ✅**決着（2026-09-16 オーナー裁定＝考え方だけ残して閉じる）**。コードの取り込みは終了（残ったのはドラムのフィル・JZ-WALK）。M6b〜M7 と旧 §8 の裁定4件は打ち切り。ケルトは移植でなく新機能として白紙から設計する対象。考え方の文書＝`docs/research/2026-09-16-phrasemaker-ideas-to-keep.md`／
  ④ 次は上の **S2 配札・S3 リフ雛形へ戻る**か（入口＝既存 `gen_riff`＋和声4型＋配札）。戻る起点＝`docs/drafts/2026-08-02-arrange-arc-handoff.md`。　**→ 次の入口は一覧化中（別担当）**
  外した理由（負の知識5項）＝design 追補 (k) の撤去記録＝**キックに揃える規則・固定2小節の表を貼る方式はリフではない**／次にリフを作るなら表を敷かず規則で選ぶ（JZ-WALK の向き）。
- **小節単位の別案（io-map 裁定#7・繰り越し・2026-09-10 受け入れ監査 中⑤で「backlog に無い」と指摘され起票）**：「気に入らない小節だけ再生成」＝**別案の粒度**（seed の当て方）であって per-note 編集レーンではない＝シーケンサー化に当たらない（正典＝`docs/archive/2026-09-09-phrasemaker-port-master-plan.md` §5-1 の4行目・design.md:507 の隣）。**M2 で足すはずが未消化**のまま phrase_maker 取り込みアークへ繰り越されている。着手時 Task 化。
- **［オーナーの好み 2026-09-13］6拍子のドラム（自作・オーナー提示）**　**✅ 2026-09-16 型として追加（`owner.six8`・表示名「6拍子（オーナー作）」・2小節の型／sd・bd の2レーンのみ）**：1文字＝8分・`.`＝休符・`x`＝打つ・2小節（6/8×2）。
  ```
  sd: .x..x. ..x...
  bd: x..x.. x...x.
  ```
  スネアとバスドラ。ヘミオラ型への感想（「すごくよくはないけど、まあまあかも」）に添えて出てきた。**自作ネタなのでリテラル保存は可**（他者コーパスではない）。drumLibrary の world68／6/8 系へ型として足すかは次に触る時に確認。
- **S1精査で発見した隣接バグ（2026-08-02・S1に混ぜない別修理）**：(i) `voiceGuitar`（`apps/web/src/music.ts:736-754`）の6声間引きが「12+fifth」「12」を各1個しか除去せず、テンション過多（13th等＝色音3個以上）で7音に破れる (ii) LH 度数トークン（`degreeInterval`・`DEGREE_CHORD_INDEX`＝3/5/7限定）が未知度数をサイレントに根音(0)へフォールバック＝データ化ミスが無音で通る。どちらも guitar/LH 径路＝S1（keyboard オルガン中心）の外。

- **［耳判定 2026-09-16・品質の苦情＝真因調査が先］オルガンの型（OG-*）は「あとにごりがひどい。テンションコードの 7th の配置がゴミってる気がする」**：棚の試聴で5型すべて。7th を含むコードのボイシング（配置・音域・重なり）に原因がありそう（オーナーの見立て・未検証）。撤去や縮退をアドホックに提案せず、まず実音化の経路で 7th がどこに置かれているかを実測する。
- **［将来の課題・2026-09-17］ジグの生成の質をもう一段上げる**：関数（`jigMelody.ts`）は耳で「ジグらしい」だが「全般的にクオリティはもう少し欲しい」「本物よりは下手」。設計＝`docs/drafts/2026-09-16-celtic-jig-reel-design.md`。本物との差を詰める方向は未調査。
- ✅**［2026-09-17 耳で合格「悪くないので進もう」］終わり方を3種から種ごとに選ぶ形で決着**（`3084b7e`〜`45ef8c7`・試聴 https://claude.ai/artifact/2KZFXHMKiwqs9kcKd5kmtP ）。残＝前半の回り方は種を変えてもほぼ同じ（未対応）。以下は経緯。
  - **［耳判定 2026-09-16］タム回しは「最後のスネア四連打はいいけどバリエーションの一つにしてほしい、なんかこれに固執するの辞めてほしい」**（試聴帳 https://claude.ai/artifact/9ThjP6mTNNLrejZ38MbL99 ・seed 違い3本とも同じ終わり方）。真因調査のうえ、四連打を変化の1つに下げる。
- **［2026-09-18 裁定済み・未実装］生成したピアノ伴奏の編集画面＝画面 A**（正準＝design「和音パターンの明示の音」節の「生成したピアノ伴奏の扱い」・見本 https://claude.ai/artifact/G9PMngjGMFvsGUHXU5raGY ）：明示の音を持つ和音パターンを開いたら升の編集を閉じ、実際に鳴る高さのピアノロールで閲覧＋別案（種違い・8分裏の単音・打鍵の揺れ）＋MIDI 書き出し。「ふつうの和音パターンに戻す」口は需要が出てから。
- **［2026-09-23 耳待ち］ピアノ伴奏の両手の音域**：`register:"piano"`（左手 C3〜B3・右手その上）を足した。耳で合格したら既定を切り替える（聴き比べ https://claude.ai/artifact/4Mb1MsUYXpq8YyCadpH1Go ・経過＝`docs/drafts/2026-09-16-handframe-evolution-design.md` 末尾）。
- **［黙って落ちる・2026-09-16 発見］型 ID を間違えると通知なしで既定の型に差し替わる**：`/music/gen_chord_pattern` は未知の `pattern` で既定へ、`/music/gen_bass`・`/music/gen_drums` は未知の `style` で既定へ、`meta.warnings` を出さずに落ちる（耳の基準線の試聴音声づくりで判明・その時は出力を1本ずつ照合して混入なし）。画面やチャットから指定を誤ると気づけない＝「通知が嘘をつく／黙る」の形。直し方＝未知 ID は `meta.warnings` で落ち先を告げる（既存の作法）。 ✅ 2026-09-17 修正（849939f）＝「〈部位〉の型『…』が見つからないので、型を使わず従来どおり生成しました」を HTTP/MCP/`/gen/section`/web に。 fill の未知の型ID（ベース・ドラム）も ✅ 2026-09-17 修正（6a22249）。
...
- ~~**［UX・要判断］左手行の埋もれ**：E2E所見＝左手プリセットは響きゾーン5行目＝CP編集で要スクロール＋チャットFABが被り気味。対処案＝(a)このまま (b)2見出し分割 (c)FAB退避。オーナー裁定待ち。~~ → **✅解消済みで閉じ（2026-08-02 実測・裁定不要）**：360×800 実機ドライブで再現せず＝チャットFABは**ネタを開いている間は描画されない**（App.tsx の !active 条件＝所見当時より後の構造）、左手行は響き最大展開（アルペジオ全ノブ）の最悪ケースでも画面内（lh y=157-260/高800）。7/22所見が腐っていた。スクショ＝進行役セッションの裁定ページに掲示・オーナー異論あれば再開。

## パターンライブラリ L4（2026-08-02）の耳確認リスト［耳/手＝オーナー実機・機械は全消化］
正準＝design「Task2/L4 トラックA」実施決定ブロック＋research `2026-07-25-L4-trackA-definitions.md`。新21型は辞書化・再seed済（118件・18セル全て4件以上）＝importダイアログのジャンル×場面で引ける。**プローブは1回に束ねる（型ごと個別依頼しない）・オーナー裁定＝ポップス/ボカロック優先**。
- **(a) バラード/ポップス系（jpop棚直結・最優先）**：PB-WHOLE-R10（10度全音符）・PB-LH8OCT（LHオクターブ8分）・PB-BLOCK8・PB-SUSBLD（**PR-SUS との差が耳で出るか**＝骨格同一・vel/tempo帯のみ違い）・BL-2BEAT・BL-ARPUP（**フォールバック譜 R-5-8 の響き**＝末尾10度を落とした形）・BL-8ROOT・ballad.rim8（サイドスティック）・ballad.soft16（**beat16.basic との差**＝弱音疎キック・差は微細）・halftime.ballad・beat8.ride。
- **(b) ボカロック系**：GT-MUTE8（**ブリッジミュート近似**＝GMにミュート音無し・rest短dur＋strumMs10）・AN-SYNC（前借りシンコペ）・VR-OCTRUN・VR-LINE8＋**co-tag妥当性**（vocarock 併記が棚で浮かないか・特に dbeat.basic＝パンク色の裁量濃）。
- **(c) EDM系（後回し可・落とさない）**：DN-PLUCK8・DN-GATE16（16分非対称ゲート）・DN-PAD4（**vel層ポンプ近似**）・ED-GATE8（裏長め）・four.edm16・four.clapride。

## 作詞補助アーク（2026-08-02）の耳確認リスト［耳/手＝オーナー実機・機械は全消化］
正準＝design §31-10 スライス7／§31-8。**残りは①だけ**（②は 2026-08-02 の裁定で消滅＝下）。詞先メロ（`gen_melody` に `lyrics` を渡す経路）の音数の決まり方は既定OFF＝今の音は従来のまま。耳で確かめて既定を反転する（反転＝コード1行＋テストの期待値、戻すのも同じ）。
- **①読みを取る（`readLyrics:true`）**：表記（漢字仮名交じり）から pyopenjtalk で読みを取ってモーラを数える。既定OFF＝表記の字をそのまま数える＝**漢字は1字1音に化けて音数がずれる**（例「君の名前を」＝5音／読みなら7音）。聴くところ＝**歌詞のとおりの音数で歌えているか**。既定にしてよいか（＝表記のまま渡せる道具にするか）。見る場所＝返りの `lyricPlan.readingSource`（none/pyopenjtalk/failed）＝どこから読みを取ったかが候補と一緒に出る。
- ~~**②「っ」「ー」に音符を立てる**～促音「っ」を VOICEVOX にどの lyric で送るか~~ → **✅ 裁定済みで消し込み（2026-08-02・不要＝撤去した）**。オーナー原文「**促音伸ばし音はいらない、打ち込むときに工夫するので**」＝**立てない（従来＝長音は延長・促音は詰め）で確定**。opt-in `standSpecialMoras`（旗・MCP パラメータ・返りの値）は同日撤去＝耳確認する物が無い。促音を VOICEVOX にどの文字で送るかも**音符を立てない以上は消滅**（§31-8）。
...
- **効きそうな方向**：(a) 動機/骨格に「長音アンカー」を持たせ拍頭や句頭を伸ばす、(b) リズム語彙に持続(タイ/白玉)を持つパターンを足し density と別に「音価の分散(rhythmicContrast ノブ)」で選好、(c) 句末カデンツ音を長く延ばす、(d) POP909の音価分布(IOI/duration)を実測して目標分布に寄せる。
- **◐実装状況（2026-07-21裏取り）＝大半が S4 リズムパーツ層で解決済**：(a)長音アンカー＝骨格側で抽出済（`skeletonModelData.ts:2` dur≥1拍を構造音に）、(b)持続パターン＝**リズムパーツ層**（`rhythmParts.ts`＝疎パーツ=白玉/長音を必ず含む・density と独立に小節ごと敷ける／MCP `rhythmParts`）で提供、(c)句末延長＝**flow ノブ**（穴を白玉で埋め句末/最終音を伸ばす・`generate.ts:167`）。テスト `melody-rhythm-parts.test.ts:73`。**残＝「1フレーズ内で自動的に長短対比を狙う」統合ノブ(rhythmicContrast)は未**（今は rhythmParts でユーザーが疎密を明示指定 or flow で句末延長する部品提供止まり）＋(d)POP909実測較正。
...
### C. 外に出す
- **音声(WAV/mp3)書き出し**（無し・MIDIのみ／2026-07-21確認：**未着手**。`OfflineAudioContext`/`MediaRecorder`/`startRendering` は web 全域 grep 0件。既存 wav はVOICEVOX歌唱の**デコード再生**用途＝書き出しではない）：`OfflineAudioContext`/`MediaRecorder` で合成をバウンス＝デモ共有/保存。**仮歌レンダとも土台共有**（仮歌の合成自体は上記で実装済＝ここは"合成音をファイルに焼く"経路）。
- **MusicXML 書き出し**（入力のみ／2026-07-21確認：**未着手**。`apps/web/src/musicxml.ts` は `parseMusicXml`(MusicXML→Note[])のみ・serialize/`score-partwise` 生成は 0件）：リードシート/外部エディタ受け渡し（**仮歌の橋＝VOICEVOX公式Song/OpenUTAU と同じ**）。
### D. メロの書きやすさ（ユーザーの苦手×最優先に効く）
- ◐**スケール/コードトーン ハイライト**＝**部分実装（2026-07-21裏取り・「無し」は腐り）**：スケールハイライトは**実装済**（PianoRoll 行を主音/調内/調外で色分け `PianoRoll.tsx:344-354`＋SkeletonEditor はスケール内ピッチに行を絞る `SkeletonEditor.tsx:120`）。**残＝コードトーン(R/3/5/7)ハイライトは未**（現コードの構成音pc を行/セル背景に付ける処理が無い＝スケールの `scaleCls` 分岐に並置するのが最小実装）。
...
## 機能（中〜大）
- ✅**仮歌入れ込み（歌声合成）＝実装済（2026-07-21実コード裏取り＝「着手時Task化」から更新・backlog腐り）**：VOICEVOX 2段合成（`apps/api/src/sing.ts` sing_frame_audio_query→frame_synthesis）＋MCP `sing_neta`（`mcp.ts:1044`）＋web `useVocalRender`（`useVocal.ts`）＋伴奏と同一クロックで wav 再生（`audio.ts`）が end-to-end で動作（asset role=render・content-hashキャッシュ・stale表示込み・engineは `CM_VOICEVOX_DIR` 依存/歌声モデル6000一択/音域[52,79]）。**残＝②書き出し(下記WAV/MusicXML項)・③表現編集(f0/volume/phonemeフレーム編集＝ビブラート/しゃくり)・歌い回しの耳較正**。以下は当初の研究/最小実装案（記録として残置）：流し込んだ歌詞(`Note.syllable`)でメロを"仮歌"として鳴らす。調査＝`docs/research/2026-07-01-singing-voice-synthesis.md`（**VOICEVOX歌唱をローカルCPUで実測＝リアルタイムより速い0.38x・音質OK・footprint≈2GB常駐1・sudo不要**）。最小実装案＝**①ラフ歌唱プレビュー**（VOICEVOX HTTP `sing_frame_audio_query→frame_synthesis`・Score={notes:[{key:MIDI,frame_length,lyric:かな}]}・93.75fps・先頭休符必須／**編集毎の自動再生成はしない＝「🎤歌わせる」明示レンダ＋content-hashキャッシュ＋stale表示**・job/asset(role=render)に乗る）→**②MusicXML/MIDI+歌詞の書き出し**（今は入力のみ＝橋を新設。VOICEVOX公式Song/OpenUTAU へ受け渡し）→**③表現編集(f0/volume/phonemeをフレーム編集＝ビブラート/しゃくり/強弱)は自作の余地**（offloadだけでなく自前UIも可・ユーザー意向）。声質そのものはモデル焼込＝SynthV領分。着手時 Task 化。
- ◐**コード楽器のテンション込みvoicing**＝**部分実装（2026-07-21裏取り）**：実音化 voiceToTop はコード品質から全構成音を積み（`music.ts:638`）、`QUALITY_INTERVALS` が 9/11/13 を保持（`packages/music-core/src/index.ts:64/74/75`）＝**コード品質が maj9/m9/13/11 なら 9/11/13 は実際に鳴る**。テンション品質を進行に載せる経路は `gen_chords genre:"citypop"`（`citypop.ts`）のみ。**残＝comping生成器 genComping 自身はテンション品質を注入せず R/3/5 三和音止まり＝voicing ノブとしての「9/11/13 を選んで積む」は未提供**（tones型 `ChordTone` は R/3/5/7 固定・`music.ts:584`）＋テンション配置(上に開く/5度オミット)設計＋**compingの声部進行最適化**（共通音保持/最小移動）は未。
...
- ~~**Capture.tsx が完全dead＋outbox退避の呼び元消滅**~~ → **✅撤去済で決着**（コミット `04aeb7c`「Capture+outbox撤去（オーナー判断）」・2026-07-21裏取り）：Capture.tsx はソースに不在（`git ls-files` 0件）・queueNeta/outbox も 0件・App.tsx に import 無し。dead でなく**撤去済み**＝この起票は消化済み（残骸は Jul18 の stale dist のみ・再ビルドで消える）。
...
- ~~**Chatの常時ポーリング（2req/4s）と target付きオープンの自動送信**~~ → **✅是正済**（コミット `04aeb7c`・2026-07-15オーナー承認・2026-07-21裏取り）：ポーリング 4s→**15s**（`Chat.tsx:213` setInterval 15000）・target付きオープンの**自動送信は無効化しボタン起点**（`Chat.tsx:599`）。1checkで2req(queued/running Promise.all)は現状も2本だが 15s間隔＝課金安全化は済み。
...
- ~~**弱起（負start）ノートの正しい扱い**~~ → **✅実装済（2026-07-21裏取り＝オーナー方針「1小節前に足す」で決着済み）**：MIDI書き出しは `prerollOffsetBeats(minStart, bpb)`（`music.ts:1073`・純関数＝テスト対象）で**小節単位に切り上げたオフセットだけ全ノートを後ろへシフト**＝弱起は前小節の途中に正しく居り小節頭も保たれる（`notesToMidi:1126`・負start無し＝bit一致・ループマーカーも同オフセット）。再生系も別途 `pickupSchedule`（`audio.test.ts:211` design#25）で保持（非ループ=+Lシフト/ループ=loopEnd+start）。「設計判断」は消化済み＝この項はクローズ。残る細部（再生とMIDIの表現差を"揃える"かは実害なく任意）。
...
- ~~**AnalysisWorkbenchのコード帯フィット/ズーム**~~ → **✅是正済（2026-07-21裏取り）**：`fitScale(totalBeat, containerWidth)`＝可視幅÷総拍数の全体フィット＋ズーム段(全体/×2/×4 `AWB_ZOOMS`)・初期=全体フィット（`AnalysisWorkbench.tsx:64-79,127,158`）。固定48pxは幅未測定時のフォールバックに降格・全座標が pxb 一元追従。以下は当初起票（記録）：バグでなくUX＝固定PXB=48px/拍でストリップが3万px超になり初期に1コードしか入らない問題。
...
## コード進行エディタ タイムライン化（#26）＝本体実装済・派生はゲート解除（2026-07-18起票→2026-07-21解除）
正典＝design.md #26／research `2026-07-18-chord-editor-timeline.md`。**✅#26本体（ChordEditor の折り返しタイムライン化）は実装済み**（2026-07-21裏取り＝`ChordEditor.tsx`＋`chordTimeline.ts` の reflow/insertAt/removeAt/snapLength/wrapRows・ピッカーシート・シーム挿入・消しゴム削除の全語彙あり。独立 `WrapTimeline`/`ChordBlock` への factor はせずインライン＋純関数配置）。**✅ゲート解除＝2026-07-21 オーナー実機評「折り返しコードエディタ良好」**。∴以下の "共有容器を他へ広げる" 派生は**着手可**（着手時 Task 化）：
...
- ~~**再生中コードのブロックハイライト**~~ → **✅実装済・テスト済（2026-07-21裏取り）**：`ChordEditor.tsx:60` activeIdx＋`:78-89` の 100ms ポーリング（`beatRef` を読み現在コードを findIndex）＋`:237` `.playing` 付与＋`chat.css:415-418` 赤左border/淡グロー。テスト `ChordEditor.test.tsx:136-148` 緑。※rAF化案は非推奨（`beatRef` は低頻度ポーリング前提の意図設計＝#76 が避けた毎フレーム再レンダ退行になる）。残＝実機で目視確認のみ。
...
- ~~**fit の歌詞対応 or 改名の判断**~~ → **✅改名で決着（2026-07-21・オーナー裁定＝②の道）**：tool id `fit`→**`weave`**／title「合わせる（基準に噛ませる・候補）」→**「絡める（基準＝コード/メロに噛み合うパートを作る・候補）」**。「合わせる/fit」の歌詞誘引を名前ごと断ち、実態（コード/メロに噛み合う別パート＝メロ/ベース/ハモ/対旋律の生成器）に合わせた。歌詞は依然扱わない（description の行き先案内は維持）。挙動・入出力は不変（名前と説明のみ）。参照更新＝mcp.ts/chat-session.ts(allowlist+散文)/http.ts/web chat-stream.ts(CANDIDATE_VERBS＝候補カード描画)/tests/design.md。api 114ファイル/1257テスト緑。正典＝design.md「MCP ツール説明文」節。**残＝web の dist 再ビルド**（chat-stream.ts 変更の実機反映）。歌詞対応の統合（①）は今回不採用。

## 作詞補助UI: イントネーション読み上げ（2026-07-27・願望・余力があれば）
句の編集画面（情報設計 `2026-07-27-lyric-assist-information-design.md` §7）で、詞のイントネーション確認の補助として **VOICEVOXで対象句を読み上げ**られると嬉しい（オーナー原文「せっかくVOICEVOXあるし読み上げとかもできるならしたいねー、あ、全然余力があればの話ね」）。既存の sing（歌唱）とは別に素の読み上げ（trai不要のTTS）。優先度低・作詞補助UI本体の実装が決まってから検討。

## ✅ 詞先で生まれるメロは歌詞で検索できない（2026-07-30 起票 → **2026-08-02 解決**・commit `67dab2e`）
**やったこと**：`apps/worker/src/cm_worker/search.py` の `_content_text` に `_lyric_text` を足し、**kind を問わず**
`content.lyric.phrases[].text` を索引テキストへ入れた（メロ＝音名＋句／曲コンテナの□・下書きも同じ形なので拾う）。
worker pytest 8緑（先に赤→緑・`test_content_text_includes_lyric_phrases`）。実測＝使い捨てのダミーメロ（title/text なし・句だけ
「きみのこえだけがのこる」）を作って検索 → **意味hitで1位**（rel 0.168・ゲート0.07）／言い換え「こえがのこる歌詞」でも1位。ダミーは削除済み。
**⚠ 判明した落とし穴（正準は design §31-10 スライス5）**：`neta_embedding` は `neta.updated` をキーにした**派生キャッシュ**なので、
**`scripts/restart.sh --search` では既存ネタは再埋め込みされない**（restart.sh のコメント「インデックス再構築 ~10分」は誤解を招く＝
実際は再構築ではなくモデル再ロード。新規/編集後のネタは次の検索で自動で乗る）。索引テキストの作り方を変えたときは
**該当行の `neta_embedding` を消す**のが唯一の反映手段。今回**既存の句持ち10件のキャッシュ削除は権限で実行できず未実施**＝
その10件（大半がQA/テスト用）は句で引けないまま＝**そのネタを一度編集すれば自動で乗る**。まとめて直すなら
`DELETE FROM neta_embedding WHERE neta_id IN (句を持つ neta の id)` を実行（数秒。全消しなら544件の再埋め込み）。
**ついでの所見（別件・未着手）**：①キーワード側（`neta-repo.listNeta` の `q`）は `title/text` の LIKE だけ＝句は**意味検索でしか**当たらない。
②`neta_embedding` に**消えたネタの残骸**が溜まっている（544ネタに対し3411行）＝検索は毎回この全行を読む。掃除の口が無い。

<details><summary>起票時の記述（2026-07-30）</summary>

**事実**：検索の索引に**句（`content.lyric`）が入っていない**。`apps/worker/src/cm_worker/search.py:70` `_text_of` が拾うのは
`kind / title / text / mood /` `_content_text`（`:43`）で、`_content_text` はメロについては**音名だけ**（`notes` の pitch を音名化）。
＝2026-07-30 に確定した (い-c) 遅延生成では**詞先のメロは title も text も持たず、歌詞は句にしか無い**ので、
**書いた歌詞の言葉では二度と探せない**（実機で確認＝「きみのこえだけがのこる」で検索してもそのネタは出ず、
別のテーマネタが意味検索で当たるだけだった。カードの見出しには出るので一覧を目で追えば見つかる）。
**直し方**：`_content_text` のメロ分岐に句の表記（`content.lyric.phrases[].text`）を足す＝数行。
ただし**意味索引の再構築が要る**（`bash scripts/restart.sh --search`・約10分・完了までクエリ無応答が仕様）。
**急がない理由**：詞先で句を作る画面（スライス5の通しで読む面）はまだ実装されていない＝いま句だけのメロは生まれない。
...
- **アプリ内クレジット画面**：`NOTICE.md` の内容（GMD の CC BY 4.0 表示）を実機の画面からも辿れるようにする。
  いまはリポジトリ直下の `NOTICE.md` とコード定数 `GMD_ATTRIBUTION` のみ。表示義務自体は満たしているが、
  配布物の画面から見えるほうが素直。
