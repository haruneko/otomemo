...
- **メロ生成エンジンの大物負債（骨格層の動機そのもの・S1-S3安定後に着手）** → **~~#10（死にプロト＋Note型）~~✅完了(2026-07-11)・#11（経路撤去＋ノブ再編・着手時に再分割）**：genMelody(generate.ts:498)に4経路がearly-return並存（補完/V2/motifModel/旧経路）・opts=40個がMCP/HTTP/web UIに伝播→**層別（骨格生成/表面化/装飾）ノブ再編＋旧経路撤去**。MCP/HTTPは常にuseV2:true＝motifModel/旧経路は本番不到達の疑い（撤去前に呼び出しグラフ全数確認が前提）。倚音/掛留は実質三重実装（applyExpression generate.ts:947／V2 melodyCells.ts:1235-が実効／motifModel melodyCells.ts:390-455）＝V2を正に一元化。
