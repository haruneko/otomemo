...
## 2026-09-16 追記＝S2/S3 へ戻る起点はここ
phrase_maker 取り込みアーク（別アーク）が「リフ文法」「変奏の層」の名で S3 を置き換えかけたが、**リフではなかった**として 2026-09-16 に撤去した
（`../archive/2026-09-16-riff-arc-revert-review.md`・design.md 追補 (k) の撤去記録＝負の知識5項）。**オーナーの課題（楽器固有のリフ・手癖を超える別パターン）の正準は本書と design「楽器アレンジの打ち込み助け＝計画確定」節のまま**。
S3 の入口＝既存 `gen_riff` verb（研究 `2026-07-14-riff-ostinato-design.md`）。足場に使える残り資産＝`verify/fretboard.ts`（指板の到達可能性）・JZ-WALK の「表を敷かず規則で選ぶ」作り・候補トレイに複数件を積む動線。
S2/S3 へ戻ることの確認はオーナー未裁定（backlog）。
...
**統合工事（Task #5）は 2026-09-16 に機械側完了**（残＝実DB再seed・実機確認）。**次＝S2（配札）着手（Task #3）**。

## 統合工事（Task #5）＝2026-09-16 機械側完了

**状態（2026-09-16）**：着手前に実コードで確認＝本当に未着手だった（取込ダイアログ・ゴースト・NetaDialog 配線・OG 表示名すべて旧状態）。
コミット `cff00ac`（design 先行）→ `048ad9e`（web）→ `3531b67`（api 表示名）で下の 1〜5 を実施。細部は design「実装の細部（2026-09-16）」。
- 精査で見つけた穴＝**ピッカーは scope:project しか並べていなかった**（型の棚は取込ダイアログからしか見えなかった）→ パターン系レーンに「ライブラリの型」群を追加。
- 文脈試聴は**追加モードに置き換え**（差し替えモードは呼び手が消えたので削除）。FormStrip のピッカーは従来ワンショット。
- **残り**：①実DBの再seed（`seed-pattern-library.ts`・並行担当の作業との衝突回避のため未実施＝親が再起動時に実施）②dist焼き・実機でピッカーを触る確認 ③オーナー耳/手触り（▶⇄■の最小表示で足りるか）。導線テストの再試験は未実施。
- 生成結果は不変（UI 導線と seed 表示名だけ。`scenes` 文言は gen_chord_pattern 型一覧の label にも出るが content は不変）。
...
- **Task #5（統合工事）**：2026-09-16 機械側完了（上記）。残＝実DB再seed・実機確認・耳。
