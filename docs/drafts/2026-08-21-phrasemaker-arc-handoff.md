...
**真因調査と裁定（2026-09-15）**＝`docs/research/2026-09-15-anchor-rigidity-guitar-motion-rootcause.md`。固さの主因は錨でなく**体（固定2小節のペダル型の表・変奏なし）**、ギターも同根。源流のオーナーもベース単体には「微妙では？」を出していた（移植時の前提の読み違い）。裁定＝**錨は変わり目だけ必須／刻みの音価はつまみ／変奏は候補を段で出す**。案2・案6 を実装中、案1（変奏の層）は drafts で設計中。
