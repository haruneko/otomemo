# Otomemo 進捗・管理表（2026-07-01 時点スナップショット／更新停止）

最終更新: 2026-07-01（凍結）。表題以外の本文は当時のまま保持。

> ⚠️ **この doc は「現況の正準」ではない（2026-07-18 確定）**：残タスク・進捗の正準は **Task 機能／`docs/backlog.md`（保留）／`docs/design.md`（設計決定）**（CLAUDE.md 準拠）。本 doc は 2026-07-01 時点の領域別スナップショットとして残す**歴史記録**（当時の到達点を辿るための参照）。以後の出荷は下記「追補」と正準 doc を見る。

**2026-07-01 以降に出荷した大物（追補・見出しのみ／詳細は design.md・backlog.md・git log）**：
- チャット常駐化＝**作曲MCP＋薄いClaudeラッパー**へ設計転換（worker脳を撤去・Claudeクライアントが脳）
- **曲フォーム＝分家モデル**（アレンジ層：フォームストリップ／part別MIDI書き出し／参照既定＋浅いfork＋系譜）
- **仮歌トラック**（VOICEVOX で歌わせる・歌声選択・アクセント注入・赤黄ハイライト）
- **アナリーゼ読み筋層 v2.1**（digest＝facts射影・所見3層・chat射影の軽量化）
- **コード進行エディタのタイムライン化（#26）** ／ **再生経路の一本化（#27）** ／ **曲編集を縦セットリスト＋ヘッダミニマップ化（#28）**
- **表現力・ヒューマナイズ統一（#29）**：生成velocity/beatsPerStep 再生・長押し分割＋コードアクセント・全楽器のノリ（P2 は長押しドラッグ+モードへ）
- **公開準備**：MIT LICENSE・README 公開語化＋帰属節・アプリ**表示名 Otomemo** へ改名（技術識別子 `creative_manager`/`cm-*` は据置）
...
（以下 §1〜§4 は 2026-07-01 時点の記述。上記追補と正準 doc で上書きされている項目がある点に留意。）
