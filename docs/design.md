...
- ノブ再編・旧経路撤去は S1-S3 の安定後に backlog「死にコード撤去」と合流。**→ J1〜J3 実装済（2026-07-11）**：J1=呼出グラフ全数調査（research doc）／J2a 3/4・6/4／J2b chordless／J2c fit useV2化／**J3 旧経路④撤去＝V2 一本化（下記【J3】・api874緑）**。残＝**#16 ③motifModel(genMotifMelody)撤去＋appoggiatura ノブ削除**（テスト移植を伴う）。
...
- **既知の残バグ（J3 スコープ外・backlog 送り）**：**6/4（bpb=6）の V2 が末尾群で負dur 音を出す**（例 start=10/dur=-2・bars 非依存・pickup 非依存＝J2a の 3+3 群マッピング由来）。J3 は②useV2 経路を無改変で切り出したのみ＝この bug は J2a 由来で本タスクでは触らない（V2 内部の bit を変えるため別 TDD が要る）。invariants の対応拍子スイープは 4/4・3/4・6/8 の健全側のみ担保し、6/4 はコメントで除外＋backlog 記載。
