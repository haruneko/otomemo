...
**直し方**：`isOnsetMora` を「全モーラが音符を持つ」に変える（`planLyricMelody` の音数＝総モーラ数になる）。#13d の受け入れ条件「モーラ⇔音数 100%一致」の「モーラ」の数え方も っ/ー 込みに更新する。**生成される音符数が変わる＝耳で確かめてから入れる（スライス7）**。`suggestLyricRhythm` の候補は「提案」であって保存物ではないので今回は触らない。
...
- `planLyricMelody` が読み（かな）を受け取る形へ（同期のまま・呼び側で先に読みを解く）／`isOnsetMora` を裁定「立てる」に合わせる（§31-8）／`generate.ts:784` の返り content に句を載せる／`mcp.ts:964`・`mcp.ts:1250` の説明文を実装ごと直す。
