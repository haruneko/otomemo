...
- **控えの鮮度**：`reading.forText !== text` なら控えを使わない＝「読みを反映」を押すまで印も高低も出さない（状態チップが「当て直していません」相当を言う＝文言は §31-11 裁定待ち16）。
...
- **画面の姿＝プルダウンの組み合わせ（2026-07-30c オーナー提案＋独立監査で是正）**。実装＝`apps/web/src/components/LyricSplitComposer.tsx`＋純ヘルパー `apps/web/src/lyricSplitCompose.ts`。監査の是正を反映：
  ...
  - **割り方の図＝「メロの概形」（2026-07-31 オーナー要望＋設計ワークフローで統合）**＝ASCII の `●○·` を廃し、`MiniRoll`（一覧の縮小ピアノロール）と同じ **SVG 矩形DNA（pitch×time）＋ドラムの拍格子**で描く。実装＝`SplitRoll`（`apps/web/src/components/SplitRoll.tsx`）＋純関数 `splitRollGeom`（`apps/web/src/lyricSplitCompose.ts`・`onsetFigure` と同じ orig/added 述語＝テストで1対1一致を縛る）。
    ...
    - **色の最終裁定は実機モックで**（黄版／青系代替版のスクショ2枚）＝有力仮説（黄）で先行実装済み・オーナー反応待ち。他のミニ表示（`MiniRoll`/ドラム）との整合は満たす。
...
- **スライス5（通しで読む面）だけが新しい全画面**＝骨格の机の形に倣う：`lyricViewTarget` 的な state を1つ・closeTop の優先順は `deskTarget` と同格の位置・`anyOpen` に足す・分岐は :718 の並びに1本。表示するパーツの選択は localStorage（既存の流儀。サーバに持つ仕組みは無い）。画面の**姿**はモック裁定待ち＝設計で決めるのは器だけ。
...
**下書きの持ち方・「確定」という操作の存在・書きかけの破棄は私の派生した設計判断＝未レビュー**（§31-11 の17 に起票）。

- **決まったこと（オーナー裁定の範囲）**：
  1. **(い-c) 遅延生成**＝**空のうちは実体を作らない。何か書いたらメロが生まれる。**
     - ここから私が派生させた形（**未レビュー・裁定待ち＝§31-11 の17**）＝詞先の枠は通しで読む面の中の下書き（クライアント state）で、
       **句を確定した瞬間**にメロを作って同時に配置する／**書きかけで離れたら下書きは消える**。
       ※「何か書いたら」は**1文字目**とも読める＝破棄も「確定」の存在も、オーナーの言葉からは決まらない。
       ※監査の指摘（推測）＝破棄は §31-7「自動保存が句を落とさないこと」と「セクション単位で往復しながら書く」作業像と逆向きになりうる。
...
14. 歌声エンジンを常時起動にするか。
15. モックの裁定待ち9件（印の規則・候補の並べ方・通しの面の既定の姿ほか）。
16. **メロ編集画面の歌詞まわりの画面裁定（2026-07-30 追加・入口＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §5・材料＝`docs/archive/2026-07-30-lyric-editor-screen-decisions.md`＋モック `docs/archive/2026-08-02-lyric-mode-ui-mock.html`＝**(a) 決着により archive へ移送済み**。この項目は (b)(c)(d) のレビュー待ちのために残る）**
    - **(b)(c)(d) は暫定実装・レビュー待ち＝この一覧から外さない**（2026-07-30b 監査の指摘1-D で訂正。以前「決着」と書いたが、決めたのは私で**オーナーは見ていない**＝次の機会に必ず上げる）。中身＝下の「⚠ オーナー不在中に推しで進めた」参照。(b)＝案C（未反映のときだけ「読みを反映」を1行に出す）／(c)＝案B（韻律トグルはパネルの奥へ＋OFF を覚える）／(d)＝文言一式（`あとN音`→**字足らずN**・未反映チップ新設**まだ反映していません**・`読みを取ります`→**まだ読みを取っていません**・`クリア`→**かなを消す**）。
    ...
    - **⚠ オーナー不在中に推しで進めた（2026-07-30b・オーナー指示「しばらく…何も見られないから進めちゃって」）**：(b)(c)(d) は**私の推しをそのまま採って実装した＝オーナー未レビュー**。根拠の表は材料 doc（`docs/archive/2026-07-30-lyric-editor-screen-decisions.md`）にそのまま残っている。
      - **戻す費用（2026-07-30b 監査の指摘2-2 で正確に書き直した）**：(c) は localStorage 1キーとトグルの置き場・(d) は文字列とテスト2行＝**どちらも小さい**。**(b) だけは注意**＝「未反映のときだけ反映ボタン」という*判断*を戻すのは表示条件1つだが、**常時1行＋パネルという器ごと戻すなら `8c4e98b` の revert**（`PianoRoll.tsx` は +166行の作り替え）。以前ここに「(b) は表示条件1つ」とだけ書いたのは過小だった。
      - **併記すべき事実**：`5bc4a49` 時点の §31-9 には「**裁定が出るまで展開パネルの実装は入れない**」と私自身が書いていた。その門を「進めちゃって」で越えている。

17. **(い-c) 遅延生成の粒度（2026-07-30b 追加・上の §31-9 の ⚠ から起票）**：①「何か書いたら生まれる」の**書いた粒度**（1文字目で生まれるのか／「確定」の操作を置くのか） ②**書きかけでやめたときに残すか捨てるか**（私の派生案は「捨てる」だったが、§31-7「自動保存が句を落とさないこと」と逆向きになりうる） ③確定の合図を置くならその形（改行／完了ボタン）。**①②③はオーナーの発言からは決まらない**＝スライス5の実装前に裁定が要る。
