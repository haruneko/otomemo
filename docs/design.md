...
> **歌詞の置き場は案(い)＝人が合図して空のメロを置き、そこに書く、で確定（2026-07-30 オーナー裁定・代わりに既存メロディなどからのコピー機能を充実させる＝スライス6）**。残る枝＝空メロの見た目（(い-a) 一覧から隠す／(い-b) 共有の空メロ1つ・何かが入った瞬間に新しいメロを産む）は未裁定（§31-9）。スライス1〜4は4案のどれでも壊れない形で確定してあった（§31-0 の検算）＝この裁定による手戻りは無い。
...
2. **スライス1の保存は「メロの content に lyric を素通しする」以上のことを決めない。** (い-b)（共有の空メロ1つ・何か入った瞬間に産む）が裁定された場合、保存の**前段**に「産む」フックが挟まる。`useNetaEditor.ts:450` の `flushSave` は既に CoW ゲート（`useCowGuard.ts:70` `guardAction`）という前段を持っており、同じ位置に挟める＝作りを塞がない。フック自体はスライス5以降の仕事で、スライス1では作らない。
...
- **残る枝（裁定待ち）＝空メロの見た目**：**(い-a) 空のメロは一覧では非アクティブにして見せない**／**(い-b) プレースホルダー用の空メロは1つだけ共有し、何かが入った瞬間に新しいメロを産む**（どちらもオーナー自身の案）。**スライス5の共通部分（通しで読む面・空メロを置いて書く操作・尺の手当て）はこの枝に依存しない＝着手可。** 枝ごとの追加作業は §31-10 スライス5。
...
**スライス5＝詞先の句と、通しで読む面（置き場＝案(い) 確定 2026-07-30・共通部分は着手可。空メロの見た目 (い-a)/(い-b) だけ裁定待ち）**
...
- **案(い)（空のメロを置いて書く）＝確定（2026-07-30）**：「空のメロを作って置く」操作（既存の作成＋配置の組み合わせ・1ボタン程度）／**尺の手当てが必須**＝音符0個のネタは尺計算で1小節と数えられる（`apps/web/src/sectionContext.ts:81,88`＝音符が無ければ `ctx.BPB`）ので、空メロは `bars` ヒントを尊重する形へ（§31-0 の範囲既定と同じ向き）／併合は配置ツリー**1系統**＝曲の書き込み口・句を移す操作・上の例外の明記は**全部不要**。
  ...
  - (い-a)（一覧から隠す）を採るなら＋`schemas.ts:6` の scope enum は閉じていて3用途共有（値を足すと `usePlacePicker.ts`・`FormStrip.tsx` の library→project 自動コピー分岐にも波及）→ enum 拡張かタグ方式かをここで決める。非アクティブの出入り規則も要る。
  - (い-b)（共有の空メロ1つ・何か入った瞬間に産む）を採るなら＋「入った瞬間に産む」を保存経路の前段に（§31-0 の守ること2）＋`useCowGuard.ts:70` の穴（トップ開き＝parentId 無しだと確認なしで原本へ書く）を**先に**塞ぐ＋産まれたメロの命名。
...
1. ~~歌詞の置き場4案~~ → **決着（2026-07-30 オーナー裁定）＝案(い) 空のメロを置いてそこに書く。代わりにコピー機能を充実（§31-9・スライス6）**。**残る枝＝空メロの見た目：(い-a) 一覧では非アクティブにして見せない／(い-b) 共有の空メロ1つ・何かが入った瞬間に新しいメロを産む**（スライス5の共通部分は止めない）。
