...
- **確定した形（2026-07-30b・実装済み）**：**常設からは外す／歌詞パネルの奥に置く**＝かな欄＋「流し込む」は `wired` でも消さない。
  ...
  - 非配線の呼び側は従来どおり常設（後退ゼロ）。テストで両側を縛る。音符を割って合わせる操作を残すかどうかは、**「音楽的に成立する割り方を候補として出せるか」の研究の結論待ち**＝出せる見込みが立たないなら操作ごと作らず余りは切り捨て（研究文書は 2026-07-30 着手済み）。残す場合の置き場は歌詞パネルの奥・明示操作＋確認つき・名前は音符が変わることが読める言葉（仮称のまま＝§31-11 裁定待ち16）。`set_lyric`・詞先メロ・句の面が配線されていない呼び側は従来どおり（後退ゼロ）。＝architecture「黙って実体を作らない」の受け＝**歌詞を打っただけで音符が割れることはもう起こらない**。
