...
16. **メロ編集画面の歌詞まわりの画面裁定（2026-07-30 追加・入口＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §5）**：(a) 詞モード（1音符ずつの直し）の見せ方＝実装済みで一等地にあるがオーナーが存在を認識していなかった事実をどう受けるか（文脈行つき2行バー案の採否含む） (b) 歌詞行1行表示の見た目（チップ1個で足りるか・句が複数になった後の畳み方） (c) 韻律チェックのトグルを歌詞パネルの奥へ移すか（印は既定ONのまま） (d) パネル・状態チップ・音符を割る操作の文言。※ボタン名「読みを反映」と余り音符「ラ」は裁定済み（2026-07-29）＝ここに含まない。
