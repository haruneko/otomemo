...
- **Section 仮歌（2026-07-15・オーナー要望「Sectionで仮歌が入らないのはもったいない」）**：**`POST /sing`**（ネタ非依存＝`{notes,bpm,speaker?}`→wav asset・ネタにリンクしない・content-hash 重複排除が自然キャッシュ＝`singHashOf`/`findCachedSing`）＋ web＝SectionEditor の**♪仮歌トグル**（TransportBar）。メロ声部は `compositeNotes` 再利用で絶対拍連結（`vocalMelodyFromComposite`・弱起はv1クランプ＋注記）→ `playNotes` の同一 Tone.Transport クロックに AudioBufferSource を予約＝伴奏と同期・ON中はメロ楽器を `muteMelodyForVocal` でミュート。ループ＝毎周リスタート（transport.schedule の性質で自然に）・pause＝歌は停止し次境界で復帰・**OFF（既定）は従来と完全一致（bit-safe）**。メロ/テンポ/歌詞変更で自動OFF＋作り直し促し。
