...
最終更新: 2026-08-02（**実機スクショで B1/B2/B3 を追認＝作詞補助アークの画面の未決がゼロになった**。オーナー原文「**B1-3 は良い→良いです**」。**B1＝通しで読む面（LyricOverview）の姿＋(い-c) 遅延生成の粒度＝確定**（スクショ3枚＝既定の静かな姿／[表示]全部オン／「＋ ここに歌詞」）＝有力仮説で先行実装した形（態A既定＝1句1行・表記が主役／読み・高低・メロ帯は[表示]シート＋localStorage／穴は□・「＋ ここに歌詞」は曲に穴を開けるだけでメロを作らない／メロは**句の確定**で生む・合図は Enter か確定・書きかけは残さない）が**そのまま正**＝**§31-11 の17 は決着で閉じ**。**B2＝§31-11 の16 (b)(c)(d) を追認＝レビュー済み・採用確定**（スクショ4枚）＝(b) 未反映のときだけ「読みを反映」（案C）／(c) 韻律トグルは奥へ＋OFF を覚える（案B）／(d) 文言一式（字足らずN・まだ反映していません・まだ読みを取っていません・かなを消す）＝**「オーナー未レビュー・1件ずつ戻せる」は解消**＝**項目16 は (a)(b)(c)(d) 全部決着で閉じ**。**B3＝SplitRoll の色＝黄で確定**（現状＝黄のスクショ2枚と青系代案モックの比較で「良い」）＝**黄を維持・青系は却下**＝§31-5 の「色の最終裁定は実機モックで」は決着。＝**作詞補助アークで「オーナー実機反応待ち／未レビュー」の札は残っていない**）／2026-08-02（**詞モードの見せ方＝(a) 決着**＝オーナー裁定「かなチップは破綻するからいや。ボタンは歌詞のまま。送り方は候補2がきれい」＝**候補2の送りバーを採用**（バーを「◀｜音名・拍｜かな｜▶｜✕」に・Enter/Tab=▶・Shift+Enter/Tab=◀・Esc=✕・`enterKeyHint="next"`・対象音符を自動スクロールで追う）／**候補4（かなチップ帯＋まとめ打ち）は却下**／**候補3（入口を歌詞行の隣へ移設）は不採用＝一等地維持**／**モード行のラベルは「歌詞」のまま＝改名しない**。未決①〜⑤は全て閉じた（①バーのボタンが正・物理キーは近道 ②一等地維持 ③「歌詞」据置 ④バーの自己記述＋enterKeyHint ⑤代替案検討は消化済）。**あわせて本書のラベル記述のドリフトを是正**＝「(a) 案B＝『詞』→『1音ずつ』で確定・実装済み」と書いていたが実ラベルは `3cd5bf3`（2026-07-31）で既に**「歌詞」**＝裁定で「歌詞」が正と確定。材料 doc とモックは `docs/archive/` へ移送（§31-9・§31-11 の16 (a)。**(b)(c)(d) のレビューも同じ日のうちに済んだ＝上の追認**）／2026-07-30b（**空メロの見た目も確定＝(い-c) 遅延生成**＝オーナー裁定「なにか書いたらメロが生まれるようにして」＝空のうちは実体を作らない・句の確定で作成＋配置を1操作・カードには句を出す。＝**スライス5は裁定待ちゼロで全部着手可**。却下＝(い-a) 一覧から隠す／(い-b) 共有の空メロが産む。副産物＝尺の手当ての bit 一致の地雷が構造で消える＝§31-9・§31-10 スライス5）。**あわせて歌詞パネル（常時1行＋展開）を実装**＝(b) 未反映のときだけ反映ボタン／(c) 韻律トグルは奥へ＋OFFを覚える／(d) 文言一式（字足らずN・まだ反映していません・まだ読みを取っていません・かなを消す）＝オーナー不在中に推しで進めた（**→ 2026-08-02 に実機スクショで追認＝採用確定**・§31-11 の16）。**かな欄＋「流し込む」は配線された画面から退役**（§31-5 の裁定どおり・非配線側は従来のまま）。実測（360×800）＝歌詞まわりの常設 約220px→**52px**・ロール上端261px/下端743px＝折り返し内・横あふれ無し。実機フロー（読みを反映→字余り3→奥の段）も通した。**(a) 詞モードの見せ方も裁定＝2026-07-30c に案B（「詞」→「1音ずつ」＝ボタンに言葉を出す）で確定・実装済み**（オーナー「文句あったら後で言う」＝推しで先行・未レビュー・戻せる。§31-11 の16 (a)）。**新しい積み＝句が検索の索引に入っていない**（backlog）／2026-07-30（**歌詞の置き場＝案(い) で確定**＝オーナー裁定「空のメロディを入れてそこに入れておく。代わりに既存メロディなどからのコピー機能を充実させる」＝§31-9。あわせてメロ編集画面の歌詞まわりの裁定済み分を反映＝適用は明示ボタン**「読みを反映」**・`placeMoras` の「ー」自動挿入をやめる・読みが足りない余り音符は仮歌「ラ」で鳴る・「合わせる」（音符を割る操作）の存否は研究待ち（§31-2 契約3／§31-3(d)／§31-5／§31-9／§31-11。根拠＝`docs/research/2026-07-29-lyric-editor-screen-design.md`）／2026-07-29（**「#31 作詞補助＝歌詞を書く」を新設**＝歌詞の正データは句（漢字仮名交じりの普通の文章）でメロの `content.lyric` に持つ・`Note.syllable` は仮歌のための写し・読みは pyopenjtalk からまとめて取る・音符を変えずに写す `placeMoras`・字余りとメロが途中の言い分け・人の直しの置き場・自動保存で句を落とさないこと。あわせて #13／#13b／「歌詞をメロディに流し込む」／#16(C')／content スキーマの、実態や確定と食い違う記述を訂正（一覧＝§31-12）。歌詞の置き場は案(い) を仮置きとしていたが **2026-07-30 に案(い) で確定**（§31-9）。**⚠ この 2026-07-29 分はオーナーのレビューを受けていない**＝オーナーの了解のうえ留保つきで先行している。上位の根拠＝requirements「歌詞を書く」節・architecture 同日追記（これらも未レビュー）／2026-07-18 表題を Otomemo に統一。GitHub リポジトリも otomemo。技術識別子＝ローカルdir/ルートpackage `creative_manager`・`cm-*`・MCP `creative-manager` は据置＝下「UI/ブランド刷新」に権威記述。本書は随時追記の決定ログ）

要件: `docs/requirements.md` ／ アーキテクチャ: `docs/architecture.md`。
ここは統合設計＝**決定ログ**。実装はこれに沿って進める。**残タスク・進捗の正準は Task 機能／`docs/backlog.md`（保留）／本書（設計決定）**（CLAUDE.md 準拠）。`docs/archive/status.md` は 2026-07-01 時点のスナップショット（更新停止・歴史記録）。
...
- **⚠ 訂正＝この (C') 段は未実装のまま棚上げ（2026-07-29・オーナー未レビュー）**：`fit_context` はリポジトリ全体で**0ヒット**＝解決器も展開形も1つも書かれていない。`fit_to` も `apps/api/src/mcp.ts:647` の説明文に `params.condition={fit_to,by}` と書いてあるだけで、`condition` は `apps/api/src/repo/job-repo.ts:113,126` が params として持ち回るだけ（読む側がいない）。さらに**解決を担うはずだった worker（ジョブ消化）は 2026-07-05 に撤去済み**で、`split_mora` も存在しない（上「歌詞をメロディに流し込む」の訂正）。＝**この段は「未実装の設計案」として読むこと。** 歌詞の音数合わせ（`by:"syllable"`）にあたる部分は #31 が置き換える＝表記から pyopenjtalk で読みを取り（§31-3）、モーラを数える正を一本化する（§31-4）。他の `by`（harmony ほか）を実装するときは、worker 前提でなく api(TS) の中で解決先を決め直すこと。
...
- 合成(section/song)：content でなく `compose_edge`（子＋position）で表す。section/song ＝エディタのメインペーン。
  - ※歌詞の置き場（メロがまだ無い場所＝詞先の句をどこが持つか）は**案(い)＝人が合図して空のメロを置きそこに書く、で確定（2026-07-30 オーナー裁定）**＝この行に例外は増えない（曲は歌詞を持たない・§31-9）。
...
- **歌詞の本体＝`melody.content.lyric`（2026-07-29 追加・⚠オーナー未レビュー・詳細は §31-1）**：メロの content に `lyric: { phrases: LyricPhrase[] }` を持つ。句が持つのは**漢字仮名交じりの表記**（＝正データ）で、かな・高低・音符への割付は表記から導かれる控え。`Note.syllable` は**仮歌のための写し**であって歌詞の本体ではない。content は素通しの JSON（`apps/api/src/schemas.ts:16` が `z.unknown()`）＝**DB スキーマ変更・移行は不要**。既存の `notes / program / sing / feel` は不変で、`lyric` 未指定なら**キーを生やさない＝既存データと bit 一致**（§31-7）。音符を割る流し込み（上の行）は**人が「合わせる」を押したときだけ**走る（§31-5）。
- **コントロールカーブは無し（例外＝サステインペダル CC64 のみ・2026-09-17 オーナー裁定）**：CC/オートメーション/ピッチベンド/エクスプレッション等は持たない。content は ノート(pitch/start/dur/vel)＋楽器 のみ。**ただしサステインペダル（CC64）に限り持てる**（下の小節）。
- **サステインペダル（CC64）の持ち方（2026-09-17・裁定＝音の長さの延長による近似は採らない・正典＝`docs/drafts/2026-09-16-handframe-evolution-design.md` 末尾の裁定）**
  - **形**＝content に additive の **`pedal?: { start: number; dur: number }[]`**（踏む時刻と踏んでいる長さ＝オン/オフの時刻の対）。**時間軸はノートと同じ**＝拍（四分=1.0）の float・content の頭＝0 起点。ステップ格子の content（和音パターン等）でも拍で持つ（離す時刻は格子に乗らない＝試作ではコードの替わり目の 20ms 前に離す）。値は踏む（127）/離す（0）の2値のみ＝ハーフペダル・連続カーブは持たない。区間は start 昇順・重ならない。
  - **既存は不変**＝`pedal` 未指定なら**キーを生やさない**＝既存 content・再生・書き出しと bit 一致。移調・トランスポーズはペダルに触れない（時刻だけのデータ）。
...
- **新 kind「コード楽器パターン（chord_pattern）」**＝**進行に解決する相対型**（相対ベースの和音版＝姉妹）。section のコード進行に合成時解決・自前の音色・複数重ねOK（ピアノ/ギター等）。content：
  ...
  - **和音パターンの明示の音（`notes?`・2026-09-17 追補・正典＝`docs/drafts/2026-09-16-handframe-evolution-design.md` §4-2）**＝ピアノ伴奏の生成器（手が届く2〜4音を打点ごとに決める）の音を、和音パターンの形のまま運ぶ additive の口。
    ...
    - **生成したピアノ伴奏の扱い（2026-09-17〜18 オーナー裁定・経緯＝`docs/archive/2026-09-17-generated-piano-handling.md`）**
      ...
      - **編集画面＝画面 B（2026-09-23 オーナー裁定・09-18 の画面 A を差し戻し）**：原文「A やるならシーケンサでいい。B くらいの抽象度がほしい」「（打点を動かしたら）実際に演奏がその辺に変わる」ことが要件。**人＝いつ弾くか／機械＝どの音をどの手の形で弾くか**の分業。
        - **右手の段**＝16分のマス×セクション全小節（小節ごとに違う打点をそのまま見せる）。1マス＝**なし／和音／単音**（タップで 空→和音→単音→空）。和音＝2音以上を同時に押さえる打点・単音＝1音。
        - **左手の段**＝「押さえる時」の1段（押さえる音＝3度と7度の殻・C3〜B3 は機械が決める）。度数の段（R/3/5/7/8）は置かない。
        - **打点を変えたら機械が弾き直す**（web でその場・music-core の同じ生成器）。**変えた位置より前の小節は変わらない**（同じ種で同じ順に決めるので）。後ろは手の流れが変わるので変わりうる。
        - **往復一致（契約）**＝生成した演奏から読み取ったマス目を渡して同じ種で弾き直すと、演奏が完全に一致する。＝開いただけ・触っただけでは音が変わらない。
        - 下に実際の高さの概形ロール（既存）。つまみ＝**別案**（種＋1・マス目は保つ）・**右手の高さ**・**打鍵の揺れ**・**マス目を生成に戻す**（人の指定を捨てる）。8分裏の単音のつまみはマス目が引き受けるので編集画面には置かない（生成の引き出しには残す）。
        - **保存**＝来歴 `gen` に `rhythm?: { rh: {step, kind:"grab"|"single"}[]; lh: number[] }`（step＝セクション先頭からの16分・人が触ったときだけ持つ）と `chords`（生成に使った進行＝拍つき）。生きたレシピの規則の内側（相対の譜＋来歴から配り直せる）。
        - **生成器への渡し方**＝`HandFrameBandOptions.rhythm`。区切り（2拍 or 1小節）ごとに「その区切りの打点と種類」を持ち、生成器は各打点でまず従来どおり決め（乱数も従来どおり引く）、**指定と同じ種類ならそのまま**・**なしなら鳴らさない**・**違えば指定の種類の候補から選び直す**（無ければ 和音＝その場で掴み直し／単音＝和音の最高音だけ）。指定の打点が器に無い位置なら、その位置も同じ決め方で回す。左手は指定の打点ごとに殻を押さえ直す。指定の無い区切り＝指定なし（なし扱い）。
        - 人が足した打点に「ふつうの和音」は混ざらない（明示の音の無い打点を作らない）。
      ...
      - **両手の音域（2026-09-23 オーナー裁定）**＝**左手の殻はベースの壁から1オクターブ `[bassMax, bassMax+11]`（C3〜B3）＝既定**（原文「左手はそれでよろしい」）。**右手の高さは選べる**（原文「右手の音域は本当は選べたらいい」）＝右手の下限 `rhFrom`＝**C4（60・既定）／C5（72）**の2段（右手の和音は根音から積む形を1オクターブ単位で上げるだけなので、間の高さ＝例 F4 は実測で C5 とほぼ同じ 69..88 になる＝段として立たない。間を作るには和音の転回を選ぶ作りが要る＝未着手）。右手の掴む和音はこの下限より上に置き、単音の下限も連動。右手の動き・リズム・ペダル・強弱の作りは不変。試作 #1 の置き方（左手が中音域・右手はベースの壁のすぐ上＝左右が重なる）は `register:"source"` で残す（Python の基準音との一致テスト用）。入口＝`pianoRhFrom`（HTTP・MCP・`/gen/section` の `body.chord`）＝60/72・未指定＝60。実測（試作の進行・種 1234/7）＝60→右手 59..76／72→右手 71..88。web＝ピアノ伴奏のつまみ「右手の高さ」。
...
- **C. 配置の長さ整合＋ピッカーのコーパス氾濫**：ループ系(リズム/コード楽器)は配置時に**セクション末尾まで自動で敷き詰め**(`loopPositions`)＝melodyは8小節なのにrhythmだけ1小節、のムラを解消。ピッカーは**コーパス(library)を既定で隠す**(トグルで表示)＝自作ネタが埋もれない。web257緑・実機で6/8・16小節セクション成立を確認。**（2026-07-23 Task1g で書き分け：この「library を隠す」は place モード（Section に子を置く）専用の方針。パターン取得の pick モードはライブラリを"見せる"のが目的＝pick では反転する。両モードで scope 方針が逆＝矛盾でなく用途別。）**
- **決定：ボイシング入力を「トップ狙い音」ベースへ（2026-07-04・オーナー発案／方向確定・エンジン先行）**：現状の R/3/5/7＋open/close＋octave は抽象パラメータで、一番audibleな**トップ声部（コンピングの旋律）を握れない**。→ **トップ声部の"狙い音"を人が決め、各コードでそれに最寄りのコードトーンを最高声部に採り、内声を下へ自動配置**（`voiceToTop` in music.ts・`ChordVoicing.top?`）。旨み＝レジスタが一定に保たれ**進行間の声部進行が自動で滑らか**（backlog「compingの声部進行最適化」を回収）。**トップは絶対採用**（調/コードが変わってもコンピングの音域は動かさない＝物理レジスタ。相対は将来トグル）。**"できるか"問題＝進行非依存で成立**：トップ＝絶対の狙い音（メロ実体でなく音域の磁石）だから、どの進行に乗せても各コードで最寄りトーンをトップに採るだけ。同距離時の優先やベース分離は「そこまでやるなら DAW」で割り切り（再生は多少雑でOK＝オーナー了承）。ベースはベースパートに任せる方針。**✅実装完了(2026-07-04・web249緑)**：①エンジン(構成音の手選択を撤去＝鳴る音はコード質から自動導出・`voiceToTop`／`powerChord`でR+5間引き／`arpDir`で向き＝当初は音域=voicing継承・別指定なし〈→**2026-07-13 の `arpOctaves`/`arpReset` 決定で「駆け上がり幅×区切り」を追加＝この記述は更新済**〉) ②エディタ再構成(ChordPatternEditorを2ゾーン＝「いつ弾く」grid主役＋長さ＋小節／「響き」＝打ち方・トップ狙い・広がり・高さ・パワーコード・arp時は向き を1枠に集約) ③トップ狙い(top)をステッパーで配線・プレビューは常にtop込み。青の壁は解体(二択セグメント/トグルはコード色/±無彩色)。音の長さは既存どおり各hitのdur(長さツール+付点)で保持。候補プレビュー化は後段。
  - **改訂：右手の積み方＝案A（密集）＋案B（右手ルート省略）（2026-09-16 オーナー耳裁定＝案A＋B・既定の出音を意図して変える）**：真因＝`docs/research/2026-09-16-organ-voicing-muddiness-rootcause.md`（旧 `voiceToTop` は残りの構成音を `QUALITY_INTERVALS` の表の順に「現在の最低音の直下」へ積んだため、表の最後の 7th/テンションが常に右手最低＝幅約22半音・低音域の音程の限界違反＝オルガンで濁る）。試聴（オルガン5型・ピアノ2型の7型すべて「案A＋B」）で裁定。
    - **案A**：トップ音の選び方は従来どおり（top 最寄りの構成音・同距離は先勝ち）。残りの各音を**トップより下で最も近い位置（トップから1オクターブ以内）**に置き、昇順に並べる＝密集形。open は従来どおり並べた後に1つおき+12。
    - **案B**：`resolveChordPattern` で content.lh があり、左手が R を弾く（mode が root/root5/oct、または custom hits に度数 R〈deg 省略＝R〉を含む）とき、**4音以上のコード（パワーコード除く）に限り右手からルートの pc を抜き、抜いてからトップ音を選ぶ**。三和音は2音になるのを避けて対象外。左手の無い型（OG-STAB/OG-PUNCH 等）・プレビュー（`voicingPreviewPitches`）は案B 非対象＝案A のみ。
    - **影響**：`voiceToTop` を通る全経路（鍵盤の型すべて・手作りネタの top 付き voicing・arp・followChords の再ボイシング）。三和音の出音は案A でも不変（真因文書 0/12）。ギター（`voiceGuitar`）・top 無しの旧 tones 経路は不変。
    - **受け入れ**：試聴に使った試作（listen6 `voicing.mjs` の mode "AB"）と、キー C・Fmaj7|G|Em7|Am7→G ×2・96bpm・各型既定で音高列が一致（web テストの固定データ）。
    - **範囲外**：左手の無い型で 7th が最低音＝分数コードに聞こえる件（案D）は未裁定。
...
- **決定：Fable UX監査の承認済み修正（2026-07-22・オーナー裁定・語彙triage＝`docs/research/2026-07-22-ui-vocabulary-triage.md`）**：奏法UIスライスA/B（上記 439〜465）の実機所見をオーナーが裁定。全て web の表示層のみ＝**データ値・実音化（`resolveChordPattern`/`genChordPattern` 出力）は1bitも変えない**。
  - **① 奏法の変更手段は一本化（案イ）**：`voicing.style` を書く手段は **MetaPanel「奏法」select（スライスA）だけ**に残す。`ChordPatternEditor` 響きゾーンの「奏法」seg（style-auto/keyboard/guitar ボタン）は**読み取り専用サマリへ格下げ**＝auto 解決結果を文言表示（例「いまの奏法：ギター（音色から）」／auto→keyboard は「鍵盤（音色から）」／明示 guitar/keyboard は「ギター」「鍵盤」）＝タップ不可。**上の 445/464 の「奏法 seg（おまかせ/鍵盤/ギター）」条項はこのサマリ表示に差し替え**（seg は編集子でなく表示子）。「じゃら〜ん」seg は響きゾーンに残す（唯一の微調整ノブ・ギター解決時のみ）。理由＝同一属性の編集入口が2つ（MetaPanel と CP エディタ）あると値の取り違え/責務の二重化を招く（Fable所見）。
  - **② 語彙統一「ストラム」→「ストローク」**：打ち方 seg の表示と MetaPanel 選択肢（「ストローク（じゃら〜ん）」）を一致させる。**データ値 `mode:"strum"` は不変**＝表示テキストだけの改名。奏法の語彙はストローク／アルペジオ／じゃら〜ん の3語に減らす（**「じゃら〜ん」→「ストロークの速さ」・MetaPanel は「ストローク」単独は下記 2026-07-22b 裁定**）。
  - **③ じゃら〜ん＝強弱でなく「速さ」（訂正）**：`strumMs` は弦をずらす**時間差**（ms/弦）であって音量ではない（上の 434 のオフセット式が正）。段ラベルを「OFF/弱/中/強」→**「OFF/速い/ふつう/ゆっくり」**に訂正（マッピング **0/8/14/25 ms は不変**＝ms が大きいほどロールが**遅い**＝ゆっくり）。上の 443 の「OFF/弱/中/強」表記も**「速さ（ロールの時間幅）」**として読み替える。MetaPanel 側「ストローク（じゃら〜ん）」表記は維持（**後続裁定で MetaPanel は「ストローク」単独・CP行ラベルは「ストロークの速さ」に改名＝下記 2026-07-22b**）。
  - **④ 選択色は青1色**：seg 選択状態は `--accent`（青）に統一。従来 `.seg.seg-chord button.on` が `--k-chord`（紫＝パーツ識別色）を選択色に流用していたのを是正＝**紫はパーツ識別色専用に戻す**（レーン/カード等）。D/U ストリップの `--k-chord`（D＝下）は選択でなく**D/U の意味色**（U＝warn 黄と対）なので据置。パワーコードのチェック風トグル（`.cp-chk.on`）の淡い k-chord も選択 seg ではないので据置。
  - **⑤ FAB 退避（最小工事＝非表示）**：チャットFAB（`chat-bubble`）がエディタで「左手」ボタンやプレビューを覆う（Fable E2E 06/09/10/31）。**ネタを開いている間（`active` が真＝NetaDialog/AnalysisWorkbench/StudyView）は FAB を非表示**＝既存の `!deskTarget && !shelfOpen && !filterDrawerOpen` と同じ「オーバーレイ中は隠す」条項に `!active` を1つ足すだけ。下部トランスポートへの退避は共有バーのレイアウト改修が要る＝非表示を選ぶ（Chat はトップ/一覧/プロジェクト画面から到達可）。
  - **⑥ 4択セグの折返し解消（360px）**：`.seg button` に `white-space:nowrap`。左手 seg のラベルを短縮＝OFF／ルート／**+5度**（旧「ルート＋5度」）／**8va**（旧「オクターブ」）。TinkerSheet「細かく（ドラム絡み）」群の左ラベル（`.knob-name`）は主文を nowrap（`<small>` 副文は従来どおり）。
  - **⑦ CTA 文言統一「候補を出す」**：候補トレイ行きの生成ボタンを「候補を出す」に統一＝TinkerSheet 各引き出しの「メロを生成／ドラムを生成／ベースを生成／骨格を生成」を改名（コード楽器は既に「候補を出す」）。**即時一式生成の NetaList「作例を生成」は候補トレイでない＝据置**。
  - **⑧ 「いつ弾く」行の表示条件＝バグではない（実コード裏取り）**：`ChordPatternEditor.tsx:154-212` の `cp-when`（いつ弾くグリッド）は**無条件レンダ**＝program/style/おまかせ に依存する表示ガードは存在しない。CSS も `.editor-body .rhythm-editor` の flex 伸長は flex 親（`.cp-when` は block）に効かず `grid-auto-rows:minmax(18px,1fr)` の自然高でグリッドは常に描画される。Fable E2E 31/32 の差は**表示条件バグでない**＝FAB 被り（⑤で対処）かスクロール位置/別 kind の見え方。**推測で直さない**＝コード変更なし・本裁定に理由を記録。
  - **語彙の一律再評価（オーナー指摘・Fable好評は判定根拠にしない）**：「キックに噛む」（lock with the kick の直訳）等を含む全 UI 語を同じ物差しで triage＝`docs/research/2026-07-22-ui-vocabulary-triage.md`。合否はオーナー裁定に委ね、今回のコード改名はオーナー既裁定（②③⑥⑦）に限定。
- **決定：UI用語の一般語化バッチ（2026-07-22b・オーナー裁定・正典＝`docs/research/2026-07-22-ui-terminology-audit.md`＋`2026-07-22-ui-vocabulary-triage.md`）**：初見で通じない造語/内輪語/直訳を一般語へ写す。**変えるのは表示文字列と aria-label のみ＝データ値・id・content契約・実音化は1bitも変えない**（rhythm part id=`backbeat`・kickLock 数値 −0.6・`mode:"strum"`・strumMs 段値・LENS_AXES id は全て不変＝bit一致は構造保証）。
  - **改名（UI表示）**：アフター→**裏打ち**（RHYTHM_PART_UI・id backbeat 不変）／期待理論→**自然な流れ**（LENS_AXES expectation・id 不変）／棚→**一覧**（TinkerSheet「← 一覧へ」）／潜る→**開く**（FormStrip「タップで中を開いて編集」）／逆相→**外す**（kickLock seg・値 −0.6 不変）／キックに噛む→**キックに合わせる**（kickLock ノブ名）／じゃら〜ん→**ストロークの速さ**（ChordPatternEditor 響き行ラベル）・MetaPanel「ストローク（じゃら〜ん）」→**「ストローク」単独**。
...
- **前景/背景の判別軸（2026-08-02 オーナー裁定・⚠未レビュー）**：上の統一原理でいう前景／背景を分けるのは「旋律か否か」ではなく**「その曲の主旋律か（歌があれば歌詞・仮歌が載る線か）」**。器楽ソロ／副次リードは旋律だが主旋律ではない＝**背景扱い＝パターンが content・per-note 編集レーンは持たない**（コンピング/ドラム同格・詰めは DAW）。判定は**"主旋律か"**で行い"歌詞の有無"では行わない＝詞先の空メロも主旋律＝前景、インスト曲の主旋律も歌詞無しで前景に残る。根拠：(1) 歌の主旋律ですらニュアンスは編集層でノート編集はピッチ/リズム止まり＝ソロだけ音符 craft を特別扱いする理由が弱い (2) 音符編集レーンを足すのはシーケンサー化＝「仕上げは人間」(#1765) に反する（オーナー：ソロに per-note 編集を付けるのはコンセプト違反に傾く／エスケープハッチも実測で必要が出るまで付けない）。実装するなら型辞書への押し込みでなく**専用 verb（gen_solo/lead 等）で"進行に解決する線の候補"**を出す＝`gen_section_inst` desc の「旋律的セクションライン＝スコープ外(後続)」の後継。正準の線引き＝requirements「達成度の線引き」の楽器アレンジ項。
...
5. **リフ雛形**＝和声4型（移調/固定/ペダル/微調整）×楽器イディオムの雛形棚を語彙帳の1カテゴリとして持つ＝「そもそも種が書けない」ゼロ起点を埋める。

**枝の裁定（7点・原文要旨）**：
- **枝1 器楽ソロ／副次リード＝初期スコープ外**（既裁定「ソロも背景」と整合＝方針変更ではなくスコープの後回し）。
- **枝2 惜しい候補の救済＝軸指定で再配札**（注文チップ）。再生成ガチャにしない。
- **枝3 試聴音源＝今のまま**（投資しない）。backlog の SpessaSynth/SF3 検討は**保留のまま**＝この計画の前提にしない。
- **枝4 採取の入口＝自作採取は不採用**。オーナー原文「手癖を超えたいのに私のライブラリを使うのは変。900POPS とかの実データ使いたい」（＝900POPS は POP909 の言い）。よって統合案にあった「**採取（小節単位）**」「**採用品の棚還元**」は**落とす**。辞書は〈一般奏法語彙〉＋〈他者実データからの統計〉で育てる。
  - **⚠著作権の硬い線（CLAUDE.md・コンセプト既定）＝統計のみ**。他者コーパスから**リテラルなパターン/旋律は保存しない**（移調しても不可）。「実データを使う」の中身＝**リズム分布・ボイシング傾向（声部間隔/転回の出方）・遷移統計**等、この線の内側で実現する。＝オーナーの言う「実データ」は**統計較正**として入る（進行役がオーナーへ復唱済み）。
- **枝5 進行変更時＝自動で当て直し**（採用済みパターンを無効化して選び直させない）。
- **枝6 手癖の鏡＝作らない**。原文「手癖の可視化とかいらんのでは？そも弾いたり打ち込んだりの口ないから手癖に縛られる意味がわからない」＝**手癖プロファイル／使用履歴メーター／遠い順ソートは全部落とす**。ただし**課題「似たニュアンスの別パターンが出せない」自体は生きている**＝解き口は〈配札〉＋〈軸指定の再配札〉＋〈実データ統計に由来する多様性〉であって、自分の癖の計測ではない。
- **枝7 セクション一括下敷き（全パート整合の一括提案）＝後段**。

**スライス順（確定版＝採取と鏡を落とした形。着手時は計画doc不要でそのまま Task 化できる粒度）**：
- **S1 写像＋試聴＋採用**（**ピアノ/オルガンから**）＝コード非依存テンプレ→曲のコード/キー/テンポへ写像→主旋律と一緒にループ試聴→パターン単位採用→MIDI書き出し。**分数コード・テンション・テンプレ1小節に対しコード2拍替わり・テンプレ長とセクション長の不一致（2小節型×7小節）**の処理をここで潰す＝**全案共通の要石**（崩れれば全部崩れる／座礁するなら最安値で座礁する）。
- **S2 配札＋軸指定の再配札**＝数枚配る→ピン/捨て→ピンを種に再配札、注文チップ（薄く/低く/開いて等）。ここで**枝2**を満たす。
- **S3 リフ雛形**＝和声4型×楽器イディオムの雛形棚（**枝の課題2**）。
- **横断＝統計較正**（枝4）＝他者コーパスの統計を S1 の写像規則・S2 の候補分布へ効かせる。単独スライスにせず各スライスの品質ノブとして入れる（統計のみ・リテラル不保存）。
...
**裁定（2026-08-02 夕・オーナー）＝ライブラリ入口の一本化**。根拠＝導線テスト実測（`docs/research/2026-08-02-arrange-s1-dousen-test.md`＝説明あり3/3・なし0/3、初見は全員「空きセル→ピッカー」へ向かい取込ダイアログを誰も発見できず）＋オーナー「最初期からあるレーンタップ選択に足すなら重複UIの意味を感じない」。
- **ライブラリの口＝レーン空きセル→ピッカー（PlacePicker）の一本**。`PatternImportControl`/`PatternImportDialog`・⤓アイコン・ゴーストCTAは**撤去**＝**Task1f〜Task1L（案C ゴーストCTA 含む・2026-08-02朝の裁定）の格下げ路線を上書きして廃止**。経緯は本節が持つ＝Task1f/1L 節は「本裁定で廃止」の追記のみ。
- **差し替え＝「外して置き直す」で賄う**（埋まったセルの選び直し口は作らない）。セクション編集のタップ操作系（空き=置く/埋まり=潜る）は**洗練されているので不変**（オーナー明言）。
- **文脈試聴▶はピッカーへ移植**＝空きセルから開いたピッカーの▶で「その候補をそのセル（レーン・位置）に置いた状態のセクション全体（主旋律込み）」をループ試聴。`contextAuditionPlan` は仮想子の**追加**モードを拡張（現行は差し替えのみ）。▶⇄■の再生状態表示＋「主旋律と一緒に試聴中」の明示を最小で付ける（導線テスト問題4）。`NetaDialog` の auditionCtx 配線（取込ダイアログ用）は撤去。
- **検索の是正**＝「オルガン」で5型全部当たるように（OG-SOUL/OG-PUNCH の表示名修正＋ピッカー絞り込みがネタ名に当たるように・導線テスト問題3）。要再seed。
- **実装の細部（2026-09-16・Task #5 着手時に実コード精査で確定）**：
  - **ピッカーにライブラリの型を並べる**＝従来 PlacePicker の一覧は `scope:"project"` だけ（型の棚＝`scope:"library"` は取込ダイアログからしか見えなかった）。一本化に伴い、**レーンが chord_pattern / bass / rhythm のとき**に同 kind の `scope:"library"` を別群「ライブラリの型」として自作一覧の下に並べる（拍子一致・検索の絞りは自作と同じ規則・器(プロジェクト)の絞りは効かない＝棚に器が無い）。置く＝既存 `placeAt` の library→copy→配置の経路そのまま。melody/chord_progression の「おすすめ（コーパス）」は不変。**検索はネタ名(title)に当たる**（既存の実装どおり）＝入力欄の案内を「名前で絞り込み」に改める。
  - **文脈試聴＝仮想子の「追加」モードに置き換え**：`contextAuditionPlan({section, children}, {neta, position, ord})`。差し替えモード（`childNetaId`）は唯一の呼び手（NetaDialog 配線）の撤去で死ぬので**消す**（二重に持たない）。候補はレーンミュートの外で必ず鳴らす（押した意図）。SectionEditor のピッカーだけが文脈を渡す＝**FormStrip（曲に section を置くピッカー）は従来のワンショット**。▶を押すと当該行が■（もう一度で停止）＋ピッカー内に「主旋律と一緒に試聴中」。
  - **生成結果は1ビットも変えない**＝UI 導線と seed の表示名（OG-SOUL/OG-PUNCH の `scenes` に「オルガン」を足す）だけ。`scenes` は `gen_chord_pattern` 型一覧の `label` にも出る＝文言だけ変わり content は不変。
- **オーナーの残不満（S2の動機として記録）**＝「棚が静的では対応幅が狭い・パターンも動的に出したい・これなら自分で大量に用意するのと同じ」。→ 応答は S2（配札＋軸指定再配札＋統計較正＝機械変奏と統計由来の多様性で"手書きでは出ない広がり"）。「最適を機械が当てる」はランキングしない裁定によりスコープ外のまま。
...
### Task1c＝コード楽器エディタを「両手一体グリッド」に作り直す（2026-07-23・オーナーFB「左手を下・線で区切って上に右手コードのリズム・一貫性がない」・裁定「案A＝一枚グリッド」・正典＝Fable×2 UI批評 ＋ 手本 `BassStepEditor.tsx`）
右手リズムグリッド（`いつ弾く`）と左手パッドは両方存在するが、間に「響き」7ノブが挟まり**分断・ベースの一枚グリッドと一貫性が無い**（オーナー不満の芯）。**content 契約・レンダ（`resolveChordPattern`/`resolveLh`）は一切変えず、エディタの見せ方だけ**を「両手一体の一枚グリッド」へ作り直す（既存ネタは鳴りも content も bit 一致）。
- **並び順をベースに統一**：`パターンを選ぶ▶ → 小節[−+] → 長さ(分) → 両手グリッド`（現行の「長さ/小節がグリッド下」を上へ・ベース `BassStepEditor` と同順）。
- **両手一体グリッド（一枚・同一小節ルーラー）**：単一グリッド容器に上下2帯。
  - **上帯＝右手（コードを打つリズム・1レーン）**。ラベル「右手」。セル＝和音ヒットを「いつ」打つか（現行 `いつ弾く/コード` の hits をそのまま）。**度数を1音ずつ置くレーンにはしない**（2026-07-04裁定「トップ狙い音の抽象・シーケンサー化しない」を死守）。vel は現行の縦ドラッグ（`fireChord`）・guitar 解決時は D/U を各ヒットに。
  - **破線の区切り線**（`BassStepEditor` の `他` レーン上ボーダーと同じ資産を流用）。
  - **下帯＝左手（土台の度数・R/3/5/8）**。Task1b の常時パッドを**この一枚グリッド内の下帯へ移設**。直接ペイント（ベースと同じ作法）。注入ボタン `[ルート][+5度][8va][クリア]` は**小さな「種」チップ**として左手帯のラベル脇/上に寄せ、グリッドを主役に（現行の大きめ seg 幅を縮める）。
  - **同一ステップ列を共有**＝上下が縦に揃い「両拍で両手が落ちる」が見える＝両手一体の実体。
- **「響き」を折りたたみへ降格＋現在値サマリ**：`▶ 響き  アルペジオ·top E6·close·↑↓·1oct`（`▶設定` と同じ disclosure 作法・既定は閉）。中身＝打ち方/トップ/広がり/向き/駆け上がり幅/区切り/高さ。**CP行契約「響きゾーン最大5行」はこの折りたたみ化で解消**（前面から退避）。
- **arp はレーンを増やさない**：右手は常に1レーン。arp は各ヒットに**斜め階段グリフ＋向き記号（↑/↓/↑↓）**で質を出す（ストローク＝無地ブロック）。実音（トップ含む）はグリッド直下の**概形ロール**で目で確認（当初は汎用 `MiniRoll`（`chord-roll`）を流用したが Task1k で作り直し＝下記）。
- **セル/一貫性の共通言語**：置いた音の見た目を**ベース `.step-cell` 系に寄せる**（塗り＋ラベル＋拍線 `beat`/小節線 `bar`/`sustain`・vel は濃淡）。ラベル列幅・セル幅もベースへ寄せて列頭を揃える。長さツール(`NoteValuePicker`)・パターン帯(`PatternPickerBar`)は既に共有＝維持。
- **細部の是正**（批評指摘）：`奏法「いまの奏法：鍵盤」` 読み取り専用テキスト→小バッジ格下げ／`区切り` ドロップダウン→セグメント（他ノブと統一）／空左手グリッドのゴースト/ヒント（"タップで土台"）。
- **guitar 解決時**：左手帯は非表示（S3 条項）＝上帯（右手＋D/U）のみの一枚。
- **bit 一致**：`ChordPatternContent`（hits/voicing/lh/patternId）・`resolveChordPattern`/`resolveLh` は**未変更**＝既存ネタは鳴りも content も deepStrictEqual 一致。変更は `ChordPatternEditor` の JSX 構造・CSS・折りたたみ state のみ。
- **TDD**：(a) DOM 縦順が `パターン帯→小節→長さ→グリッド`（ベース順）。(b) 右手レーンと左手レーンが**単一グリッド容器**内・破線区切りで隣接・同ステップ数。(c) 「響き」既定折りたたみ・サマリに現在値・展開で7ノブ。(d) 注入チップ/区切りセグメント/奏法バッジの存在。(e) guitar 解決で左手帯非表示・右手のみ。(f) 回帰＝content 出力（hits/voicing/lh）は編集操作で従来と同一（resolve の deepStrictEqual 群が緑）。

### Task1d＝右手モードツール（描く/消す）＋左手土台ボタン撤去（2026-07-23・オーナーFB「土台ボタンいらない・消して1行詰め」「arpのとき鉛筆＝何をするかボタンが要りそう」・洗い出し正典＝Fable use-case 監査）
**Task1c の「注入チップを残す」条項（上記 610/623）を撤回**（オーナーFB①＝撤去）。加えて右手セルの操作オーバーロード（頭=消す/伸び=長さ/空=配置）が**明示ボタンなしで不可視**＝arp で特に「何をするか」が読めない（FB②）を解消。**content 契約・レンダは不変**（編集モードは UI state・OFF の bit は下記で安全化）。
- **左手の土台ボタン＋「左手」ラベルを撤去し1行詰め**（FB①）：keyboard 解決時の左手ブロックは **パッド（R/3/5/8×step）＋破線区切りのみ**。度数レーンが自己ラベル＋破線で右手と隔たる＝「左手」明示は無くても識別可。ベースと同じ**直接ペイント**に統一。`[ルート][+5度][8va]` の一括注入と `[クリア]` は撤去（preset 既定は各小節頭 R を materialize 表示済＝種は最初から見える）。
- **左手 OFF の bit 安全化（撤去の代替・必須）**：`toggleLhPad` で**最後の hit を消して hits が空になったら `lh` キーごと削除**（従来 `clearLh` 経路）。従来は空でも `lh:{mode:"custom",hits:[]}` が残り「左手なし（lh 未定義）」と bit が違った＝[クリア] 撤去後の OFF を bit 一致にする（Task1b「空パッド＝左手なしと等価」の実装的裏取り）。
- **右手モードツールバー（描く/消す）**（FB②）：右手グリッド上に mode seg を追加（メロの `proll-modes`＝`KindEditorBody.tsx` を流用）。**描く**＝配置（＋伸びタップで長さ調整・長押し縦ドラッグで vel＝現行 `toggleHit`/`fireChord` を描くモード内に温存）／**消す**＝タップで削除（頭タップ削除の不可視を解消）。**「選ぶ」は足さない**（vel/長さは描く内で足り、per-hit の複数選択編集は 2026-07-04 裁定「シーケンサー化しない」に抵触）。**常時表示**（ストロークでも頭タップ削除は不可視ゆえ・メロと一貫）。arp の向き/幅/区切りは**大域のまま響き（折りたたみ）に据置**＝per-hit 化しない（裁定死守）。
- **軽量ヒント**（任意）：右手に「描く＝置く・伸ばして長さ・長押しで強弱／消す＝タップで削除」の1行（guitar D/U の `cp-hint` に倣う）。
- **bit 一致**：content 契約・`resolveChordPattern`/`resolveLh`/`emptyChordPattern` 不変。編集モードは UI state。左手 OFF の空→lh 削除は「左手なし」を正典化＝既存 preset/未定義ネタは不変・deepStrictEqual 群は緑。
- **TDD**：(a) 左手土台ボタン（ルート/+5度/8va/クリア）と「左手」ラベルが**非存在**・パッドは在る。(b) パッド全消し→`lh` キー削除（bit＝lh 未定義一致）。(c) 右手 mode seg（描く/消す）存在・消すモードでタップ削除・描くモードで配置＋長さ＋vel 温存・**選ぶは無い**。(d) 右手は1レーン維持・arp グリフ不変・向きは per-hit 化しない。(e) 回帰＝resolve 出力の deepStrictEqual 群が緑。

### Task1e＝ステップグリッドをモバイルで「1小節一望」に圧縮（2026-07-23・オーナー裁定・実機監査でモバイル360pxのグリッド右端3step見切れを検出）
Fable 実機監査＝360px幅で16step グリッドの step13-15 が画面外・横スクロール手掛かり無し（ベースも同じ既存挙動）。オーナー裁定＝**セル圧縮で1小節を一望**。**content・レンダ不変＝純レイアウト**（見た目のみ・bit一致）。
- **共通寸法を圧縮（chord 両手グリッド／bass／drum 横断で統一）**：セル幅 20→**16px**・セル間 gap 2→**1px**（1step=17px）・ラベル列幅 44/40→**36px**・プレイヘッド `BEAT_PX` 88→**68**（=4step×17px）。16step＋ラベル=16×17+36=308px＝360px に収まる（横スクロールなし）。
...
### Task1f＝「パターンを選ぶ」帯の格下げ＝ライブラリから読み込む二次アクション（2026-07-23・オーナー裁定・正典＝`docs/archive/2026-07-23-pattern-picker-demotion-plan.md` 案A）

> **⚠ 廃止（2026-08-02 夕のオーナー裁定＝ライブラリ入口の一本化・2026-09-16 撤去実装）**：本節の入口（取込ダイアログ・⤓アイコン・ゴーストCTA）は撤去済み。ライブラリの口はレーン空きセル→ピッカーの一本（正準＝「アレンジS1＝写像規則の契約」節末尾の裁定）。以下は経緯として残す。
...
- **文言**＝「パターンを選ぶ」→**「ライブラリから読み込む」**（一般語・メモリ「造語でなく一般的な語で」）。適用ボタンは「適用」→「読み込む」に寄せる（入口と一貫）。
- ~~**案C（空グリッドのゴーストCTA）は今回見送り**~~ → **2026-08-02 のオーナー裁定「A2 おすすめ」で撤回＝案C は実装した（下方 Task1L）**。**案B（⋯メニュー沈め）は共通メニュー器が要るため過剰＝将来の一貫性アークへ**（引き続き不採用）。
- **⚠ この節の入口の姿は Task1g/1j で上書き済み**＝文言リンク「⤓ ライブラリから読み込む」ではなく **`Icon name="import"` 単体のボタン**（オーナーFB「アイコンだけで」・`c2923b6`/`018aa90`）。器も `PatternPickerBar` から `PatternImportControl` へ。
- **Section 側（`useMelodyGen`）は文言統一のみ**（既に「いじる▾」内トレイ＝二次・格の移動不要）。compound/管弦ゲートの非表示条件は不変。
- **bit 一致/回帰**：`PatternCand` 契約・候補取得(L3 `fetchPatterns`)・試聴/適用・非表示条件は不変＝差分は見出し variant＋配置＋文言のみ。`variant="bar"` 経路で既存挙動の退避テストを残す。
- **TDD**：(a) 3エディタが `variant="link"`・小リンク文言「ライブラリから読み込む」・設定行配置。(b) 開くと従来 body（chip/候補/▶/読み込む）。(c) 適用＝従来の content 置換（apply 不変）。(d) `variant="bar"` は現行 DOM と一致（退避）。(e) 非表示条件（ベース6/8・showPicker=false）不変。

### Task1g＝パターン取得を「ネタ選択ダイアログでライブラリをブラウズ」へ作り直す（2026-07-23・オーナーFB「ジャンルchip→固定4件がつらい・既存ライブラリ全体から取りたい・共通ダイアログを使いたい」・正典＝Fable 調査「ネタ選択ダイアログ流用の実装地図」）
**この節は Task2/L3（下方「ピッカーをライブラリ検索へ差し替え」）の「`PatternCand` 契約不変・genre タグで最大N＝固定4件」機構と、Task1f の body（chip→「候補を出す」→4カード）を撤回する。** 両手グリッドで直接入力が主役になったので、パターン取得は「本当にライブラリから丸ごと持ってくる時だけ」＝**既存のネタ選択ダイアログ（`PlacePicker`）でライブラリ全体を検索/ブラウズして1件選ぶ**形に上流から作り直す。
...
- **撤去**：`PatternPickerBar` の chip→固定4件カード body と `patternLibrary.ts` の `fetchLibraryPatternNetas`（genre→最大4件）・`PATTERN_CAND_MAX=4`。`netaToPatternCand`/`sceneTagOf` は表示（名・場面タグ）に流用余地あり。
- **据え置き（今回スコープ外）**：`useMelodyGen`（Section メロ生成道具のパターン参照）＝別文脈で据え置き。`section_inst` は `showPicker=false` のまま。生成器 API は第二経路で温存。
- **未決（オーナー任意・既定で進める）**：①自作(project)パターンも母集団に混ぜる＝**混ぜる**（「ライブラリ全体」）②意味検索＝**今回見送り**。要変更なら後日。
- **bit一致/回帰**：place モード（SectionEditor/FormStrip）は無改修で緑。apply/試聴/非表示条件（6/8・showPicker）不変。差分は候補の見せ方（固定4件→ライブラリ全ブラウズ）＋`onPick` 返し口新設のみ。
...
### Task1i＝読み込みダイアログに Source（プロジェクト軸）絞り（2026-07-23・オーナーFB「プロジェクト所属/非プロジェクト/全部を選びたい」・モック実測で案C＝コンパクト1行採用）
`PatternImportDialog` は今 `scope:"all"` で工場出荷＋自作を無差別に出す。**プロジェクト軸の絞り**を足す（オーナー）。360px 実測（モック）＝素直に3 select は末尾が切れる→**案C＝短縮ラベルで1行**採用。
- **ソース select（案C・コンパクト）**：フィルタ行に3つ目。既定 option ラベル＝**「ソース」**（カタカナ＝UIの日本語に統一・オーナー裁定／value ""＝全部・未絞り）／`ライブラリ`（`scope:"library"`＝工場出荷・非プロジェクト）／`このプロジェクト`（`scope:"project"`＋`prj:<activeProject>`＝自プロジェクト所属）。**ジャンル/場面の既定ラベルも「ジャンル」「場面」に短縮**（「：すべて」を落とす）＝3つが1行に収まる（実機実測で切れ確認）。
- **配線**：`activeProject`（`App.tsx` グローバル）を **NetaDialog→KindEditorBody→3エディタ→PatternImportDialog** へ**追加 optional prop**（`activeProject?`）で下ろす（純追加＝既存経路 bit 一致）。`activeProject` 空/未配線なら「このプロジェクト」option は非表示 or「自作（scope:project 全部）」にフォールバック。
- **絞りロジック**：""＝全 netas／`library`＝`scope==="library"`／`project`＝`scope==="project"`＋`prj:` に activeProject を含む。genre/scene 絞りと AND。
- **範囲＝pick ダイアログのみ**・純追加（content/apply/試聴/onPick 無変更＝bit一致）。place/一覧不変。3エディタ（chord/bass/drum）共通。
- **TDD**：(a) Source select が3値・案C で1行（幅計算 or DOM）。(b) library＝scope:library のみ・このプロジェクト＝scope:project＋prj一致のみ・全部＝両方。(c) activeProject 無し時のフォールバック。(d) genre/scene と AND。(e) apply/place 不変。

### Task1j＝パターン取込の共通化＋絞りの日本語/データ駆動＋ライブラリアイコンボタン（2026-07-23・オーナーFB「コード楽器のみになってる・他の楽器も直して・絞り込みの仕方直して・全パターン個別対応にならないよう共通化・ライブラリから読み込むはライブラリっぽいアイコンでボタン化」）
現状3エディタ（chord/bass/rhythm）が **importOpen state＋入口リンク（`PatternPickerBar`）＋`PatternImportDialog` の定型配線＋audition/apply** を各自持つ＝**構造的重複**（楽器追加＝個別対応）。かつ `showScene` がコード固定＝**ベース（`scene:`タグ有り）で場面絞りが出ない**＝「コード楽器のみ」の芯。genre/scene 絞りは英語タグ値の生表示。3要望を**1つの共通化リファクタ**で解く。**apply/audition/母集団取得/onPick/place 経路/kind別content は不変＝bit一致**（構造リファクタ＝挙動不変）。
- **共通化＝`PatternImportControl` コンポーネント新設**：**入口ボタン**（`Icon name="library"`＋「ライブラリから読み込む」・ボタン化）＋`importOpen` state＋`PatternImportDialog`（`onClose`/`onPick→onApply&close`/`onPreview→onAudition` の定型配線）を1つに集約。props＝`kind`/`fallbackName`/`onApply(content)`/`onAudition(content)`/`nowLabel?`/`activeProject?`/`contentFilter?`。**3エディタは各自の `applyPattern`/`auditionPattern`（editor固有＝content形/再生文脈）を `onApply`/`onAudition` に渡すだけ**＝importOpen/入口/dialog定型の重複を排除・楽器追加が容易。`PatternPickerBar` は Control に吸収（入口ボタンは Control が描画）。
- **絞りの日本語化＋データ駆動 scene**：`PatternImportDialog` の genre option を **`genreLabel(g)`**、scene option を **`sceneLabel(s)`**（role→日本語 map・`genres.ts` に新設＝intro/verse/prechorus/chorus/bridge/interlude/outro→イントロ/Aメロ/…）に（英語値の生表示をやめる）。**scene 絞り＋カードの場面タグは母集団に `scene:` タグがあれば自動表示**（`showScene` ハードコード撤去）＝**bass(scene有)でも出る・drum(scene無)では出ない**＝データ駆動で全楽器統一（「他の楽器も直して」の実体）。
- **入口＝アイコン単体のボタン（オーナーFB「アイコンだけで」「libraryはコピー的→インポートっぽく」）**：`⤓` テキストを廃し **`Icon name="import"`（新規＝トレイに下向き矢印＝万国共通の取込記号・library=重なった矩形はコピー的なので別に新設）単体**の正方ボタン（`.pp-icon-btn`）。文言「ライブラリから読み込む」は `title`/aria へ退避。「いま：<型>」はボタン外の小テキストで維持。
- **bit一致/回帰**：`PatternImportDialog` の母集団取得（`scope:"all"`）・`onPick`/`onPreview` の呼び口・apply（content コピー）・audition（`notesForContent`）・place 経路（PlacePicker）は不変。共通化は各エディタの importOpen/link/dialog を Control へ移すだけ＝レンダ結果・content 出力は同一。
- **TDD**：(a) 3エディタが `PatternImportControl` 経由で入口ボタン（library アイコン）を出す。(b) scene 絞り＋カード場面タグは母集団に `scene:` タグ有りで**自動表示**（bass で出る・drum で出ない・chord で出る）。(c) genre/scene option が日本語ラベル（`genreLabel`/`sceneLabel`）。(d) onApply/onAudition が従来の apply/audition と同一呼び出し＝bit一致。(e) place 経路不変。

### Task1k＝概形ロールをパッド整列の下段ピアノロールへ作り直す（2026-07-24・オーナーFB「左手を付けたら概形表示がパッドと横幅一致せず違和感・高さも読めずピアノロールに見えない」・必要性の裁定＝オーナー「揃えて作り直す」）
両手グリッド直下の `MiniRoll`（`.chord-roll` aria=`voicing-roll`）は**カード用サムネ部品の流用**で、`width:100%`＋`preserveAspectRatio="none"` で**エディタ幅いっぱいに引き伸ばす**＝固定列(17px/step)・横スクロールの両手グリッドと**時間軸が原理的に一致しない**（真因）。加えて音程軸は lo..hi を箱高いっぱいに正規化した 3px 棒で**絶対の高さ目盛が無い**＝ピアノロールに見えない。さらにエディタ内は**進行未指定**で解決するため全打点が**固定Cメジャー**＝和声の動きも映らず情報が薄い。→ **グリッドの列ジオメトリ・横スクロールを共有する「下段ピアノロール」に作り直す**。必要性は「揃えて作り直す」で確定（撤去でなく整列）。**content 契約・`resolveChordPattern`/`resolveLh` は不変＝純表示の作り直し（bit一致）**。
- **列整列（真因の解消）**：概形は `.cp-grid`（`scrollerRef` の横スクロール容器）**内**の1帯（`.cp-roll-lane`）にする＝上のパッドと**同一スクロール・同一列ピッチ(1step=17px＝セル16px+gap1px・`STEP_PX`)**。ノートの x は `step = start/BASS_STEP_TO_BEAT` から `x=step*17`・幅 `w=stepDur*17−1`＝各ステップ列の真下に厳密整列。ラベル列は既存 `.cp-hand-label`(36px sticky) を流用。
- **本物のピアノロール見え＝音程の高さ＋尺**：ノートは音程で縦配置（高音=上）。左ラベル列に**最上音/最下音の音名**（`pitchName(hi)`/`pitchName(lo)`）を上下に出して高さの尺を示す（「高さが読めない」の解消）。帯高は固定（`ROLL_H`）＝レイアウト揺れ無し。
- **中身を型試聴と一致**：概形は**型試聴（`auditionPattern`）と同じプレビュー進行**（ネタ `preview_chords` があればそれ／無ければ `C→Am→F→G` を key へ移調）に当てて `notesForContent("chord_pattern", …)` で実音化＝**見た目＝聞こえ**（固定Cから改善・和声が動く）。`resolveChordPattern` は `content.lh` を keyboard 解決時に含める＝**右手ボイシング＋左手土台の両手が1枚のロールに出る**（左手追加の答え合わせ）。弦順ロール(guitar strumMs)は tempo 未指定で off＝ノートが step 境界に乗る（整列を崩さない）。
- **純関数 `voicingRollRects(notes, steps)`**（TDD 対象）：解決ノート(拍単位)→ `{ rects:{x,w,y,pitch}[], lo, hi, stripW }`。x=step 整列・y=音程（高音ほど小 y）・単音/同高は中央寄せ(frac=0.5)。列ジオメトリの SSOT。
- **撤去**：旧 `MiniRoll`／`previewNeta`／`.chord-roll` CSS（`assets-bass.css`）は本タスクで用済み＝削除（`MiniRoll` はライブラリダイアログ等で継続利用＝コンポーネント自体は残す・本エディタからの import のみ外す）。
- **bit 一致**：`ChordPatternContent`・`resolveChordPattern`/`resolveLh` 未変更。変更は `ChordPatternEditor` の JSX/CSS＋新純関数のみ＝既存ネタは鳴りも content も deepStrictEqual 一致。
- **TDD**：(a) `voicingRollRects`＝step 整列（start=1拍→x=68・dur=1拍→w=67）・音程順（高音ほど小 y）・stripW=steps*17−1。(b) エディタに `voicing-roll` 帯が在り `.cp-roll-strip` を含む・**旧 `mini-preview`(MiniRoll) は非存在**。(c) 右手 hits 空でも左手 lh があれば概形が出る（両手のうち片手だけでも描く）。(d) 回帰＝resolve 出力の deepStrictEqual 群が緑（content 不変）。

### Task1L＝空グリッドのゴーストCTA（案C）＝格下げアークの締め（2026-08-02・オーナー裁定「A2 おすすめ」＝計画 `docs/archive/2026-07-23-pattern-picker-demotion-plan.md` の推奨＝案A＋案C併用）

> **⚠ 廃止（2026-08-02 夕のオーナー裁定＝ライブラリ入口の一本化・2026-09-16 撤去実装）**：本節の入口（取込ダイアログ・⤓アイコン・ゴーストCTA）は撤去済み。ライブラリの口はレーン空きセル→ピッカーの一本（正準＝「アレンジS1＝写像規則の契約」節末尾の裁定）。以下は経緯として残す。
**裁定と実態の突き合わせ（重要・計画docは腐っていた）**：オーナー裁定は「案A（設定行の二次リンク化）＋案C（空グリッドのゴーストCTA）併用」だが、
**案A相当は 2026-07-23〜25 に既に実装済みで、しかも計画より強い格下げになっている**＝Task1f（帯→設定行の二次リンク）→ Task1g/1j（共通化）→
`c2923b6`「取込入口をライブラリアイコン**単体**のボタンに（オーナーFB『アイコンだけで』）」→`018aa90`（記号を import アイコンへ）。
計画 doc（2026-07-23 起草）はこの実装より古く、`PatternPickerBar`／文言リンク「⤓ ライブラリから読み込む」を前提に書かれている＝**その部分は既に無効**。
- **したがって文言リンクへは戻さない**（オーナーFB「アイコンだけで」への逆行になる）。設定行の入口は現行どおり `Icon name="import"` 単体ボタン（文言は `title`/aria へ退避）＝`PatternImportControl` の既定 `variant="icon"`。モック実測でも 360px 幅では文言リンク版は設定行が2行に折り返す＝アイコン単体が正解（補強材料）。
- **今回の裁定は「案C の追加」として消化**＝格下げアークで唯一未実装だった空グリッドの誘導だけを足す。Task1f の「案C は今回見送り」は本節で**撤回**（2026-08-02 裁定が上書き）。
- **器＝`PatternImportControl` に `variant?: "icon" | "ghost"`**（既定 `"icon"`＝現行 DOM と bit 一致の退避経路）。**body（`PatternImportDialog`・`onPick`/`onPreview`/`onClose` の配線）は両 variant で共有**＝候補取得（`listNeta({kind,scope:"all"})`）・適用（content コピー）・試聴（`notesForContent`）には一切触れない＝**bit 一致は構造保証**。
- **ゴーストの姿**＝グリッドが空のときだけ出す破線＋淡色のブロック（`.pattern-ghost`）。CTA「⤓ ライブラリから読み込む」＋副文「またはタップして自分で置く」。1つでも置けば消える＝作業中は面積ゼロ。**設定行のアイコン入口は常設のまま**（案A＋C 併用＝後から別パターンへ差し替える導線が消えない）。
- **aria は別名（`pattern-ghost` / `pattern-ghost-cta`）**＝設定行の入口が名乗る `pattern-picker` / `pattern-picker-toggle` と**同時に DOM に居る**ため、既存テスト/E2E が握る一意性を壊さない（計画 §6 の地雷への対処）。
- **置き場＝グリッド容器の「直下の兄弟」**（進行役裁量・最小/可逆）。理由：`.cp-grid`/`.bass-grid` は**横スクロール容器**ゆえ内側に absolute オーバーレイを置くとスクロールで流れる／外側にラッパ div を足すと `chat.css` の `.editor-body .bass-step .bass-grid` が前提にする flex 子構造（モバイル高さ）を崩す。直下ブロックなら 390px でも折返し/はみ出しが起きず、撤去も1ブロック削るだけ。
- **展開の形＝既存の pick ダイアログを再利用**（計画 未決④の決着・裁量）＝ポップオーバー化しない＝**ビューポートはみ出しクランプの流用は不要**（地雷を踏まない）。
- **空の定義（kind ごと）**：コード楽器＝右手 `hits` が空 **かつ** 左手が鳴らない（`lh` 未定義 or `lh.mode==="custom"` で `hits` 空）＝preset（root/root5/oct）は hit 無しでも実音化されるので「空」ではない／ベース＝`pattern.length===0`／ドラム＝全レーンの `hits` が空。
- **非表示条件を継承**（不変）：コード楽器 `showPicker=false`（管弦=section_inst）・ベース compound meter（6/8系）では**ゴーストも出さない**＝「入口を出さない場面」は格が変わっても不変。
- **Section 側（`useMelodyGen`）は据え置き**（計画 未決⑥の決着・裁量＝計画推奨どおり最小）。文言は Task1j 時点で既に「ライブラリから読み込む」語彙に統一済＝追加作業なし。**案B（⋯メニュー沈め）は引き続き不採用**（共通メニュー器が要る＝過剰）。
- **TDD**：(a) `variant="ghost"` が別 aria で文言つきCTA＋副文を出す・`pattern-picker*` を名乗らない。(b) ゴースト押下＝同じ取込ダイアログ→`onPick`→`onApply(content)` 素通し（従来 apply と同一）。(c) 既定 variant はゴーストを出さない（退避）。(d) 3エディタ＝空でだけ出る/1つ置けば消える/`showPicker=false`・6/8 では出ない。
...
- **grid の一級化**：`CompType.grid`/`BassType.grid` を `16 | 12` に。`parseCompRh`/`parseCompLh`/`parseBassPattern` は 12セルも受理（`T` ファクトリが**セル数で grid 判定**＝16 or 12・それ以外は throw・LH は RH と同 grid）。drum は既に `grid:16|12`（six8.ballad が先例）。
- **world68 型（genre:`world68`）**：chord10（`w68-ch-drone5`/`pad`/`arp6`/`thumb`/`strum-all`/`strum-dud`/`strum-jig`/`strum-sync`/`pm`/`roll12`）＝keyboard（LH/RH分業 or arp）／guitar（strum-*・pm はパームミュートを `x` チャックで近似＝**GMにミュート音無し＝要耳較正**）。bass8（`w68-bs-anchor`/`drone`/`r5`/`r56`/`approach`/`oct`/`hemi`/`jig`・度数は R/5/6/8 中心＝3度回避で旋法曖昧を維持・`approach`は末尾 `#7>`＝次小節頭ルートへの半音下先取り＝`next`）。drum8（`w68-dr-shaker`/`clap-beat`/`dumtek-a`/`dumtek-b`/`jigskel`/`tarantella`/`clap-hemiola`/`full`・meter:"6/8"・GM代用＝Dum:41/Tek:37/ka:42/Shaker:69/Tamb:54/Clap:39）。全型に roles（scene タグ SSOT）・tempo（付点4分 min-max）。
- **steps の meter 対応**：`genChordPattern`/`genBass` は `stepsPerBar = meterInfo(meter).beatsPerBar × 4`（4/4→16・6/8→12）＝既に meter 追従。実音化の step→時間は `BASS_STEP_TO_BEAT=0.25`（1step=16分）で**meter 非依存**（6/8=12step×0.25=3拍＝付点4分2大拍）＝タイミングの外科手術は不要。`realizeBassGrid`/相対 `cellsToSteps` は 16固定を撤廃し **grid=cells.length**（16 or 12）でスロット幅 `perBar/grid` を導出。`next`(R>/8>/#7>) の barStart 判定に渡す beatsPerBar は meter 由来（6/8=3）。
- **grid ガード（型格子を敷ける拍子の一致）**：`genChordPattern`（`canLib` を 6/8 も許可＋`compType.grid===stepsPerBar` を確認）・`genBass`（`styleGrid=compound?12:16` に一致する型のみ resolve）。**4/4型を6/8枠へ／6/8型を4/4枠へは敷けない＝従来経路（bit 一致）**。3/4 等の非4拍は従来どおり対象外。
- **seed 6/8 解禁**（`seed-pattern-library.ts`）：world68 型は grid で拍子判定（12→`meter:"6/8"`）して焼く。chord/bass の 4/4 固定・drum の非4/4除外を **world68（6/8）については通す**（six8.ballad 等の非 world68 6/8 は据え置きスキップ）。genre:world68＋scene:＋tempo:＋pat: タグ。冪等削除（scope:library+lib:factory）不変。
- **プレビュー配線（web）**：`notesForContent`→`resolveRelativeBass` に **beatsPerBar を meter 由来**で渡す（`BassContext.meter` 新設・未指定=4/4=bit一致）＝相対 bass の `next` が 6/8 で正しい次小節頭コードへ解決。MiniRoll（概形）は `neta.meter` を透過・BassStepEditor 試聴も meter を透過。タイミングは step×0.25 で自動対応。**chord パターン試聴は step→拍が meter 非依存＝steps:12 で自動**（配線不要）。※4/4前提のハードコードは `notesForContent` の resolveRelativeBass 既定 beatsPerBar=4 のみ（meter 透過で解消）。
- **UIラベル**：`genres.ts` GENRE_META に `world68:{ color:"terra", label:"民族調(6/8)" }`（base.css `--genre-terra` 追加・light/dark 両値）。
- **GENRE_TABLE 据え置き＝生成器選抜不変**（L5 領域）：world68 は chord/bass/drum いずれの GENRE_TABLE にも未登録＝`pickCompType`/`pickBassType`/`pickBeatPattern` は返さない。world68 の唯一の消費者は**型ID直指定（seed）**＝出音・選抜に無影響。
- **耳確認待ち**：6/8 は**全型が耳確認前提**（プローブ束）＝strum-* の strumMs・pm チャック近似・drum GM代用音色・`approach`の#7>着地・旋法色（scale差替）は採否待ち。
...
- **⚠ 訂正（2026-07-29・オーナー未レビュー）**：**worker の `split_mora` は実体が無い**。`apps/worker/src/cm_worker/` にあるのは `search.py`/`db.py`/`serve.py` だけで、ジョブ消化の worker は 2026-07-05 に撤去済み（ディレクトリは cm-search の住所として残っているだけ＝architecture 参照）。モーラ分割の実体は TS の2本（`apps/web/src/lyrics.ts:8`・`apps/api/src/lyric.ts:16` の `splitMora`）のみ。この2本と `analyzeMoras`（`packages/music-core/src/prosody.ts:46`）を **music-core 1本に寄せる**＝§31-4。

**~~決定L1：歌詞は{表記+読み}を行ごとに持つ~~**（ユーザー決定・**2026-07-29 破棄／オーナー未レビュー**）。漢字はモーラ数が一意でない→読み(かな)を音数の正準に、という向き自体は正しい。
...
- **⚠ 破棄の理由**：この形は**一度も実装されていない**（`lines` / 行ごとの `reading` を読み書きするコードはリポジトリに無い）。かつ「読みを人が手で書く」前提が、確定「歌詞の正データは漢字仮名交じりの普通の文章」と合わない（読みは機械が表記から導き、人は直すだけ）。**置き換え＝句 `LyricPhrase`（表記が正・読みは控え・人の直しは別に持つ）＝§31-1／読みの取得は §31-3。** 語単位の対応（表記↔音符）は §31-1 の `LyricReading.words` と §31-6 の直しで入る。

**決定L2：音数合わせ＝自動(多=分割/少=メリスマ)＋手動調整**（ユーザー決定）。`flowLyric(notes, moras, opts?)`：
- **⚠ 限定（2026-07-29・オーナー未レビュー）**：`flowLyric` は**音符を作り替える**（最長音符を半分に割る）。これは architecture「黙って実体を作らない」に触れるため、**人が「合わせる」を押したときだけ走る**ようにする＝詞を打っただけでは音符が1つも増えない。詞を書くときに使うのは、音符を変えずに読みを写す新関数 `placeMoras`（§31-2）。呼び出し元の限定＝§31-5。
...
- **後続**：**作業代行(arrange)**＝対象ネタ＋指示で単発作業→content（kind曖昧さの解消が要・consultの判別ユニオン流用候補）。**歌詞整合(align_lyrics)**＝歌詞の音節をメロのノート数に合わせる（design#13・syllable 表示UIとセット）。いずれも判断が要るため別スライス。※`align_lyrics` は**未実装のまま**。作るときは返りを #31 の言い分けの語彙（「字余りN」「あとN音」「メロが途中」「メロなし」「ちょうど」）に合わせ、**機械は詰めない・削らない**（§31-5・2026-07-29 追記・⚠オーナー未レビュー）。
...
> **⚠ 改訂（2026-07-29・#31 で置き換え／オーナー未レビュー）**：下の「データモデル決定（2026-07-15）」の**結論の部分だけ**が、コンセプトの確定「歌詞の正データは漢字仮名交じりの普通の文章」と正面から食い違う。**歌詞の本体は句（メロネタの `content.lyric.phrases`）で、`Note.syllable` は仮歌のための写し**へ改める（**§31-1**）。ただし**根拠のほうはそのまま生きる**＝「同じ音符列に別歌詞は成立しない」「同じ音・違う歌詞は別のメロディネタ（複製）」は #31 でも保たれる（句もメロの中に持ち、2番は複製で作る＝§31-1・スライス6）。歌詞流し込みの経路（下3項目）も #31 で作り替わる＝**読みは表記から pyopenjtalk で取る**（§31-3）・**音符を割るのは人が「合わせる」を押したときだけ**（§31-5）。

- **データモデル決定（2026-07-15・オーナー裁定）＝歌詞は独立エンティティにしない**。~~歌詞は `Note.syllable` としてメロに埋め込み（現行どおり）が正典。~~（→ 2026-07-29 改訂＝§31-1。「独立エンティティにしない＝ネタの中の層として持つ」は維持）根拠＝**譜割り一体論**：日本語は1音1モーラ原則ゆえ歌詞が変わればメリスマ分割等で音符列自体が変わる＝「同じ音符列に別歌詞」は成立しない。**同じ音・違う歌詞は別のメロディネタ**（copy_neta／分岐スタック realized_from の一種）として扱う。~~歌詞先行の下書きは knowledge/テキストネタで書き溜め、メロと出会った時点で流し込み＝埋め込みに変換。~~（→ 詞先の句は**案(い)＝人が合図して空のメロを置き、そこに書く、で確定**＝2026-07-30 オーナー裁定・§31-9）
...
- 歌詞流し込み：text→(漢字あれば読み)→モーラ列→melodyのnotesに左から1モーラ1ノートで`syllable`へ。音数チェックはフレーズ/小節単位でモーラ数とノート数を比較。
  - **⚠ 訂正（2026-07-29・オーナー未レビュー）**：「(漢字あれば読み)」の経路は**実装されなかった**。実体は `splitMora`（`apps/web/src/lyrics.ts:8`・`apps/api/src/lyric.ts:16`）だけで、これは**かな専用**＝漢字1字を1音と数える（実測「雨の日は」→4／正5）。表記からの読みは **pyopenjtalk（`apps/audio/accent.py`）から取る**＝§31-3・§31-4。
...
- **入力源**：~~本スライスは `syllable`（かな・モーラ片）とピッチ/オンセット列で完結。~~ かな→モーラは自作分割（#13の SMALL 拗音結合＋ー/っ/ん 独立＝既に api `lyric.ts` splitMora と同規約、prosody は各モーラを `{kana,kind,vowel}` に**分類**して返す）。
  - **⚠ 破棄（2026-07-29・実測により／オーナー未レビュー）**：「`syllable` で完結」は成り立たない。音符に載ったかなを連結して pyopenjtalk に渡す現行経路（`apps/api/src/accent.ts:76` `accentsFromSyllables`）は、実測30行で句の割れ方が **30/30行**変わり、高低が **139/284モーラ＝48.9% 反転**した。しかもモーラ総数は一致するため `mapAccents`（`accent.ts:59`）が null を返さず、**誤った高低が検出されないまま通る**。＝**入力源は表記（漢字仮名交じり）に改める**＝§31-3。分類（`{kana,kind,vowel}`）そのものは生きる。
...
- **WP-L1 配線**：(a) `genMelody` opts に **`phrases`（配列直渡し）** を追加＝runV2 で `opts.phrases` を最優先（未指定＝従来 skelPhrases/phrasing 派生＝bit一致）。rhythmParts は既存経路。(b) 新 `genLyricMelodyCandidates`（generate.ts）＝N=8生成→各候補 `flowLyric(notes, plan.syllables)`（音数一致で1:1）→ `analyzeLyricFit`（accents 供給）→ **句頭A-01赤＞総赤＞整合score でソート**＋多様 top-k＋`meta.lyricFit={score,a01Head,a01Total,red,yellow,onsetMatch}` 添付（select はしない＝機械は候補まで）。(c) 出口＝`gen_melody` に `lyrics` オプション追加（新 verb を作らず CHAT_VERBS 整合維持）。ハンドラ（async）が accents を **`accentsFromSyllables(plan.syllables)`** で1回だけ spawn 注入（失敗＝内蔵ヒューリスティック fallback・`accentSource` を返す）。
  - **⚠️ 訂正（2026-07-28）**: ここに「**既存 chat verb** `gen_melody`」と書いてあったが、**この前提は実装時から誤り**だった。`gen_melody` は `mcp.ts` の legacy 専用ブロック内にあり、**`surface:"chat"` ではツール一覧に登録されていなかった**（＝chat verb ではなかった）。そのためプレイブックが「`gen_melody` を `lyrics` 付きで呼べ」と指示しているのに呼べず、**詞先メロは黙って死んでいた**（過去のBUG#1と同型）。2026-07-28 に修理＝登録を共通ブロックへ移し `CHAT_VERB_NAMES` にも追加。再発防止として「**プレイブック本文が呼べと言っている verb は全部 `CHAT_VERB_NAMES` に含まれる**」を機械検査するテストを `apps/api/test/chat-session.test.ts` に置いた。
...
- **✅ 決着（2026-08-02 オーナー裁定）**：WP-L0 の「オンセットロール」＝**長音ー＝直前へ延長・促音っ＝詰め＝音符を立てない**（`apps/api/src/music/lyricsPlan.ts` `isOnsetMora`）が**正で確定**。2026-07-29 にここへ書いた⚠訂正（「『っ』『ー』にも音符を立てる側へ変える」）は**オーナー自身が撤回**＝原文「促音伸ばし音はいらない、打ち込むときに工夫するので」＝`planLyricMelody` の音数＝**オンセット数のまま**・工事は消滅（§31-8）。**入口を表記にそろえる**話は別件で生きていて実装済み＝読みは呼び側が先に解いて `opts.readings` で渡す（§31-3(f)・スライス7＝既定OFF の `readLyrics`）。`mcp.ts` の説明文の嘘（「漢字はpyopenjtalkが読む」）も同時に直した。
- **受け入れ（機械）**：モーラ⇔音数（＝計画オンセット数）100%一致（全候補・property）／句頭A-01赤の件数を候補メタに表示（ゼロ強制なし）／整合score降順ソート／E-rule ガード維持（V2既存）／`lyrics` 無し bit一致。※この「モーラ」の数え方＝**オンセット（っ/ー を外した数）で確定**（2026-08-02 裁定・§31-8）。**残り＝リズム配分ヒューリスティックの質と耳較正約30%（A/C 指摘）＝M-1 は「音数一致＋句割り＋整合レポート」まで**。
...
- **(e) 物理フィルの結線（step2 完了・2026-08-21）＝再生/MIDI へ流す**：`buildPhysicalFill` は grid 経路（`applyDrumFill`）と同じく **N 小節へ展開**した自己完結 content を返す（base bar0 groove をタイル・`bars=N`/`steps=N*grid`）＝**セクション合成はタイルしない**（`compositeNotes` は content の実尺をそのまま置く）ので、物理フィルも N 小節で自己完結させる必要がある。web `rhythmToNotes`（api/web 両再生・MIDI 書き出しが収束する単一変換点）が `r.fillNotes`（絶対 qb＝`Note.start` と同単位で 1:1）をドラムノートに重畳＝**再生（Tone.js）と MIDI 書き出しの両方で鳴る/書ける**。`fillNotes` 無し/空＝grid のみ＝従来 bit 一致（additive）。到達口＝`/music/gen_drums`・MCP `gen_drums`・`/gen/section`（body `drums.fillStyle:"physical"`＋任意 `fillKind`）。web UI＝TinkerSheet ドラム引き出し「細かく」内の**「フィルの作り方」3択 select**（`格子（従来）`＝未送信／`型辞書`＝`fillStyle:"physical"`／`解いて作る`＝`fillStyle:"body"`＝M3・下の (g)）。**フィルが立っている時だけ送信・既定＝格子＝従来 bit 一致**。`body` を選んだ時だけ意図つまみの select が2つ出る＝**フィルの行き先**（おまかせ／上る＝`bodyDepth:-0.3`／落とす＝`bodyDepth:0.8`）と**手触り**（おまかせ／フラム多め＝`drummer1`／フラム少なめ＝`drummer7`／統計なし＝`bodyDrummer:"none"` の純物理）。**生の数値ノブは出さない**＝つまみの意味（行き先・手触り）で選ばせて内部値は UI が持つ。**耳＝実際の作曲で使う中で採否**（抽象 A/B ゲートは置かない）。
  - **(j) 耳判定の決着（2026-08-30・オーナー試聴）＝辞書と生成は併存／ノリ既定は 0.5**：
    ...
    - **三連（シャッフル）のフィルは耳で通った**＝三連スロット増分は採用。
  - **(i) 文言とフォールバック通知（2026-08-29・オーナー裁定）**：
    ...
    - **未知の型ID も告げる（2026-09-17・backlog「黙って落ちる」修正）**：`gen_chord_pattern` の `pattern`／`gen_bass`・`gen_drums` の `style` が
      ...
      同じ作法で **`fill` の文字列（フィル型ID）** も：ベース＝`BASS_FILLS`・ドラム＝`FILL_TYPES`（build.* 含む）/`FILL_6_8` に無ければ「〈ベース／ドラム〉のフィルの型『…』が見つからないので…」。数値の強さ・`fillStyle`・`bodyAim` は対象外（出音は不変）。
- **(f) 物理フィルの置き方＝源流 `apply_fills` 準拠へ修正（2026-08-29・オーナー耳判定の差し戻し）**：step2 の初版は「**フィル小節を丸ごと空けて**、`placeFill(fillBar, 0.0, "bar", …)` の裸のフィルを1小節鳴らす」形だった。これは phrase_maker に**存在しない極端形**で、耳判定で「退屈・音楽的に成立していない」と却下された（比較対象の grid 経路 `FILL_TYPES` は、たとえば `fill.snare.1beat` が HiHat/Kick を3拍鳴らしたまま4拍目だけスネアという形＝**グルーヴ継続が前提**）。修正3点：
  ...
  - **(f-2) 型ごとの長さ＋プール再編（2026-08-29・耳判定2巡目）**：一律「末尾1拍」も粗すぎた。耳判定＝「32分連打は無し／連打でない型はリズムとして成立していない＝選び方が変／型の種類が少ない」。
    ...
    - **プールの再編（現況のプール定義は (d) が正準・ここは理由だけ）**：初版は `flam_accents` / `sixteenth_groove` がどちらの aim プールにも入っておらず、**aim 指定時に10型中2型が永久に出ない**穴だった（「型の種類が少ない」の直接原因のひとつ）→ 方向を持たない**中立型**として両プールへ。`snare_roll_32`（32分連打）は耳判定で却下 → **既定プールから降ろす**が**削除はしない**＝明示 `fillKind` でのみ到達（捨てない）。
  - **(f-3) 3/4 のグルーヴ格子（2026-08-29・耳判定「3/4だけ変」）＝物理フィル以前のバグ**：`defaultDrumBar` は 4/4 の16格子と 6/8(compound) の12格子しか持たず、**3/4 が 4/4 の16格子へ落ちて 1step=3/16拍へ引き伸ばされていた**（1小節=3.008拍・バックビートが 0.75/2.26拍という拍にも三連にも乗らない位置）。`resolveStyleContent` も同じ式で 4/4 型を 3/4 に当てていた。修正＝**3/4 は12格子（16分×3拍・beatsPerStep=0.25）**の専用分岐（1拍目キック・2/3拍目バックビート）＋**拍数ゲート**（4/4 型を 3/4 に当てない＝3/4 型が出来るまで既定グルーヴへ落とす）。4/4・6/8 は不変（bit 一致）。**フィル側（`fillMeter("3/4")`）は源流にも 3/4 があり正しかった**＝変だったのは下のグルーヴ。残り＝3/4 用の定型ビート型と grid フィル型がまだ無い（`FILL_TYPES` は 4/4 と 6/8 のみ＝3/4 では grid フィルが付かない・物理フィルは付く）。
  - **(g) 身体シミュレータの移植（2026-08-29・`bodyFill.ts`）＝辞書ではなく生成器**（実装計画では **M2 の続き**＝ドラム本実装の増分。計画の M3 は「ベース」なので番号を混ぜない）：耳判定「型の種類が少ない」の根本解。phrase_maker `bodyfill.py`（676行・fills/ で最大）を忠実移植した。**型を表から引かず、毎回 DP で解く**：グルーヴが手を置いていった位置（右手=ハット・左手=スネア）から、キットの座標・打面の高さ・リバウンド係数・手の連打レート上限・移動速度の制約を通って、crash+kick の着地まで、**両手が辿れるいちばん安い経路**をスロット単位のビーム探索で求める。骨（skeleton）＝いま鳴っているグルーヴ自身を引用するので、フィルが曲から浮かない。到達口＝`fillStyle:"body"`＝`/music/gen_drums`・MCP `gen_drums`・`/gen/section`（body `drums.fillStyle:"body"`）・web UI（(e) の3択 select）。意図つまみ（`bodyDepth`/`bodyDensity`/`bodyCrescendo`/`bodyTailAnchor`/`bodyDrummer`）と `fillLength`/`fillBeat` も同じ4口すべてから渡せる（2026-08-29 受け入れ監査＝**作ったのに触れないノブは硬化**なので、MCP の enum と `/gen/section` の型を実態へ揃えた）。
    - **経緯の訂正**：アーク引き継ぎ書が `bodyfill.py` を「死にコード」と誤記していたため M2 で移植から漏れていた。実際は `fill_engine="body"` の opt-in 代替エンジンで、デモ `generate.py` から参照されていないだけだった。
    - **決定論**＝コストは整数マイクロ単位へ量子化・同値の並びは md5・k-best の抽選も md5 の整数（RNG も builtin hash も不使用）。**検証＝データ一致**：3拍子×3長さ×3強度×3ソルト×2テンポ＝162件＋GMD prior/意図つまみ84件＝**246/246 で打点・打圧・手の割当・コストが Python と完全一致**。代表10件を `music-core/test/body-fill.test.ts` にゴールデンとして固定（phrase_maker 不在でも走る）。
    - **意図つまみ**（形の辞書を増やさずに狙いを指す）＝`bodyDepth`（行き先＝アークの到達点 -0.30..0.80／負は crash へ**上る**タム回し）・`bodyDensity`（忙しさ 0..1.8）・`bodyCrescendo`（0..1）・`bodyTailAnchor`（0..1＝末尾をスネアの引力に潰させない紐）。**`bodyDensity` は純物理のときだけ効く**（GMD prior 下では骨外密度もバウンスも実測カーブが担う＝源流どおり）。
    - **GMD prior（統計のみ・裁定済み 2026-08-29＝B3 決着）**：実ドラマーの **order-0 集計のみ**（8ビンの打圧/密度カーブ・フラム率・足ゴーストの率）を `gmdPrior.ts` に同梱。**bigram / n-gram / リテラル打点列は持たない**＝「他者コーパスは統計のみ」の内側。ライセンス＝**CC BY 4.0**（商用可・改変可・**非コピーレフト**）＝義務は**表示と改変の明示のみ**。表示の置き場＝`NOTICE.md` ＋コード定数 `GMD_ATTRIBUTION`（アプリ内クレジットにも出す）。`bodyDrummer:"none"` で prior を外した純物理も選べる＝統計は必須ではなく色付け。
    - **三連スロット（2026-08-29 増分・源流に無い＝移植ではなく新規）**：定規は2本＝16分（0.25qb・源流どおり）と**三連（1/3qb・shuffle 型）**。`planBodyFill` の `sub` で選ぶ（既定 0.25＝ゴールデン10件と bit 一致のまま＝additive）。DP・物理はスロット幅を**秒**に換算して効かせる設計だったので、連打レート上限（10 hits/s）・リバウンド窓（35..130ms）・「同じ手の3連続禁止」「届かない移動禁止」は三連でもそのまま成り立つ。バウンス（ダブルの2打目）は定数 0.125qb でなく**半スロット（sub/2）**＝三連では 1/6qb・遅テンポでは窓が閉じて物理的に打てない（`bouncePlayable`）。`buildBodyFill` は beatsPerStep から定規を判別（0.25→16分／0.333→三連・qb 計算は正準の 1/3 で行い round3 の 0.333 で敷かない）＝**shuffle 系の曲でも body エンジンが解ける**（従来は型辞書へフォールバックしていた）。テスト＝`body-fill.test.ts` の「三連スロット」describe＋`drum-physical-fill.test.ts` の shuffle 件。
    - **限界（黙って歪めない）**：**混在格子（1本のフィル内で16分と三連を混ぜる）はやらない**＝フィルは「いま鳴っているグルーヴの引用」なので定規はグルーヴのものを1本だけ使う（混ぜるには 1/12qb 格子が要り、隣接スロット＝バーストの物理判定が壊れる）。6/8 の付点拍の3分割は八分＝既存 0.25 格子で表せる＝三連定規の出番ではない（1.5qb は 1/3 で割り切れず、渡せば黙らず投げる）。0.25/1/3 以外の格子は付与せず型辞書へ落ちる（fillKind に型名が出る＝取り違えない）。
    - **意図的な移植差分**（「忠実移植」を名乗る以上は明記する）：Python `skeleton_from_spec` の **`rhythm.cell` 分岐**（セル持ちリズムの `span_bars` で小節ごとに onset/accent/kick を差し替える）は TS `skeletonFromSpec` に**移植していない**。呼び手（`buildBodyFill`）が bar0 の展開済みグルーヴを `BodyRhythmSpec` として渡すので現状は実害ゼロだが、**セル持ちリズムを直接渡す呼び手が出来たら小節別 onset が効かない**＝その時に足す。
    - **コスト**＝1本あたり 17ms（1拍）/ 41ms（2拍）/ 99ms（1小節）＝生成呼び出しとしては許容、ホットループには置かない。
  - **(g-2) 狙い「タム回し」＝`bodyAim:"tom_tumble"`（2026-09-16・オーナー耳判定で取り込み）**：phrase_maker のフィル複数案（`ensemble.py` `AIM_PRESETS`）を同じ場所（4小節目→5小節目着地）で聞き比べ、オーナー判定「**タムを転がすまではいい／スネアとゴーストロールは差が無い／駆け上がりは聞いたことない音で却下**」（レビューの「ドラム3点は移植しない」を耳が覆した）。**取り込むのはタム回しだけ**。
    - **中身の判定（実物で確認）**：源流の `tom_tumble` は**型の表ではない**＝`dict(depth=0.45, density=0.9)` という**意図つまみの1点**で、毎回 `bodyfill.plan_fill` の DP が解き直す（源流コメント "NOT a shape dictionary"）。試聴の条件は `intensity="flashy"`・`length="bar"`（小節頭から叩いて次の小節頭へ右寄せ着地）・**`fill_prior=None`＝純物理**（demo の Chart は prior 無し）。Otomemo 移植済みの `bodyFill.ts` は **既存つまみ（`bodyDepth`/`bodyDensity`/`bodyDrummer:"none"`/`fillLength:"bar"`/`fillBeat:0`）で既に同じ点を出せる**ことを tsx で実測（4/4・3/4・6/8 とも着地 crash+kick まで解け、タムの打点が現行既定より増える）。＝**新しい知識は要らない。足すのは「タム回しという狙いを名前で選べる口」だけ**。
    - **コンセプト確認**：(A) Otomemo＝opt-in の候補1本を出すだけ（自動適用しない・未指定は bit 一致）／シーケンサー化しない（小節に何を置くかは利用者のまま）／**配るのは生成器**（名前はつまみの値の束で、打点列を持たない＝表を配らない）→ 合う。(B) phrase_maker＝身体シミュレータ（物理 DP が経路を解く＝物理層が生成する層のまま・バリデータに堕ちない）／平均を出さない（seed ごとに別の経路）→ 合う。**外したもの**：`ghost_roll`・`snare_build`（耳で差が無い）、`uphill_crash`（耳で却下）、実効メニュー（20点の近似重複刈り込み）と自動バリエーション割当（今回の耳判定の対象外＝入れない）。
    - **契約**：`DrumsGenOpts.bodyAim?: "tom_tumble"`。`fillStyle:"body"` かつフィル要求があるときだけ効く。解決＝`bodyDepth ??= 0.45`・`bodyDensity ??= 0.9`・`bodyDrummer ??= "none"`（試聴どおり純物理。`bodyDensity` は prior 下では効かないため）・`fillLength ??= "bar"`・`fillBeat ??= 0`。**明示つまみが常に勝つ**。未指定＝従来 bit 一致。
    - **通知（`meta.warnings`・落ち先ごと）**：①知らない狙い名＝「知らない狙い『…』は無視しました」②body でない／フィル無し＝「タム回しは『生成する』フィルのときだけ効きます（今回は使っていません）」③body が解けず型辞書へ落ちた＝既存の落ち先文言に加えて「タム回しの狙いは使えませんでした」④解けたがタム（GM 41/43/45/47/48/50）が1打も無い＝「タムへ回る経路が解けませんでした（タムの無いフィルです）」。
    - **到達口**：`/music/gen_drums`（body `bodyAim`）・MCP `gen_drums`（inputSchema に enum）・`/gen/section`（`drums.bodyAim`）・web「フィルの行き先」select に「タムを転がす」。
    - **受け入れの主は耳**（データ一致は既存 246 件のつまみ検証で担保済み＝新規の数値移植は無い）。試聴＝scratch `listen5/`。
    - **追補：終わり方の軸（2026-09-17・オーナー耳判定「最後のスネア四連打はいいけどバリエーションの一つにしてほしい、固執するの辞めてほしい」）**：
      - **真因（実測・104bpm/5小節/強さ最大・seed 1〜50）**：直す前は**50本すべてが同じ終わり方**（最終拍＝フロア2打→スネアのダブル2組＝16分＋32分の四連打）。phrase_maker 源流 `bodyfill.plan_fill` を同条件（8ビート骨・16分骨の2種）で回しても **50/50 同じ**＝移植の誤りではなく**源流の性質**。仕組み＝(a) コストの形：`density 0.9` は着地前のバウンスを `dense·frac²` で安くし、バウンス代 `0.35/REBOUND` はリバウンド1.0のスネアが最安＝スネアのダブルが着地前の唯一の最安経路になる。(d) seed の抽選は最適から温度220マイクロ単位（0.22）の帯の中だけ＝別の終わり方は帯の外（≧0.5 高い）なので seed が終わり方に届かない。(b) 着地の紐 `tailAnchor` は既定0（スネアの引力に任せる）。つまみを1つずつ動かしても**どの1点も1種類へ収束する**（tailAnchor 0/0.25＝四連打 50/50、0.75/1.0＝フロアのまま 50/50、density 0/0.4＝8分のフロア、depth 0.2/0.8＝スネア六連/四連）＝「1本のつまみの1点」は必ず平均（その点の最安）を出す＝`docs/research/2026-09-16-phrasemaker-ideas-to-keep.md` §1-3・負の知識8 の実例。
      - **直し方**：タム回しの狙いに**終わり方の軸**を1本明示し、**質の違う3点**を持つ＝`tailAnchor` ∈ {**0＝スネアへ戻って四連打**／**0.6＝スネアのダブル→フロアで着地**（混ざり）／**1.0＝フロアに留まって着地**}。どの点を使うかは seed で決める（`|trunc(seed)| mod 3`＝決定論・乱数/時刻不使用・隣の seed が別の終わり方になる）。**各点の中身は毎回 DP が解く**＝打点列の表は持たない（身体シミュレータの芯＝コスト・物理制約・ビーム・k-best 抽選は1行も変えない＝`bodyFill.ts` 無変更）。`bodyTailAnchor` を**明示したら軸は使わない**（明示つまみが勝つ）。
      - **効く範囲**：`bodyAim:"tom_tumble"` の解決の中だけ。**`bodyAim` 未指定の body フィルは1ビットも変えない**（既存テスト＋seed×拍子の完全一致テストで担保）。
      - **ゲート（機械）＝最低限だけ**：seed 1〜50 で「四連打以外の終わり方が1つ以上」「四連打も出る」。**割合を合否にしない**（分布は診断として数え、形の要求を硬化させない）。直した後の分布の実測はテストの診断出力と scratch `listen9/` に。
      - **コンセプト確認**：(A) Otomemo（`docs/requirements.md` 冒頭＝候補までが機械・仕上げは人）＝seed を変えれば**質の違う終わり方の候補**が並ぶ／どれを使うかは人が選ぶ・自動適用しない・未指定 bit 一致→合う。(B) phrase_maker（`CONCEPT.md`＝平均を出さない・身体性シミュレータ）＝1点つまみが1種類に収束する「平均」を、軸ごとの複数点で割る／各候補は物理 DP が解いた経路のまま（形を表から引かない・物理層はバリデータに堕ちない）→合う。**源流への逆輸入は未実施**（向こうの `AIM_PRESETS` も同じ固執を持つ＝記録のみ）。
      - **受け入れの主は耳**：scratch `listen9/`（直した版 seed 5本＋直す前1本）。
...
**正典・出典**：設計検討3フェーズのモック（オーナー受渡し済＝`chord-editor-mockup.html`〔バッジ精緻化〕／`chord-progression-rethink.html`〔概観4方式＋入力再考〕／`chord-progression-unified.html`〔Section 整合・横スクロール版〕）と分析（Q1/Q2 比較・ピアノロール判定・手数表・実機スクショ実測）は `docs/archive/2026-07-18-chord-editor-timeline.md`。本節の折り返し採用はモックの横スクロールを覆すオーナー最終判断（同 doc に追記）。
...
## #31 作詞補助＝歌詞を書く（設計 2026-07-29・スライス1〜4を確定／置き場は 2026-07-30 裁定＝案(い) 確定でスライス5〜7も一本化）

> **⚠ この節（2026-07-29 追加分）はオーナーのレビューを受けていない。** オーナーの了解のうえ、留保つきで先行している。
> **上位の根拠**＝`docs/requirements.md`「歌詞を書く」節（2026-07-29 追加）と `docs/architecture.md` 同日追記。**この2つもオーナー未レビュー**。
> **設計の元**＝`docs/research/2026-07-29-lyric-assist-design-settled.md`（研究文書は根拠・本節が正典）。コンセプト＝`docs/research/2026-07-29-lyric-concept-settled.md`。判断材料＝`docs/research/2026-07-29-lyric-storage-decision-material.md`。
> **歌詞の置き場は案(い)＝人が合図して空のメロを置き、そこに書く、で確定（2026-07-30 オーナー裁定・代わりに既存メロディなどからのコピー機能を充実させる＝スライス6）**。**空メロの見た目も (い-c) 遅延生成で確定（2026-07-30）＝空のうちは実体を作らず、句を書いた瞬間に作って置く**（§31-9）。スライス1〜4は4案のどれでも壊れない形で確定してあった（§31-0 の検算）＝これらの裁定による手戻りは無い。
> 本節が引くオーナーの言葉は、すべて `docs/research/2026-07-29-lyric-concept-dialogue.md` か `docs/research/2026-07-27-lyric-assist-v7-facts-brief.md` に実在することを grep で確かめた。

### 31-0 芯（1画面ぶん）＋置き場に依存しないことの検算

- **歌詞の正データ＝句**（漢字仮名交じりの普通の文章のひとかたまり）。**メロネタの `content.lyric.phrases` に持つ**。かな・高低・音符への割付は表記から機械が導き、人がいつでも上書きできる。`Note.syllable` は**仮歌のための写し**であって歌詞の本体ではない（原文「基本的にかながきは仮歌のためにあるものであって、歌詞の正データは漢字仮名交じりの普通の文章です」）。
- **正の規則（1行）**：**句の範囲にある音符では句の読みが正。どの句にも覆われていない音符では従来どおり `Note.syllable` が正**（人の手打ち）。＝既存のかな入力の口（`set_lyric`・PianoRoll の流し込み欄・詞先メロ生成）を1つも殺さない＝**後退ゼロ**。
- 音符を変えずに読みを写す純関数 `placeMoras` を `packages/music-core` に**1本だけ**置く（§31-2）。
- 読みは表記から pyopenjtalk（`apps/audio/accent.py`）で取り、画面からは新設 `POST /music/reading` で届く（§31-3）。**複数句は必ず1回にまとめて渡す**（実測：1句ずつ40回=5,799ms／まとめて1回=149ms）。
- 従うコンセプトの確定：正データは漢字仮名交じり／かな書きは仮歌のため／~~「っ」「ー」にも音符を立てる（Q7「立てる」）~~ → **2026-08-02 オーナー裁定で撤回＝立てない（従来＝長音は延長・促音は詰め）で確定**（§31-8）／字余りとメロが途中を分ける／割付の手直しは最初は作らないが、手でずらす・自動符割りを見据えた作りにする。

**置き場非依存の検算（実コードで経路を全部当たった・結論＝スライス1〜4は依存しない）**
...
| 4（手直し） | 句の中の `edits`（§31-6） | 句の中に閉じる |

4案の違いは「**メロの無い区間**の句がどこに入るか」だけで、スライス1〜4が扱うのは**音符を持つメロの句**だけ。**守ることが2つ**：

1. **句の範囲の既定を「音符の広がり」で定義しない。** 既定＝そのメロの**尺**（`bars` × 1小節の拍数）。音符から導くと、(い) 系を採ったとき音符0個の空メロで範囲が定義できなくなる。尺から導けばどの案でも同じ式で立つ。字余り／メロが途中の言い分け（§31-5）も、範囲が音符と独立であることが前提。
   - **⚠ 監査の訂正（2026-07-29）**：ここに「`bars` は保存時に必ず入る」と書いていたが、**それはエディタから保存した場合だけ**（`useNetaEditor.ts:334` の `bars: Math.ceil(len/bpb)`）。生成や MCP で作られてエディタを通っていないメロは `bars` が空のまま入る＝**実データ 39件中 26件が `bars` NULL**（2026-07-29 実測・`data/cm.sqlite`）。`bars` 空のときの既定範囲をどう決めるかが要る（決めないと、既存メロの2/3で句の範囲が0になり `placeMoras` が何も書かず `phraseStatus` が全部「メロなし」になる）。
     **決めた（2026-07-29・スライス3で確定）＝`bars` があれば `bars × bpb`、無ければ音符の終端を小節単位へ切り上げ、下限は1小節**（`PianoRoll.tsx` の `phraseSpan`）。`bars` は編集画面から `KindEditorBody` 経由で渡す。
     - **下限1小節があるので、音符が0個でも範囲は0にならない**＝「置き場の裁定に引きずられない」という守ることの目的は満たす。
     - **一度エディタの表示尺 `len` を使って失敗している**（2026-07-29・実測）。`len` には 16拍の下限があるため、2小節のメロでも範囲が16拍になり、§31-5 の言い分けが**何を書いても「メロがまだ途中」**になった（範囲に1小節以上の空きが常にあるため）。言い分けが意味を持つには、範囲がそのメロの実際の尺でなければならない。
     - **済んだ（2026-07-29）＝`createNeta` で尺を入れる**（`core.ts` の `withDerivedBars`）。尺が言われていないネタに、音符の終端を小節単位へ切り上げた値を入れる。明示された `bars` は上書きしない・音符を持たない kind は触らない・長さのある音符が無ければ触らない。これで生成・MCP・取込で作られたメロにも尺が入り、句の範囲が音符に頼らなくなる（既存ネタは空のままなので `PianoRoll` の切り上げが受け皿として残る）。
       **安全性の根拠**＝`bars` は曲の中でのメロの尺には使われていない（`sectionContext.ts` の `childDur`/`contentDur` は音符だけを見る）。使うのは編集画面の表示尺と句の範囲だけ＝**配置も出音も動かない**（api の既存テスト1494本が1本も壊れないことで確認）。
   - **同じく実測の注意**：`len` には 16拍の下限がある（`useNetaEditor.ts:141` の `Math.max(16, …)`）ので、内容が短いメロほど保存される `bars` が実際の音符より広くなる。既存メロ37件（音符あり）のうち、尺が音符の広がりを1小節以上超えるものは1件＝「メロが途中」の誤検出は今のデータでは少ないが、ゼロではない。
2. **スライス1の保存は「メロの content に lyric を素通しする」以上のことを決めない。** ~~(い-b) が裁定された場合、保存の前段に「産む」フックが挟まる~~ → **不要になった（2026-07-30 裁定＝(い-c) 遅延生成）**：実体を作るのは**通しで読む面での句の確定**（＝メロを新規作成する既存の口）で、既存メロの保存経路には何も挟まらない。＝この検算は結論だけ残す＝スライス1の保存は素通しのままでよい。

スライス7（詞先メロと分析の入口を表記にそろえる）**も置き場に依存しない**。生成が返すのはメロネタの content（`generate.ts:784`）で、句を載せる先はメロ自身。順番の制約は「音符数が変わるので耳で確かめてから入れる」だけ＝5・6 と並行に進めてよい。

### 31-1 句のデータの形（`content.lyric`）

**現況**：歌詞テキストは4経路すべてで捨てられる＝`apps/web/src/components/PianoRoll.tsx:69`（`lyricDraft` は「永続せずUIだけ」とコード自身が明記）・`apps/web/src/lyrics.ts:70`（詞モード手打ちは `Note.syllable` へ直書き）・`apps/api/src/mcp.ts:945,953`（`set_lyric`）・`apps/api/src/music/generate.ts:762,784`（詞先メロ＝返り content は `{notes}`／`{notes,feel}` のみ）。メロの content の既存キーは `notes / program / sing / feel`（`useNetaEditor.ts:334`）。**content は素通しの JSON**（`apps/api/src/schemas.ts:16` が `z.unknown()`）＝**DB スキーマ変更・移行は不要**。

型は `packages/music-core/src/lyric.ts`（新設）に置き web/api で共有する（純データ＋純関数のみ＝music-core の憲章「アプリ非依存の純粋派生」に収まる。`index.ts` から re-export）。
...
export interface LyricReading {
  ...
  hl: (0 | 1)[] | null;   // モーラごとの高低。門番（§31-3(c)）で不一致なら null＝読みは出すが高低は出さない
  breaks: number[];       // 読みの句の切れ目（モーラ添字）
}
```

- **かなの使い分け**：音符と仮歌に載せるのは **`pron`（発音・「は」→ワ）から「’」を除いたもの**。`read`（読み・「は」→ハ）はふりがな表示用に語列へ持つ。根拠＝「かな書きは仮歌のためにある」＋既存実データ4件が発音側で入っている（「思い**わ**」「だろ**お**」）。※これはオーナーの明示裁定が無い設計判断＝§31-11 の裁定待ち10。
- **範囲の規則**：同じネタの句同士は重ならない・隙間があってよい。「範囲内の音符」＝`phrase.start <= note.start < phrase.start + phrase.beats` の音符（start 昇順）。
- **曲(song)側**：4案の(あ)を採った場合だけ `song.content.lyric` を同じ `LyricLayer` で持つ（§31-9）。**スライス1〜4では作らない・触らない。**
- **`kind:"lyric"` ネタ**：曲もセクションも無い思いつきのメモ・下書き置き場として残す。句へ写すときは**表記のまま**（かなに潰さない）＋ `source` を記録。
- **既存の `syllable` 持ちメロ**：表記は機械で復元できない（仮歌用に崩れたかな）。句を作るまでは「句に覆われていない音符」として従来どおり鳴る＝後退ゼロ。句を作った時点で旧 `syllable` は「音符のかな上書き」（§31-6 の `kind:"kana"`）として取り込める。

### 31-2 音符を変えずに読みを音符へ写す（`placeMoras`）

**現況**：`syllable` を書く純関数は `flowLyric` だけで、**同じものが2本ある**（`apps/api/src/lyric.ts:37` と `apps/web/src/lyrics.ts:34`）。どちらも (1) モーラが多ければ最長音符を半分に割る（下限16分＝`MORA_FLOOR=0.25`）(2) 全音符の `syllable` を頭から無条件に上書き (3) 余ったモーラは最後の音符に連結。既存の割付を守る引数は無い。＝**音符を変えずに写す処理はリポジトリに存在しない**。実装者がこれを呼んだ瞬間、詞を打つたび音符が増える。

**決め＝新設 `placeMoras`（`packages/music-core/src/lyric.ts`・1本だけ）**

```ts
export function placeMoras(
  notes: Note[],
  moras: string[],                          // effectiveReading(phrase) 由来＝人の直し反映後のかな列
  range: { start: number; beats: number },  // 句の範囲
): Note[]
```

**契約（テストで縛る）**
1. 純関数・非破壊。返りの音符列から `syllable` を取り除くと、**入力と完全一致**（数・順序・start・dur・pitch・vel すべて。property test）。
2. 範囲外の音符は **`syllable` も含めて**一切触らない（他の句・手打ちかなを壊さない）。
3. 範囲内の音符（start 昇順）へ moras を頭から1対1で `syllable` に書く。**モーラが尽きた先の範囲内の音符には書かない＝`syllable` を空にする**（かなが空の音符は仮歌の既定「ラ」で鳴る＝`sing.ts:28`・**2026-07-29 オーナー裁定**）。**モーラが余れば書かない＝音符は増えない**。余りの数はここでは言わない（言い分け §31-5 が別途言う）。
   - **改訂（2026-07-30）**：旧契約「音符が余れば `"ー"`（メリスマ）」をやめた。理由＝(1) メリスマを自動で書く先例が15種中ゼロ (2)「あとN音」表示と矛盾（あと3音と言いながら音符は埋まって見える）(3) 実害＝詞モードで人が打ったかなを写し直しが「ー」で消す（`kanaEditForNote` はモーラの尽きた先で null を返し直しとして記録されないため）。根拠＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §2-4。手でメリスマを書きたいときは詞モードで「ー」を打てばよい（`kind:"kana"` の直しになる…のは句に覆われ moras の範囲内のときのみ。範囲外は従来どおり `syllable` 直書き）。
4. **導出（表記→モーラ・高低）／割付（モーラ→音符＝この関数）／音符の作り替え（`flowLyric`）は別の段**。`flowLyric` は**人が明示の操作をしたときだけ**走る（操作の存否・名前・置き場は §31-5＝研究の結論待ち）。

**`flowLyric` の二重の始末**：`flowLyric`・`splitMora` を `packages/music-core/src/lyric.ts` へ**移し**、`apps/api/src/lyric.ts` と `apps/web/src/lyrics.ts` は music-core からの re-export に置き換える（呼び側 import 無変更・挙動 bit 一致のまま2本→1本）。`placeMoras` を最初から music-core に置くのはこの再発防止。

### 31-3 読み取り（pyopenjtalk）の経路と、まとめて一度に渡す決め

**現況**
- `apps/api/src/accent.ts:76` `accentsFromSyllables` が**音符に載ったかなを連結して**渡す。実測30行で句の割れ方が 30/30行変わり、高低は 139/284モーラ＝**48.9%反転**。モーラ総数は一致するため `mapAccents`（`accent.ts:59`）は null を返さず、**誤った高低が検出されずに通る**。
- `apps/audio/accent.py` の `extract()` の返りは `moras` が**ローマ字音素の連結**（`"".join(m["phones"])`）＝**かな・語列を返す経路が無い**。stdin の一括受けには対応済み（`accent.py:107`＝1行1文）だが、`accent.ts:18` の `run()` は stdin に書いていない。
- 画面が叩ける `/music/:op`（`apps/api/src/http.ts:201`）は35個で**歌詞系ゼロ**。いま画面の赤黄は9語の内蔵辞書（`prosody.ts:185-195`・`lyricFit.ts:31` はブラウザ内同期計算）で、核を持つのは「はし・そら・ゆめ」の3語だけ。
- `/music/:op` は switch 全体を1つの try/catch で包み、投げられたものを**全部 400 に落とす**（`http.ts:379-381`）。

**決め**

**(a) `accent.py` に語列とかなを足す**（追加費用実測 0.02ms/回）：各文の返りに `words: [{surface, read, pron, mora_size, pos}]`（`pyopenjtalk.run_frontend`）を追加。既存の `moras/hl/trans/phrases/mora_total` は不変（既存呼び出し＝`analyze_lyric_fit` の自動注入は壊さない）。「’」の除去・`pron` のかな片への割りは TS 側でやる（Python は素通し）。

**(b) `accent.ts` に一括口を足す**：`extractReadings(texts: string[], signal?): Promise<ReadingResult[]>`＝texts を改行区切りで **stdin にまとめて書いて1回 spawn**（`run()` に stdin 書き込みを足す）。1行1文プロトコルのため、**呼び側（api）が句の表記を「行」と「穴」で断片に割ってから渡し、返りを句へ組み戻す**（表記の改行は行の切れ目・「＿」1個以上は穴＝断片の境。空断片は送らない）。タイムアウトは一括でも 15秒で足りる（40句149ms 実測＋起動揺れの余裕）。**1句ずつ呼ばない**＝1回あたりのほとんどが Python の起動と辞書読み込み（40回で5,799ms）。

**(c) TS 側の門番（2つだけ・順に）**
1. 語ごとに「pron を割ったかな片の数 ＝ mora_size」。合わない語だけ**語まるごと1区間**（`word=-1`＝モーラ対応不明）として持つ＝句全体を捨てない（実測38行199語で不一致1語）。
2. **hl の長さ ＝ モーラ列の長さ**（words は run_frontend・hl は extract_fullcontext＝別々の解析。実測6例は一致だが保証が無い）。合わなければ**その句は `hl=null`**＝読みとモーラは出す・高低と印は出さない。

**(d) 画面から届く口＝`POST /music/reading`（新設・`/music/:op` に case を1つ追加）**
- 入力：`{ texts: string[] }`（1要素＝1句の表記まるごと。断片化は api 側でやる）。上限＝texts 200件・合計2万字（通しの面でも曲1本で足りる）。
- 返り：`{ results: [{ words, moras, hl, breaks, error? }] }`（`LyricReading` から `forText` を除いた形）。1句の解析失敗は `results[i].error`＝他の句は生きる（`accent.py` の流儀と同じ）。
- **失敗の返し方**：入力が変（配列でない・上限超）＝400。**Python 側の失敗（venv 無し・起動失敗・時間切れ・JSON 取れず）＝ case の中で自分で `reply.code(502)` を返す**。外側の catch は投げられたものを全部 400 にする（`http.ts:380`）ため、**throw で外へ出さない**。400/502 を分けないと画面が「入力が変」と「機械側の失敗」を区別できず、下の規則が効かない。
- **失敗時の画面の規則**：「読みが取れませんでした」と出し、表記・割付・音符はそのまま・**印と高低は出さない**＝表記を持つ句では9語辞書に落とさない（誤った高低が「機械の読み取り」の顔で通るのを止める）。句に覆われていない旧データでは従来どおり9語辞書が動く＝後退ゼロ。
- MCP には今回足さない（スライス1〜4は画面から使う）。足すときは `CHAT_VERB_NAMES`（`apps/api/src/chat-session.ts:19`）への登録が必須（BUG#1「登録したが許可漏れで黙って死ぬ」型の再発防止）。
- **読み取りの契機＝「読みを反映」ボタン押下時（2026-07-30 改訂・明示適用）**。打鍵デバウンスでの自動取得はやめる（`READ_DEBOUNCE_MS`・`askedRef`/`appliedRef` の競合対策も不要になる）。ボタン名「読みを反映」は 2026-07-29 オーナー裁定。根拠＝先例15種すべてが明示適用（`docs/research/2026-07-29-lyric-input-ui-survey.md` 定石2）＝打った瞬間に仮歌・音符上のかなを変えない。押したときにやること＝読みを取る→控えを更新→`placeMoras` で音符へ写す（音符は変えない）。
- **控えの鮮度**：`reading.forText !== text` なら控えを使わない＝「読みを反映」を押すまで印も高低も出さない（状態チップが「当て直していません」相当を言う＝文言は **「まだ反映していません」で確定**＝§31-11 の16 (d)・2026-08-02 追認済み）。
- **入口は2つ・中身は1つ**（architecture 2026-07-29「画面が直接使う口も同じ格で置く」）。読み取りの本体は `accent.ts` に1つ置き、MCP と HTTP はその入口。

**(e) `analyzeLyricFit` への渡し方＝`noteHl`**（`prosody.ts:332` の opts に追加）
「音符ごとの高低」＝`(0|1|null)[]`・**音符列と同じ長さ**。モーラ列をそのまま渡さない理由＝字余り（モーラ≠音符）が一級の状態になるため、添字で突き合わせると途中から1つずつずれた印が出る（いま直そうとしている `accent.ts:76` と同じ形の事故）。呼び側が `placeMoras` と同じ範囲規則で音符の並びに直してから渡す純関数 `noteHlOf(phrase, notes)` を music-core に置く。`noteHl` が来たら読みの輪郭はそれから直に作り、`accents`（語ごと核位置）からの組み直しはしない（`AccentEntry={kana,kernel}` は「モーラ1つだけ反転」した人の直しを表現できない＝`buildContour`（`prosody.ts:289`）が `analyzeMoras(a.kana)` で数え直すため）。**`noteHl` 未指定＝従来と完全一致**（property）。`accentSource` に `hand` を足す（`explicit|pyopenjtalk|hand|heuristic`）。

**(f) 同期性**：`planLyricMelody`（`lyricsPlan.ts:85`）と `genLyricMelodyCandidates`（`generate.ts:737`）の「同期・決定的」は壊さない＝読みが要る呼び側（`mcp.ts` の `gen_melody` ハンドラ）が**先に読みを解いて**かなを渡す（スライス7）。
...
| 場所 | 用途 | 実測の狂い |
|---|---|---|
| `apps/web/src/lyrics.ts:8` `splitMora` | 画面の流し込み | 「雨の日は」→4（正5） |
| `apps/api/src/lyric.ts:16` `splitMora`（同じもの） | `set_lyric` | 同上 |
| `apps/web/src/lyrics.ts:25` `moraLines` | 歌詞ネタ画面の音数表示 | 「君の声が聞こえて」→8（正11） |
| `packages/music-core/src/prosody.ts:46` `analyzeMoras` | 詞先メロの音数計画（`lyricsPlan.ts:107`）・`suggestLyricRhythm`・`buildContour`（`prosody.ts:295`） | 「今日は雨の日は靴が濡れる」→12（正14） |

**決め＝正は2つに分け、線を1本引く**
- **表記（漢字仮名交じり）のモーラ数の正 ＝ pyopenjtalk**（`LyricReading.moras.length`）。句がある場所の音数・「字余りN」は**全部これ**から出す。
- **かな列の分割の正 ＝ `analyzeMoras`（music-core）1本**。`splitMora`（2本）は music-core へ移して `analyzeMoras(kana).map(m => m.kana)` の薄い皮に置き換える（分割結果は同じ＝拗音結合・ー/っ/ん 独立の同規約。既存テストで bit 一致を確認）。`moraLines` も同じ皮を使う。
- **線＝「かな分割関数に漢字を含む文字列を渡さない」**。歌詞ネタ画面の音数表示（`moraLines`）は、表記にかな以外が混ざるとき「音数」を出さず「〜文字」に落とす（嘘の音数を出すくらいなら出さない）。読みを取れる画面（句のある編集画面）では pyopenjtalk 由来の数を出す。
- `analyzeMoras` 依存の残り（`lyricsPlan.ts:107`・`buildContour`）は、呼び側が読み（かな）を先に解いて渡す形へ（スライス7・§31-3(f)）。MCP 説明文の食い違い2箇所（`mcp.ts:964`「漢字は先に読み(かな)へ」は指示として正しいが実装が数え直す／`mcp.ts:1250`「漢字はpyopenjtalkが読む」は実装がそうなっていない）はスライス7で実装ごと直す。

### 31-5 字余りとメロが途中の言い分け＋音符を増やさない

言い分けの純関数を music-core に置く。分けられるのは**句が自分の範囲を持つから**（範囲は音符から導かない＝§31-0）。
...
export type PhraseStatus =
  ...
  | { kind: "ちょうど" };
export function phraseStatus(
  range: { start: number; beats: number }, moraCount: number, notes: Note[],
  opts?: { gapBeats?: number },
): PhraseStatus
```

- 「メロなし」は今スライスでは（音符を持つメロを開いているので）ほぼ出ないが、**関数には最初から入れる**＝置き場の裁定がどちらに転んでも（曲持ちの句／空メロ）この関数がそのまま言える＝分岐を関数の外に出す。
- 「1小節」の目盛りは引数（既定値）＝人が変えられる余地を塞がない（§31-11 の裁定待ち8）。
- **機械は詰めない・削らない・止めない**。直すのは人（表記を変える／メロを変える／「合わせる」を押す）。
- **`flowLyric` の呼び出し元を人の明示操作だけに限定する**：現在の呼び元は PianoRoll の流し込みボタン・`set_lyric`（`mcp.ts:953`）・詞先メロ（`generate.ts:762`）。**改訂（2026-07-30）＝句の面が配線された画面では、常設の「流し込む」ボタンとかな欄は退役**（既定で音符を割る先例はゼロ＝調査の定石3）。**2026-07-30b に歌詞パネルと同時に実施済み**（配線側は出ない／非配線側は従来どおり＝両方 `PianoRoll-lyric.test.tsx:241,247` で縛った）。
- **⚠ 「退役＝消す」は誤りだった（2026-07-30b 監査の指摘1-C → オーナー裁定で確定）**：この行自体を私が `5bc4a49` で書いた＝根拠は先例調査で、オーナー帰属は無い。**オーナーの実際の言葉は 07-29 の「流し込みも歌詞入力もあまり使わないから奥でいい」＝残して奥に置く**（一次資料＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §1 の(1)(3)）。2026-07-30b にオーナー裁定＝**「パネルの奥に戻す」**。
- **確定した形（2026-07-30b・実装済み）**：**常設からは外す／歌詞パネルの奥に置く**＝かな欄＋「流し込む」は `wired` でも消さない。
  - 置き場＝歌詞パネルの奥の段（かなを消す・韻律チェックと同じ引き出し）。ラベルは**「かな」**（正データは句＝表記なので、こちらは仮歌の写しだと読める語にする）。
  - **これで「音符を割る口」は配線画面にも在る**（`flowLyric`）。§31-11 の2（割る操作の存否）は研究の結論後に裁定＝**新しい操作を作るかどうかの話**で、既にある口を先に消してはいけない、が今回の学び。
  - 非配線の呼び側は従来どおり常設（後退ゼロ）。テストで両側を縛る。`set_lyric`・詞先メロ・句の面が配線されていない呼び側は従来どおり（後退ゼロ）。＝architecture「黙って実体を作らない」の受け＝**歌詞を打っただけで音符が割れることはもう起こらない**。

**音符を割る操作＝案A（候補提示版）で確定（2026-07-30c オーナー裁定）**。研究 `docs/research/2026-07-30-lyric-overflow-note-split-research.md` が「音楽的に成立する割り方を候補として出せる」見込みを立て（割り位置は `metricWeights` だけで決まり実測91.8%で裏取り／候補の実在性は `RHYTHM16_DATA` で言える）、独立監査を通過（実証再現・捏造なし）。オーナー裁定＝案A。**却下＝案B（切り捨て）・案C（貪欲法のまま口だけ残す）。**

- **候補生成は純関数 `splitCandidates`（`packages/music-core/src/lyricSplit.ts`）**。入力＝範囲内音符・句のモーラ（かな＋kind）・拍子（beatsPerBar/gridPerBeat/tempo）・オプション（コーパス照合は注入・families A/B/C・上限 j・下限音価・句末保護・語境界）。出力＝候補の配列（各候補＝割り位置の集合＋割った後の音符＋添える事実：割った音符数・コーパス既出か・CV・句末比・syncPerBar・特殊拍/語境界の当たり）。**1本化しない・既定は何もしない**（研究 §3）。
- **段1＝音符ごとに割り方を列挙**（断片内で `metricWeights` 最大の位置・同点は中点寄り。家族A＝素直／B＝16分1つ手前の食い・音符あたり最大1／C＝2番目に強い位置）。**段2＝合計 j=k をDFS**（句末音は既定 j=0）。**段3＝下限音価で落とす**。**段4＝柔らかい条件を「添える事実」として計算**（弾かない＝`sectionNoriLens` の warnings と同じ流儀）。
- **コーパスは注入**＝music-core はコーパスを持たない（`corpus?: (bar16:string)=>number|undefined`）。api 側が `RHYTHM16_DATA` を渡す。**裏取りは 4/4・16分格子のときだけ有効**＝`backedByCorpus`。**非4/4（6/8含む）は候補を出すが「拍の重みからの位置提案・実曲統計の裏はまだ」と正直にラベル**（研究 §4-4＝オーナーの問い「6/8できないのは？」への回答＝6/8はハード不能でなく裏取り統計だけが4/4限定）。将来の自作コーパス（P2）に非4/4曲が入れば同じ裏を持てる。
- **並べ方は2軸を併記**（点数で1本化しない・研究 §3-4）＝事実基準（割った音符数の少ない順・既出か）と好ましさ基準（コーパス頻度順・CVが帯の中央に近い順）。「よくある形」であって「良い形」ではないと明記。
- **`flowLyric`（最長音符を半分に割る貪欲法）は「候補提示版ができるまでの旧実装」**。画面の常設「流し込む／かな」は歌詞パネル奥に退役済み（上）。案A の候補UI が入ったら、画面の割る口は `splitCandidates` に置き換える。`set_lyric`・詞先メロ経路は句に覆われていない音符が対象なので当面 `flowLyric` のまま（後退ゼロ）。
- **`align_lyrics`（未実装・#82 の後続）を作るときは、返りに `splitCandidates` の候補（割り位置＋添える事実）を載せる形を予約**＝候補を返すのは「詰める」ではない（適用は人）。
- ~~**「っ」「ー」は音符を立てる（オーナー裁定）**～既存の `prosody.ts roleOf`・`lyricsPlan.ts isOnsetMora` が裁定と食い違う件は別途直す~~ → **2026-08-02 オーナー裁定で撤回＝「立てる」数え方は要らない**（§31-8）。`roleOf`・`isOnsetMora` は**実装どおりで確定**＝直す工事は消滅（研究 §4-2 穴3 も閉じ）。`splitCandidates` は**現状のまま据え置き**＝ここは「既にある音符へ句のモーラを1対1で写す（足りなければ割る）」割付側で、詞先の数え方とは別の層（促音が拍頭に来たら警告を添える現状の作りも維持）。
- **画面の姿＝プルダウンの組み合わせ（2026-07-30c オーナー提案＋独立監査で是正）**。実装＝`apps/web/src/components/LyricSplitComposer.tsx`＋純ヘルパー `apps/web/src/lyricSplitCompose.ts`。監査の是正を反映：
  - **本丸は割り方を「図」で見せること**＝各候補・組んだ形を onset 図（●元・○足した・·無し）で描く＝「散らす/詰める」が目で分かる（オーナーの「一言ラベルでは見分けられない」への答え）。
  - **音符ごとにプルダウン**（割れる音符だけ・句末や16分は出さない）＝`割らない / 2つに割る / 3つに割る`。**同じ個数で位置が複数あるときは選択肢に小図**（例「2つに割る（●·○·）」）＝位置を言葉（「強拍で」は誤り）でなく図で見分ける。
  - **初期は全「割らない」＝機械は事前選択しない**（事前選択＝隠れた1本化＝思想違反）。代わりに**「実曲でいちばん多い形を入れる（◯回）」ボタン**（頻度は事実であって判断ではない＝「1本化しない」と両立・「おすすめ」等の判断語は使わない）。
  - **合計＝余りモーラ数の制約**を「残り：あと◯／ちょうど」で示し、揃うまで適用不可。**リストは2軸（割り方が少ない順・実曲に多い順）を併記**（点数で1本化しない）・**リストの候補をタップでプルダウンに読み込む**（ゼロから組まず微調整から入る）。
  - **ライブAPIは作らない**＝有効な組み合わせ＝候補集合なので、開時1回 `POST /music/split-candidates` で全候補（上限600）を取り、以後クライアントで局所照合（デバウンス・レース問題ごと消す）。**メロが変わったら候補は無効化**（腐り防止）。促音/語境界の警告・非4/4縮退も引き継ぐ。
  - **割り方の図＝「メロの概形」（2026-07-31 オーナー要望＋設計ワークフローで統合）**＝ASCII の `●○·` を廃し、`MiniRoll`（一覧の縮小ピアノロール）と同じ **SVG 矩形DNA（pitch×time）＋ドラムの拍格子**で描く。実装＝`SplitRoll`（`apps/web/src/components/SplitRoll.tsx`）＋純関数 `splitRollGeom`（`apps/web/src/lyricSplitCompose.ts`・`onsetFigure` と同じ orig/added 述語＝テストで1対1一致を縛る）。
    - **元の音符＝`var(--k-melody)`（青）／割って足した頭＝`var(--k-lyric)`（黄）＋縦ティック**。`--accent`（青）は `--k-melody`（青）と見分け不能なので採らない（3案が独立に同一結論）。意味も「余りモーラ＝歌詞の都合で足した頭」で歌詞色が正しい。
    - **時間軸は等尺**（originBeat 基準・`splitRollGeom`）＝全候補で拍列が縦に揃い、割りはピッチ不変なので**輪郭は全候補で同一・動くのは黄の位置だけ**＝縦スクロールで候補が見分けられる（初見レビューの「一言ラベルでは見分けられない」への答え）。ピッチは句で1回だけ正規化（origNotes の lo/hi を配る）。拍ガイド線は小節=濃/拍=薄（`.rhythm-cell.bar/.beat` の実hex）。非4/4は `total=bars*bpb*gpb` と slot だけで描くので無破綻。
    - 差し替えは候補リスト行と「組んだ形」プレビューの2箇所。**プルダウンの `<option>` 内の小図だけ ASCII 続投**（HTML の option に SVG は入らない）。まだ何も選んでいないときのプレビューは「割らない形（元のメロ・全青）」を出す＝空箱にしない。
    - **色＝黄で確定（2026-08-02 オーナー裁定）**：現状（元＝青／足した頭＝黄）の実機スクショ2枚と、青系代替版のモックを並べて見せて **「良い」＝黄を維持・青系は却下**。有力仮説で先行実装していた形がそのまま正になった＝**この色は動かさない**。他のミニ表示（`MiniRoll`/ドラム）との整合も満たす。
- `align_lyrics`（未実装・design「plan intent カタログ拡張(#82)」の後続に置いてある）を作るときは、返りをこの言い分けの語彙に合わせる（詰める・削るは機械がしない）。

### 31-6 手で直せる範囲の持ち方（`LyricPhrase.edits`）

**現況**：人の直しの置き場は無い。音符のかな手打ち（`apps/web/src/lyrics.ts:70` `setSyllable`）は `Note.syllable` に直書き＝機械が写し直すと消える。
...
- **表記が変わったときの付け直し（純関数・テスト3通り）**：同じ位置に同じ文字列→そのまま／位置がずれても句の中にその文字列が1つだけ→そこへ／見つからない・2つ以上→ `detached` を立てて人に見せる（「この直しの付け先が分からなくなりました：〔今日→きょう〕　付け直す／捨てる」）。**黙って捨てない・黙って別の語に付けない。**
- **機械が読み直しても直しは消えない**：控え（`reading`）を引き直したあと、`edits` を適用した「効いている読み」を純関数 `effectiveReading(phrase)` で導く。`placeMoras`・`noteHlOf`・仮歌はこの適用後を読む。＝architecture「人の手直しが機械の読み取りより上」。
- **読みを直した語の高低（当面の形）**：機械はその語の高低を作り直せない＝その語のモーラは hl 不明（高低表示は空白・印は出さない）とし、「手で直した」と出す。人が続けてタップで高低を与えられる（hl の edit になる）。§31-11 の裁定待ち7。
- 出所表示：直しが1つ以上効いている句は `accentSource: "hand"`（§31-3(e)）。
- 曲から潜って句に覆われた音符へ詞モードでかなを打った値は `kind:"kana"` の直しとして句へ取り込む。ネタ帳から直接開いた（句の無いメロの）詞モードは従来どおり `Note.syllable` へ。

**実装の現在地（2026-07-29・スライス4の1本目）**
- **入った**＝データと純関数の層（`packages/music-core/src/lyric.ts`）：`effectiveReading` が `read`／`kana` の直しを重ねる（機械が読みを引き直しても消えない）・`noteHighLow` が `hl` の直しを重ね、**読みを手で直した語は高低を「分からない」**にする・`reattachEdits`（付け直し3通り）・`kanaEditForNote`（音符のかな手打ち→直し1件）・`upsertEdit`（同じ貼り先は後が勝つ）・`hasHandEdit`。
- **入った**＝画面の配線2本：詞モードで句に覆われた音符へ打った値が**直しとして句に残る**（従来は機械の写し直しで消えていた）／表記を直すと**直しの貼り先が付け直され、付かない直しも捨てずに `detached` が立つ**。
- **まだ無い**（次の1本）＝①**読みを語ごとに直す口**（語をタップして読みを打つ）②**高低をタップして反転する口**（高低の表示自体が画面にまだ無い）③`detached` の直しを人に見せて「付け直す／捨てる」を選ばせる面 ④出所表示 `accentSource:"hand"`（`hasHandEdit` は用意済み・画面が読んでいない）。
  ＝**いまは「打ったものが消えない」ところまで**。人が機械の読み間違いを直す動線そのものは次。

### 31-7 メロの自動保存が句を落とさないこと

**現況**：`apps/web/src/useNetaEditor.ts:327` `savePatch()` は kind ごとに content を**キー決め打ちで作り直す**。メロ分岐（:334）は `{notes, program, ...singContent(), ...(feel ? {feel} : {})}`＝**ここに無いキーは 600ms 後の自動保存（:505）で消える**。同じ理由で `feel` が落ちていた前例と直し方が同ファイルにある（:328 の C-6 コメント＝「content 再構成に feel を透過。未指定はキーを生やさない＝bit一致」）。

**決め（feel の直し方に倣う）**
1. 初期化：`const [lyric, setLyric] = useState<LyricLayer | undefined>(() => lyricOf(neta.content))`（`lyricOf` は `apps/web/src/music.ts` に `feelOf`（:22）と同型で足す）。
2. メロ分岐（:334）：`{ content: { notes, program, ...singContent(), ...(feel ? { feel } : {}), ...(lyric ? { lyric } : {}) }, … }`。**lyric 未定義＝キーを生やさない＝既存データと bit 一致**（自動保存の空振り差分を作らない）。
3. テスト（先に赤）：`content.lyric` を持つメロを読み→音符を1つ動かす→`savePatch` の返りに lyric が残る／lyric 無しのメロ→返りに lyric キーが無い。
4. container 分岐（:347・content を返さない）は**触らない**＝(あ)を採った場合の曲の書き込みは「歌詞だけを更新する専用の口」で別に作る（スライス5・§31-9）。ここで触らないことが置き場非依存の担保。
5. **透過する範囲＝音符を持つ kind すべて**（melody / bass / counter / riff）。詞を書く欄が出るのは今のところ**メロだけ**（`enableLyric`）だが、透過をメロだけに絞ると、何かの経路で `content.lyric` を持った他 kind のネタを開いて保存したときに**歌詞が黙って消える**。この節の目的は「落とさないこと」なので、広く通す側に倒す。ハモ（counter）に詞を書けるようにするかは裁定待ち（§31-11）＝そのとき欄を出すだけで、データの通り道は既にある。

### 31-8 「っ」「ー」に音符を立てるか＝**立てない（従来）で確定**

**✅ 決着（2026-08-02 オーナー裁定・旧裁定 Q7「立てる」はオーナー自身が撤回）**。原文＝「**C1 促音伸ばし音はいらない、打ち込むときに工夫するので。**」＝**「っ」「ー」にも音符を立てる数え方は要らない**。理由＝打ち込むときに人が工夫する（機械が音符を増やして先回りする筋ではない）。

**確定した形**：詞先メロの計画は**長音ー＝直前へ延長・促音っ＝詰め＝音符を立てない**（`isOnsetMora`＝`apps/api/src/music/lyricsPlan.ts`）＝**従来のままが正**。`prosody.ts` `roleOf`（`suggestLyricRhythm` の候補 role）も同じ＝**実装どおりで確定**（もう「裁定と食い違う側」ではない）。#13d の受け入れ条件「モーラ⇔音数 100%一致」の「モーラ」＝**オンセット（っ/ー を外した数）**で確定。

**この裁定でやめたこと**：2026-08-02 に一度入れた opt-in `standSpecialMoras`（`planLyricMelody` の旗・MCP `gen_melody` のパラメータ・返りの `lyricPlan.standSpecialMoras`）は**同日撤去**＝使う者がゼロの死にノブを残さない。促音「っ」を VOICEVOX にどの文字で送るかの確認事項も、**音符を立てない以上は消滅**（`sing.ts` は従来どおり syllable をそのまま送る）。

**触らないもの**：既存の**割付**側（`splitMora`・`flowLyric`・`splitCandidates`・仮歌 `sing.ts` の `syllable:"ー"`＝母音継続）は現状のまま＝ここは「既にある音符へモーラを1対1で写す」話で、**今回の裁定（新しく音符を立てる数え方）とは別の層**。2つの流儀が割れている扱いはこれで閉じ＝**直す工事は無い**。

### 31-9 画面をどこに足すか＋歌詞の置き場（(い) 確定・空メロの見た目も確定）

**現況**：ルーターは無く、`apps/web/src/App.tsx:718` からの三項演算子の連鎖で切り替え。**新しい全画面には必須作業が3箇所**＝`closeTop`（:107 付近・戻る優先順に足す）・`anyOpen`（:126-128）・分岐本体（:718〜）。過去にここで2回不具合が出ている。最新の実例＝骨格の机（`deskTarget` state・:78/:107/:718）。

**決め**
- **スライス1〜4は新しい画面を足さない。** メロを開いて詞を書く面＝既存のメロ編集画面（NetaDialog→KindEditorBody→PianoRoll）の中。**App.tsx の3箇所に触らない**＝戻る対応の不具合の再発面を増やさない。
- **歌詞まわりの姿（2026-07-30 改訂・根拠＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §3）**：現状の常設6段（歌詞欄・読み行・読みを取り直す・かな欄・流し込む/クリア・韻律トグル＝実測約220px。描画は `PianoRoll.tsx:402` 以降）を畳み、**常時は「歌詞行」1段（約40px）だけ**＝句のテキスト1行（あふれは省略・空なら「歌詞を書く」）＋状態チップ1個。書く/直すときは歌詞行タップで**歌詞パネル**に展開＝表記の入力欄（複数行）＋読み表示＋**「読みを反映」**（§31-3(d)）＋奥の段（読みを取り直す・かなをクリア・韻律チェックON/OFF＋凡例。音符を割る操作は研究の結論待ち＝§31-5）。かな欄＋「流し込む」は句の面が配線された画面から退役（§31-5）。**2026-07-30b に確定＝実装済み／2026-08-02 にオーナー実機スクショで追認＝レビュー済み・採用確定**（§31-11 の16。不在中に推しで先行した (b)(c)(d) がそのまま通った＝「未レビュー・戻せる」の札は外れた）。確定した姿を以下に書く。
  - **常時1行（歌詞行）**＝句のテキスト1行（あふれは省略・空なら「歌詞を書く」）＋**状態チップ1個**＋**「読みを反映」は未反映のときだけ出す**（(b)＝案C）。理由＝表記を1文字直すと控えが古くなり（`lyric.ts:316` `isReadingStale`）チップが消える＝**手が要る状態こそ画面が空白になる**ので、そこにだけボタンを出す。5状態のうち人の手が要るのは「未反映」だけ（字余り・字足らず・メロが途中は報告）。
  - **歌詞パネル（歌詞行タップで展開）**＝表記の入力欄（複数行）＋読みの様子＋「読みを反映」＋**奥の段**＝読みを取り直す・**かなを消す**・**韻律チェックON/OFF**（(c)＝案B＝常設から奥へ移し、**OFF を localStorage で覚える**。既定は ON。移すだけだと `useState(true)` のせいで開き直すたび ON に戻り毎回2タップになる）。音符を割る操作は研究の結論待ちなので**まだ置かない**（§31-5）。
  - **文言（(d)）**＝状態チップ：ちょうど／字余りN／**字足らずN**（`あとN音` を廃止＝字余りと対の詩歌の語で向きが読める）／メロがまだ途中／メロなし／**まだ反映していません**（新設＝ボタン「読みを反映」と動詞をそろえ、押すべきボタンの名前がチップから読める）／歌詞が空なら「歌詞を書く」。読みの様子：`読みを取ります`→**まだ読みを取っていません**（機械が勝手に動くと読めない形へ）。操作：`クリア`→**かなを消す**。「読みを反映」は据え置き（2026-07-29 裁定）。
  - **詞モードの見せ方＝2026-08-02 オーナー裁定で確定（§31-11 の16 (a)）**＝原文「**かなチップは破綻するからいや。ボタンは歌詞のまま。送り方は候補2がきれい**」。
    - **モード行4つ目のラベルは「歌詞」**（`KindEditorBody.tsx`）＝**改名しない**。入口はモード行のボタンのまま・一等地維持（ダブルタップ/長押し・入口移設はいずれも却下/不採用）。
      ※ 本書はここに「(a) 案B＝『詞』→『1音ずつ』で確定」と書いていたが、実ラベルは `3cd5bf3`（2026-07-31）で既に「歌詞」＝**記述側のドリフト**。2026-08-02 の裁定で「歌詞」が正と確定＝是正済み。
    - **送りバー＝候補2**＝詞モードの固定バーを「**◀｜音名・拍｜かな input｜▶｜✕**」に。▶＝確定して次へ（旧「確定→次」）・◀＝確定して**前へ**（誤送りの復帰）・✕＝閉じる（打ちかけは確定してから）。
      キー＝**Enter/Tab=▶・Shift+Enter/Shift+Tab=◀・Esc=✕**（`preventDefault`・IME `isComposing` ガード維持）。input は `enterKeyHint="next"`。対象音符が画面外なら**自動スクロールで追う**。未選択時ヒント＝「音符をタップ→かなを打つ→Enterか▶で次へ」。
    - **物理キーは近道であって正の口ではない**＝◀▶✕のボタンが正（「スマホにタブキーはないんだわ」への回答）。**かなチップ帯＋まとめ打ちは却下**（作らない）。
    - **文脈行（バー1行→2行）は今は足さない**＝1行に収める形が決まった。足すなら別スライス。
  - 読み・高低・かなの直し（スライス4後半）もこのパネルの中。
  - **初見レビュー→Fable監査→修正（2026-07-31・Pass 2）**：初見の一般人3人（自由/デザイン/導線）に画像だけ見せた感想＝「割る画面が情報過多・『実曲』等が謎語・確定ボタンが選択肢の途中・入口が弱い」→ Fable が具体案を出し、ワークフローで実装。反映＝(1) **文言**：モード行「1音ずつ」→「歌詞」・「いじる▾」→「ツール▾」（SectionEditor/TinkerSheet も）・「長さ(分)」→「音の長さ」・「実曲」→「既存曲」・「流し込む」→「音符に割り当てる」・「韻律チェック」→「アクセントの印を表示」・「ちょうど」→「過不足なし」（句ステータスと割る画面で統一）。(2) **画像1の導線**：歌詞1行に開閉キャレット▾＋空なら「歌詞を書く」・字余りチップをタップで割る機能へ直通・空の格子に「タップで音符を置く（いまの道具：描く）」・ツールバーの主従（動作＝主/音の長さ＝従）。(3) **割る画面のレイアウト**：確定「この形で音符を割る」を候補リストの下（最下部）へ・拍プルダウンを「▸拍ごとに細かく変える」に畳む・かな欄と奥の段を「そのほか▶」に畳む・「◯回」は下のリストだけに（種ボタンは残し重複解消）。**コンセプト死守**（2軸併記・事前選択なし・SplitRoll 図は不変）。実機360px検証済（ツールバー収まり・空格子ヒント・確定最下部）。web 1227緑。
- **スライス5（通しで読む面）だけが新しい全画面**＝骨格の机の形に倣う：`lyricViewTarget` 的な state を1つ・closeTop の優先順は `deskTarget` と同格の位置・`anyOpen` に足す・分岐は :718 の並びに1本。表示するパーツの選択は localStorage（既存の流儀。サーバに持つ仕組みは無い）。画面の**姿**は当初モック裁定待ちだったが、**2026-08-02 のオーナー実機スクショ追認で確定**（下の実装現況の「有力仮説」がそのまま正）＝設計で決めるのは器だけ、という当初の切り分けはそのまま成立した。
- スマホ幅の土台は既にある（`apps/web/src/useIsMobile.ts:5` の `max-width:820px`）。UI に出す言葉は言い分けの語彙（§31-5）そのまま＝「字余り2」「あと3音」「メロが途中」「メロなし」「読みが取れませんでした」。**比喩・造語を使わない**（原文「強い言葉はいらない。イメージを誤るので」「恒等の意味はわからん。内輪語になってない？？」）。

**歌詞の置き場＝案(い) で確定（2026-07-30 オーナー裁定）**

オーナー裁定＝**「空のメロディを入れてそこに入れておく。ただし既存メロディなどからのメロディのコピー機能を代わりに充実させる」**（2026-07-29 の傾き「空のメロディを作る方のが無難な気がするな、その代わりコピーとかを充実させるとか」の確定）。
- 帰結（当初）＝詞先の句も必ずメロの `content.lyric` に入る＝歌詞の置き場は一種類。曲・セクションは歌詞を持たない。通しで読む面の併合は配置ツリー1系統。

**⚠ 置き場の一段の緩和＝「歌詞の穴（□プレイスホルダ）」のために曲が歌詞を持てる（2026-07-31 オーナー裁定）**
- きっかけ＝オーナー「全貌を見て"ここに歌詞が要る"と穴を開けて埋める導線が無い。理想は**文字だけの歌詞一覧に□（四角いプレイスホルダ）**を並べる。**入れ物は曲がふさわしい**＝歌詞箱エンティティを作るより**曲に歌詞をもたせるほうが整合的**」。
- 裁定＝**曲が `content.lyric`（メロと同じ `LyricLayer`）を持てる**＝そこに **□の穴（text 空の句）** と **詞先の下書き（メロがまだ無い歌詞）** を置く。**本物の歌詞がメロに載る不変は保つ**（曲が持つのは穴・下書きだけ）。＝「曲・セクションは歌詞を持たない」を「**曲は歌詞の穴・下書きを持てる**」に一段だけ緩める。
- 帰結の更新＝**通しで読む面の併合は「曲持ちの句＋メロ持ちの句」の2系統を時間順に**（案(い) で消したはずの枝が穴のために戻る＝理想の対価として呑む）。□＝text 空の句（曲でもメロでも）＝どちらも□で見せて、埋めると文字になる（残る）。
- **(い-c) との関係＝穴には「書いた瞬間にメロが生まれる」を適用しない**（有力仮説・2026-07-31）＝**□に書いても曲が歌詞を持つだけ・メロ化は別の後の操作**（詞先＝歌詞が先、メロは後、に素直）。(い-c) 遅延生成（書いた瞬間にメロを生む）は「メロ側で書く」経路に残す／穴（曲側）はメロを強制しない。
- 実装＝`song.content.lyric` に句を持つ／`collectLyricRows` が曲持ちの句とメロ持ちの句を時間順に併合／□の作成＝「＋ここに歌詞」で曲へ空の句を足す／□を埋める＝曲の句へ text を書く（`updateNeta`）。
- **メロ化の橋＝実装済み（2026-07-31）**＝曲の句（□/詞先の下書き）の行に「メロにする」＝**メロ作成（notes空・content.lyric に句）＋配置を1操作((い-c) 遅延生成)＋曲側から句を外す**＝詞先→曲先。失敗で `deleteNeta`＝孤児なし。実機検証＝「詞だけ先に」→メロにする→曲の lyric 句数0・セクションに melody「詞だけ先に」が生成配置。□の削除口も（曲の句を消す）。
- **＋パートを足す＝実装済み（2026-08-01）**＝オーナー「歌詞は各パートに入れられるが、パート（Aメロ/Bメロ）を追加する口が無い＝Cメロしかない曲にA,Bメロの置き場がほしい」。→ 通しの面の末尾に「＋パートを足す」＋役割メニュー（`ROLE_KEYS`＝Intro/Aメロ/Bメロ/サビ/落ちサビ/大サビ/Cメロ/間奏/Outro）＝**新セクションを曲の末尾に開く（作成＋配置1操作・失敗で deleteNeta 孤児なし）**＝歌詞を入れる前に置き場を作る（穴の粒度 曲＞セクション＞句）。`collectLyricRows` は `songNextBeat`（曲の末尾拍・次小節頭へ丸め）を返す。追加は末尾。**並べ替え＝実装済み（2026-08-01）**＝各パート見出しに ↑/↓＝末尾に足した Aメロ/Bメロ を Cメロ の前へ動かせる。仕組み＝FormStrip と同じ「カード順→position 前置和射影」（`stripPositions`）＝順を入れ替えて全セクションの position を振り直す。**⚠ placeChild は追加（compose_edge は同 child を複数置ける）＝先に `removeChild`（位置なし＝全消し）してから振り直す**（そうしないと重複配置になる・監査で捕捉）。実機検証＝Cメロだけ→A,Bメロを足す→↑で A/B/C 順に（重複なし・小節番号も更新）。web 1234緑。**＝スライス5は 読む＋推敲＋インライン編集＋□プレイスホルダ＋パート追加＋詞先↔曲先の橋 が一通り動く。**
- **コピー機能の充実はスライス6の範囲**＝来歴・比較とあわせて中身を詰める（何を「充実」と呼ぶかは §31-11 の裁定待ち3〜5と連動）。
**空メロの見た目＝(い-c) 遅延生成で確定（2026-07-30 オーナー裁定）**

オーナー裁定＝**「なにか書いたらメロが生まれるようにして」**＝**空のうちは実体を作らない**。裁定材料＝`docs/archive/2026-07-30-empty-melody-appearance-decision.md`（実機CSSのモックで5案を比較）。
**⚠ 裁定の範囲を切り分ける（2026-07-30b 独立監査の指摘1-A で訂正）**：オーナーの原文は上の1文だけで、しかも直前に
**「何いってんだかわからないから」**と言っている＝提示した案E の説明文（下書きの破棄・確定の合図を含む）は**読まれていない**。
よって**オーナーが決めたのは「空のうちは実体を作らない／何か書いたら生まれる」まで**。下の 1. のうち
**下書きの持ち方・「確定」という操作の存在・書きかけの破棄は私の派生した設計判断＝未レビュー**（§31-11 の17 に起票）だった。
→ **2026-08-02 に実機スクショで追認＝派生分もそのまま確定**（オーナー「B1-3 は良い」）＝**§31-11 の17 は決着で閉じた**。確定した粒度＝①生まれる合図は**句の確定**（1文字目ではない）②**書きかけは残さない**（下書きはクライアント state）③合図の形は **Enter（Shift+Enter は改行）か確定ボタン・Esc で取り消し**（`LyricOverview.tsx` の `editorBar`）。あわせて□の穴は (い-c) の対象外＝「＋ ここに歌詞」は曲に穴を開けるだけでメロを作らない、も追認された。

- **決まったこと（オーナー裁定の範囲）**：
  1. **(い-c) 遅延生成**＝**空のうちは実体を作らない。何か書いたらメロが生まれる。**
     - ここから私が派生させた形（**2026-08-02 実機追認で確定＝§31-11 の17 は決着**）＝詞先の枠は通しで読む面の中の下書き（クライアント state）で、
       **句を確定した瞬間**にメロを作って同時に配置する／**書きかけで離れたら下書きは消える**。
       ※（決着前の留保・記録として残す）「何か書いたら」は**1文字目**とも読める＝破棄も「確定」の存在も、オーナーの言葉からは決まらなかった。
       ※（同）監査の指摘（推測）＝破棄は §31-7「自動保存が句を落とさないこと」と「セクション単位で往復しながら書く」作業像と逆向きになりうる。
       → **2026-08-02 の実機追認で「確定で生む・書きかけは残さない」が正と決まった**（実機で触った上での「良い」）。§31-7 との整合は**保存されているのは句・下書きは未保存の入力途中**という区別で立つ。
  2. **カードは句を出す**（これはオーナーが選んだ案B の中身＝「Bかなあ」の発言に対応）：音符0個のメロのカードに**句の表記**を出す（`MiniRoll.tsx:51` は音符0個で `null` を返し、カード見出しは `title`／`text` しか見ない＝いまは中身があっても空白のカードになる）。**遅延生成でも「句はあるが音符が無いメロ」は必ず生まれる**（それが詞先の主役）＝この表示は遅延生成の有無と無関係に要る。
- **却下**：(い-a) 一覧から隠す＝仕組みが増えるだけで**ゴミを隠すのみ**（減らない）。B のあとで困ったら足せるので急がない。(い-b) 共有の空メロ1つが産む＝**機械が黙って実体を作る**＝architecture「黙って実体を作らない」と正面衝突（(い-c) は人が句を書いたことが合図なので衝突しない）。何もしない＝空白カードが残る。
- **制約（実コード）**：配置は `compose_edge.child_id TEXT NOT NULL REFERENCES neta(id)`（`apps/api/src/db.ts:38`）＝**実体の無い枠は曲に置けない**。ゆえに遅延生成は「作成＋配置を1操作でまとめる」形にしかならない。**引き換え＝「枠だけ先に並べて構成を眺める」使い方は無い**（並べるには何か書く）。確定の合図（改行/完了）をどこに置くかはスライス5の実装で決める。
- **いま決めた利得**：通しで読む面も「空メロを置く」操作も**まだ1行も実装されていない**（2026-07-30 に grep で確認）＝作り直しゼロ。さらに尺の手当ての bit 一致の地雷が**構造で消える**（下の §31-10 スライス5）。

### 31-10 スライス計画（テスト先行・触るファイル・完了判定）

ブランチは切らない（main 直）。各スライスは UI＋API＋中身を薄く一緒に通す。既存機能は1つも止めない。

**スライス1＝自分の曲のメロに漢字仮名交じりで詞を書くと、正しい読みで仮歌が鳴る**
- 先に書くテスト（赤）：(1) music-core `lyric.ts` の句の形・「控えが古いか」（`forText !== text` なら使わない）・`effectiveReading`（スライス1では edits 空でも関数の口は作る）／(2) `placeMoras` の契約1〜3（syllable を除いた音符列が入力と完全一致＝property／範囲外不干渉／余った音符は "ー"／余ったモーラは載らない）／(3) `apps/api/test/accent.test.ts`＝複数行を stdin 一括で渡し1回の起動で全行返る・返りに words(surface/read/pron/mora_size) が入る・門番2つ・「今日は雨の日は靴が濡れる」が14モーラ・pron から「’」が除かれる（venv 未導入環境ではスキップ）／(4) `POST /music/reading` の正常系（texts 複数）・不正入力 400・Python 失敗 502（スクリプトパスを env で壊して検証）／(5) `apps/web/test`＝`savePatch` が `content.lyric` を落とさない・無ければキーを生やさない。
- 触るファイル：`packages/music-core/src/lyric.ts`（新設）＋`index.ts`（re-export）／`apps/audio/accent.py`（words 追加）／`apps/api/src/accent.ts`（stdin 一括 `extractReadings`）／`apps/api/src/http.ts`（`/music/reading`・case 内 502）／`apps/web/src/useNetaEditor.ts`（:334 lyric 素通し・lyric state）／`apps/web/src/music.ts`（`lyricOf`）／`apps/web/src/api.ts`（reading 呼び）／`apps/web/src/components/PianoRoll.tsx` ほか（表記欄・読みを取り直す・`placeMoras` で音符上のかな表示。仮歌は既存経路のまま＝`syllable` を読む）。
- 完了の判定（オーナーが自分の曲で）：手持ちのメロを開き表記欄に漢字仮名交じりで1行打つ → 音符上のかなが読みどおり（1字1音に化けない）→ ▶ でその読みのまま歌う → **音符の数・位置は打つ前と1つも変わっていない** → 閉じて開き直しても表記が残る → 数秒放置（自動保存）後も残る。読みが取れない環境では「読みが取れませんでした」＝表記も音符もそのまま。

**スライス2＝印（赤黄）と高低が表記由来になる**
- テスト：`analyzeLyricFit` の `noteHl` 有り／無し（**無し＝従来と完全一致**を property で）／`noteHlOf`（モーラが余る・音符が余る・範囲外 null）／表記のある句では9語辞書に落ちない・読み取り失敗時は印を出さない。
- 触る：`prosody.ts`（noteHl・accentSource hand）／`apps/web/src/lyricFit.ts`／`PianoRoll.tsx`／`mcp.ts`（`analyze_lyric_fit` の句対応は任意＝id のメロに句があれば noteHl を組む）。
- 完了：自分の詞で、読みの下がり目をメロが上がる箇所に赤が出る（9語に頼らない）。高低の表示と印が食い違わない。

**スライス3＝字余りとメロが途中を言い分け、詞を打っても音符が増えない**
- テスト：`phraseStatus` の5状態すべて／`flowLyric` の呼び出し元が「合わせる」経路だけ（PianoRoll の onChange 経路のユニットで確認）。
- 触る：music-core（`phraseStatus`）／`PianoRoll.tsx`（「合わせる」ボタン・状態表示）。
- 完了：詞を書き足しても音符が1つも増えない。「字余り2」「あと3音」「メロが途中」が句ごとに出る。「合わせる」を押すと従来どおり割れて1対1になる。

**スライス4＝読み・高低・音符のかなを手で直せる**
- テスト：付け直し3通り（そのまま／1つだけ見つかる→移る／見つからない・2つ以上→detached）／直しが効いた句の `accentSource` が hand／控えを引き直しても直しが消えない（`effectiveReading` が同じ値を返す）。
- 触る：music-core（edits・付け直し・`effectiveReading` の本実装）／`apps/web`（読みの欄・高低タップ反転・出所表示・detached の「付け直す／捨てる」）／保存は §31-7 の素通しに乗る（新しい口は不要）。
- 完了：機械が読み違えた語を直すと、直後の高低・印・仮歌が直った読みで動く。表記を1文字足しても直しが剥がれない。

**スライス5＝詞先の句と、通しで読む面（置き場＝案(い)＋空メロは (い-c) 遅延生成＝2026-07-30 全部確定・裁定待ちなし）**

> **実装の現況（2026-07-31・読む面の第一版＝設計ワークフローで姿を統合し実装）**：**通しで読む面（LyricOverview）を実装**＝全画面（骨格の机と同格・App.tsx に `lyricViewTarget` を routing 4箇所＝state/closeTop/anyOpen/分岐/FAB条件・入口は曲の SectionEditor ヘッダ「歌詞を通しで読む」）。姿は **2026-08-02 のオーナー実機スクショ追認で確定**（**態A既定＝1句1行・表記が主役・小節番号＋事実チップだけ／読み・高低・メロ帯は[表示]シートでトグル・localStorage `cm-lyric-view-parts`／メロ帯は MiniRoll 流用**。有力仮説で先行実装した形がそのまま正になった）。句の時間順併合は純関数 `collectLyricRows`（`apps/web/src/lyricOverview.ts`・TDD＝song→section→メロを position 累積で辿り時間順1系統・セクション見出し・拍→小節・字余りN等の事実）。実機360px検証済（既定=静かな読む面／全部オン=読み+高低￣＿+メロ帯青輪郭）。web 1231緑。**「読む面」を「書ける面」にした（2026-07-31 続き実装）**＝①既存句の**表記インライン編集**（行タップ→textarea→Enterで確定・空にすると句を消す＝PianoRoll と同じ約束・`getNeta`で最新を読み`updateNeta`で保存し再取得）②詞なしメロの行タップで句を新設（`set_lyric`相当）③**(い-c) 遅延生成の書き口**＝セクション末「＋句を足す」→確定で**メロ作成（notes空・content.lyric に句）＋配置を1操作**（`createNeta`→`placeChild`・失敗したら`deleteNeta`で孤児を作らない）。純関数側は `OverviewSection` に `netaId`/`nextBeat`（配置先と次拍）を追加。実機360px検証＝既存句を打ち直し→保存／＋句を足す→行が増える、を通した。**尺手当て・配置位置も実装（2026-07-31）**＝④**尺手当て**＝`sectionContext.ts` の leaf 尺計算（`childDur`/`contentDur`）を「音符0個でも句があれば句の尺（`phrase.start+beats`）を尊重」に（新設 `leafDur`）。**句なしの音符0個ネタは従来どおり `ctx.BPB`＝既存の音符0個50件は不変＝bit一致を条件式で保証**（テストで「句なし=1小節／句あり=句の尺」を縛る）。`lyricOverview.collectLyricRows` の `childDur` も同手当て。⑤**配置位置**＝＋句を足すの `nextBeat` を次の小節頭へ丸める（新句が小節途中から始まらない）。⑥**音符0個メロのカード見出し**＝`NetaList` の `phraseLabelOf` フォールバックで既に実装済み（句を見出しに）＝実害潰れ済み。web 1233緑。**スライス5＝読む＋推敲＋インライン編集＋遅延生成＋尺手当てが一通り動く。** **姿と遅延生成の粒度（§31-11 の17）＝2026-08-02 のオーナー実機スクショ追認で確定**（既定の静かな姿／[表示]全部オン／「＋ ここに歌詞」の3枚を見て「良い」）＝有力仮説で先行した形がそのまま正・**作り直しなし**（意味/感情メモは踏み込みなので引き続き未実装・シートにも出さない）。**句が意味検索の索引に入った（2026-08-02）**＝`apps/worker/src/cm_worker/search.py` の `_content_text` が kind を問わず `content.lyric.phrases[].text` を拾う（メロは音名＋句、曲コンテナの□/下書きも同じ形なので拾える）＝**title も text も持たない詞先メロを「書いた言葉」で探せる**（実測＝句だけのメロが該当句の検索で1位・言い換えでも1位）。**注意＝`neta_embedding` は `neta.updated` をキーにした派生キャッシュ**なので、索引テキストの作り方を変えても既存ネタは再埋め込みされない（cm-search の再起動でも同じ）＝**該当行のキャッシュを消して初めて反映される**（新規・編集後のネタは自動で乗る）。

- 共通（どの案でも作る）：通しの全画面（§31-9・骨格の机の形）・`getComposition`（`apps/api/src/core.ts:280`）で曲1回読み・メロの拍→曲の拍換算（position は拍）・同じセクション2回置きでも2個目は空にならない（`core.ts:283` で 2026-07-04 修正済み。古い前提の注記が `apps/web/src/formStrip.ts:77` の注記に残っている＝ここで始末）・表示パーツ選択は localStorage。
- **案(あ)（曲が持つ）＝2026-07-30 裁定で不採用**。以下は記録（採っていたら要った作業）：`song.content.lyric`（同じ `LyricLayer`・範囲は曲の拍）＋ 本書「content の厳密スキーマ」の「合成(section/song)は content でなく compose_edge」への**例外の明記**（＝スペックを先に直す）／曲の**歌詞だけを更新する専用の書き込み口**（自動保存の container 分岐は content を返さないため既存経路に乗せない）／通しの併合が**2系統**（配置由来の句＋曲持ちの句を時間順に）／「メロが来たので句を移す」操作（最初は「この区間にメロが来ています」→人が『このメロに載せる』で移す。黙って動かす側は後から足せるが、黙って動いたものを戻す仕組みは後から剥がしにくい）／新たな裁定＝曲の構成を並べ替えたときメロの無い歌詞は付いてくるか・その場に残るか。
- **案(い)（空のメロを置いて書く）＝確定（2026-07-30）**：「空のメロを作って置く」操作（既存の作成＋配置の組み合わせ・1ボタン程度）／**尺の手当てが必須**＝音符0個のネタは尺計算で1小節と数えられる（`apps/web/src/sectionContext.ts:81,88`＝音符が無ければ `ctx.BPB`）ので、空メロは `bars` ヒントを尊重する形へ（§31-0 の範囲既定と同じ向き）／併合は配置ツリー**1系統**＝曲の書き込み口・句を移す操作・上の例外の明記は**全部不要**。
  - **⚠ 尺の手当ては bit 一致にならない（2026-07-30 実データで確認）**：`data/cm.sqlite` の leaf 系104件のうち**音符0個が50件**、**うち6件が曲/セクションに配置されている**（全部 bass・延べ78箇所）。5件は `bars=1` で結果が変わらないが、**1件（`2fc3e1de…`・`bars=2`・16箇所に配置）は曲の長さが1小節→2小節へ動く**。＝素直に「`bars` を尊重」と書き換えると既存の曲の尺が変わる。回避＝手当てを**句を持つネタ（`content.lyric` がある）に限る**か、当該ネタを先に始末するか。実装時にどちらかを選び、**既存6件の尺が変わらないことをテストで縛る**。
  - **(い-c) 遅延生成＝確定（2026-07-30）でやること**：通しで読む面に**下書き行**（クライアント state・id 無し）を持つ／「＋ ここに句を足す」で下書きを増やす／**句の確定で「メロ作成＋配置」を1操作**（配置は `child_id` の FK があるので作成が先・失敗したら行ごと捨てる＝孤児を作らない）／書きかけで離れたら下書きは破棄（保存しない＝ゴミが出ない）／産まれたメロの見出しは句そのもの＝命名の裁定は要らない（下の表示の決めと同じ理由）。
    - **却下した (い-a)/(い-b) に要っていた作業は全部不要**＝(い-a) の `schemas.ts:6` scope enum 拡張／タグ方式／非アクティブの出入り規則／一覧の「除外」フィルタ新設、(い-b) の保存経路前段での暗黙生成／産まれたメロの命名。**`useCowGuard.ts:70` の穴はスライス6の範囲として残る**（(い-b) では「先に塞ぐ」が前提条件だったが、遅延生成では共有の枠が存在しないので前提条件から外れる）。
  - **カードの表示（音符0個のメロ）＝確定**：`MiniRoll.tsx:51`（音符0個で `null`）を、句があれば**句の表記1行**を返す形へ。カード見出し（`NetaList.tsx:40` の `title ?? text ?? "(無題)"`）はメロで両方空なら句を使う。**見出しと絵の枠で同じ文字を2回出さない**＝見出し＝句／絵の枠＝状況（「音符はまだ無い ・ 10音」）に分ける（モック裁定どおり）。句も音符も無ければ「まだ空」。**表示だけ＝保存の形・一覧の絞り込み・`scope` は不変。** テストは `NetaList`／`MiniRoll` の既存テストに1本ずつ。
  - **⚠ の始末＝上の bit 一致の地雷は遅延生成で構造的に消える**：遅延生成では**句を持つメロしか音符0個で生まれない**ので、尺の手当てを **`content.lyric` を持つネタに限る**（上の回避2択のうち前者を採る）と決めれば、既存の音符0個50件（配置6件・全部 bass＝句なし）は**1件も挙動が変わらない**＝bit 一致が条件式で保証される。実装時は「句なしの音符0個ネタは従来どおり1小節」をテストで縛る。

**スライス6＝2番＝複製の受け皿（どの案でも要る・置き場非依存）**
- **独立部分＝実装済み（2026-08-02）**：`copyNeta`（`apps/api/src/core.ts`）が元 id への繋がりを **`relation_edge(copy_of)`＝新→元**で残す（`vary` の `variant_of` とは**別の種類**＝copy_of は「別物にした」出自の記録だけ・元を直しても及ばない／variant_of は「同じものとして育てる」宣言）。**deep copy ではコピーした各ネタから元の各ネタへ**張る＝1番と2番をパーツ単位で突き合わせられる（共有 child は memo で1本）。**既存の複製の振る舞い（content/tags/scope/子の deep copy）は一切変えていない**。元ネタ側は outgoing が増えない＝`getBacklinks(元, "copy_of")` で「この元の複製たち」を辿る。テストは `apps/api/test/core.test.ts`（リーフ／deep copy の2本・先に赤）。2番を作る操作は既にある＝曲フォームの「複製する」（`apps/web/src/components/FormStrip.tsx` `duplicateUnit`）＝この操作の産物に出自が載るようになった。**画面はまだ copy_of を読んでいない**（`RelationsPanel` は未知の type を kind ラベルで出す＝害は無い）。「1番と2番の差を見る面」（§31-11 の5）はこの辺の上に作る。
- **残＝裁定待ち3・4依存（未実装）**：共有されたメロへの書き込み確認：`useCowGuard.ts:70`（`parentId` 無し＝ネタ帳から直接開くと何も聞かずに原本へ書く。`App.tsx` で親から潜ったときだけ parentId が付く）。詞を持つメロではこの経路が正面玄関になる。「この曲だけ変える」が分家（vary）を呼ぶことと、2番の作り方が複製であることの食い違いも、ここで扱う。範囲は §31-11 の裁定待ち3・4に依存。

**スライス7＝詞先メロと分析の入口を表記にそろえる（置き場非依存・順番の制約は耳確認だけ）**
- **実装済み（2026-08-02）＝ただし音符数が変わる2つは既定OFFの opt-in**（「音符数が変わるので耳で確かめてから入れる」を、入れずに待つのではなく**入口を作って既定は従来のまま**という形で満たした＝耳確認の前に本物の音を出せる）。
  - `planLyricMelody(rawLines, opts)` が**読み（かな）を受け取る形**になった＝`opts.readings`（行と同じ並び・**呼び側が先に解く**＝この関数は同期の純関数のまま）。渡された行は読みでモーラを数える／未指定・空の行は従来どおり表記の字をそのまま数える（漢字は1字1音に化ける）＝**未指定なら従来と bit 一致**。
  - ~~`opts.standSpecialMoras` で「っ」「ー」にも音符を立てる側へ切り替えられる（既定 false）~~ → **撤去（2026-08-02 オーナー裁定）＝旗ごと消して従来一本**。`isOnsetMora`（長音ー＝延長・促音っ＝詰め）が**正で確定**＝#13d の「モーラ⇔音数 100%一致」の数え方もオンセット基準で確定（§31-8）。裁定原文「促音伸ばし音はいらない、打ち込むときに工夫するので」＝**耳確認も既定反転も不要になった**。
  - `genLyricMelodyCandidates` の返り content に**句を載せた**（`lyricLayerOfPlan`＝行1つ＝句1つ・表記が `text`・範囲は V2 へ渡した句割りと同じ拍・読みの控えは載せない＝画面が開いたときに取り直す）。**音符は1つも変わらない**／計画が空＝`lyric` キーを生やさない。
  - MCP `gen_melody` に `readLyrics`（表記から読みを取る・pyopenjtalk）を追加＝**既定OFF**。読みが取れなければ黙って従来の数え方へ落ちる（生成を止めない）＝返りの `lyricPlan.readingSource`（`none`/`pyopenjtalk`/`failed`）で**読みの出所**が分かる。（同時に足した `standSpecialMoras` は**同日の裁定で撤去済み**＝上。）
  - 説明文2箇所も実装に合わせた＝`gen_melody` の `lyrics`（「かな推奨(漢字はpyopenjtalkが読む)」は**嘘だった**＝読みを解く経路が無かった。今は「既定はかなで渡す／読みを取らせるなら `readLyrics:true`」＋句が載ることを明記）／`suggest_lyric_rhythm`（**実装は据え置き**＝この型候補では ー/っ に音符を立てない。2026-08-02 の裁定で**実装どおりで確定**＝説明文からも「裁定は立てる」の断りを外した）。
  - テスト＝`apps/api/test/lyrics-plan.test.ts`（読み・**既定＝従来と JSON 一致**・計画→句）＋`lyrics-first-melody.test.ts`（content に句が載る・既定の音数7／`readLyrics` で7＝pyopenjtalk 未導入でも落ちない）。
- **残＝耳で確かめて既定を反転する**（オーナー手番）＝**①読みを取る側を既定にするかだけ**（表記のまま渡せるようになる）。~~②「っ」「ー」に音符を立てるのを既定にするか＋促音の VOICEVOX 送り字~~ → **2026-08-02 裁定で消滅**（立てない＝従来で確定・旗ごと撤去＝§31-8）。backlog「作詞補助アークの耳確認リスト」も①だけが残っている。

### 31-11 設計層としてなお決まらない＝オーナーの裁定が要るもの

（requirements 末尾「作詞まわりの未決」と architecture「未決・要調査」は、番号でここを参照する形にして二重管理にしない。）

1. ~~歌詞の置き場4案~~ → **決着（2026-07-30 オーナー裁定）＝案(い) 空のメロを置いてそこに書く。代わりにコピー機能を充実（§31-9・スライス6）**。~~残る枝＝空メロの見た目~~ → **これも決着（2026-07-30）＝(い-c) 遅延生成＝空のうちは実体を作らず、句を書いた瞬間に作って置く＋カードには句を出す**（§31-9・スライス5）。**＝この項は完全に閉じた。**
2. 仮歌はどこまで自動で鳴るか＋字余りのまま鳴らしたら余ったモーラをどうするか。**うち「読みが足りない余り音符」側は決着（2026-07-29 裁定）＝仮歌の既定「ラ」で鳴る**（`sing.ts:28` の既定に乗る＝§31-2 契約3）。音符を割る操作の存否は「音楽的に成立する割り方」の研究の結論待ち（§31-5）。
...
14. 歌声エンジンを常時起動にするか。
15. モックの裁定待ち9件（印の規則・候補の並べ方ほか）。**うち「通しの面の既定の姿」は 2026-08-02 の実機追認で決着**（態A既定＋[表示]シート＝§31-10 スライス5）／**SplitRoll の色（黄）も同日決着**（§31-5）＝残りは印の規則・候補の並べ方ほか。
16. ~~**メロ編集画面の歌詞まわりの画面裁定**~~ → **✅ 完全に閉じた（2026-08-02）**（2026-07-30 追加・入口＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §5・材料＝`docs/archive/2026-07-30-lyric-editor-screen-decisions.md`＋モック `docs/archive/2026-08-02-lyric-mode-ui-mock.html`＝ともに archive へ移送済み）。**(a) は同日先に決着**（下）・**(b)(c)(d) はオーナー実機スクショ4枚で追認＝レビュー済み・採用確定**。記録として中身を残す。
    - **(b)(c)(d)＝2026-08-02 追認済み（採用確定）**：(b)＝案C（未反映のときだけ「読みを反映」を1行に出す）／(c)＝案B（韻律トグルはパネルの奥へ＋OFF を覚える）／(d)＝文言一式（`あとN音`→**字足らずN**・未反映チップ新設**まだ反映していません**・`読みを取ります`→**まだ読みを取っていません**・`クリア`→**かなを消す**）。
      - 経緯（残す）＝2026-07-30b に私の推しで実装し、監査の指摘1-D で「決着」と書いたのを**「暫定実装・レビュー待ち」へ差し戻していた**。2026-08-02 にオーナーが実機スクショで見て**「良い」**＝差し戻し分もそのまま通った。**「1件ずつ戻せる」という退路の注記はもう要らない**（戻す前提が消えた）。
    - **(a) 詞モード（1音符ずつの入力）＝2026-08-02 に決着（オーナー裁定）。この項目からは外れる。**
      - **オーナー原文**：「**かなチップは破綻するからいや。ボタンは歌詞のまま。送り方は候補2がきれい**」。
      - **採用＝候補2の送りバー**（材料の 2026-08-02 増補・モック `#f2b`）＝詞モードの固定バーを **「◀｜音名・拍｜かな input｜▶｜✕」** に整える。**▶＝確定して次の音符へ**（既存「確定→次」の後継＝aria-label `syllable-commit` は据置）／**◀＝確定して前の音符へ**（新設・誤送りからの復帰）／**✕＝編集を閉じる**（打ちかけは確定してから閉じる＝据置）。キーは **Enter/Tab=▶・Shift+Enter/Shift+Tab=◀・Esc=✕**（`preventDefault`・IME `isComposing` ガードは既存流儀を維持）。input に **`enterKeyHint="next"`**（スマホIMEの確定キーが「次へ」表示になる）。**対象音符のハイライトが画面外なら自動スクロールで追う**。未選択時のヒントは「**音符をタップ→かなを打つ→Enterか▶で次へ**」。
      - **却下＝候補4（かなチップ帯＋まとめ打ち）**＝オーナー「**かなチップは破綻するからいや**」。句のモーラ列を帯に並べてタップで任意音符へ跳ぶ案・複数かなの自動分配（`splitMora` 流用）ごと**作らない**。
      - **不採用＝候補3（入口を歌詞行の隣へ移設）**＝選ばれず。モード行は4つのまま（一等地維持）。**候補1（説明増補）は候補2に吸収**（enterKeyHint・ヒント文言）。
      - **改名しない**＝オーナー「**ボタンは歌詞のまま**」。モード行4つ目のラベルは**「歌詞」**で確定（`KindEditorBody.tsx`）。歌詞行（句の面）と「歌詞」ボタンで口が2つ並ぶことは**許容**＝名前で割らず、バーの自己記述（ヒント文言）で用を伝える。
      - **⚠ 現況ドリフトの是正（2026-08-02）**：本書はこれまで (a) を「2026-07-30c に案B＝『詞』→『1音ずつ』で確定・実装済み」と書いていたが、**実ラベルは「歌詞」**＝`3cd5bf3`（Pass2 初見レビュー・2026-07-31）で「1音ずつ」→「歌詞」に変更済みだった。**本書の記述が実装に追いつかず残っていた**（§31-9 の同記述もあわせて是正）。今回のオーナー裁定「ボタンは歌詞のまま」で、実装のほうが正しかったことが確定した。
      - **未決①〜⑤の帰結（すべて閉じた）**：
        - ①**デスクトップ作法とスマホの両立**（「スマホにタブキーはないんだわ」）＝**「バーのボタンが正・物理キーは近道」で閉じる**。◀▶✕という**画面上のボタンが正の口**で、Enter/Tab/Shift+Tab/Esc は**同じ動詞への二つ目の引き金**にすぎない＝キーの無い環境で機能が欠けない。
        - ②**一等地（モード行の4つ目）に置き続けるか**＝**維持で閉じる**（候補3 不採用）。
        - ③**ボタンの言葉**＝**「歌詞」のままで閉じる**（オーナー原文）。
        - ④**発見性の穴（説明が `title` だけ＝スマホでは出ない）**＝**バーの自己記述＋`enterKeyHint` で閉じる**。押す前の説明ではなく、押した直後にバーが「音符をタップ→かなを打つ→Enterか▶で次へ」と手順を名乗り、IMEの確定キー自体が「次へ」と表示される。
        - ⑤**「他にもっといいアイディアないかの検討はほしい」**＝**消化済み**（2026-08-02・Fable 3レンズで発散→4候補→実CSSモック。裁定で候補2採用・候補4却下・候補3不採用）。
      - **⚠ 硬化させた記述の訂正（監査の指摘1-B）**：07-29 の「個別の音符の修正がメインでは。」は**打診**だったのに、`docs/research/2026-07-29-lyric-editor-screen-design.md` と材料 doc で「オーナーの見立て／主動線」と書いて**未裁定の判定基準に使っていた**。2026-07-30b の発言（「このツールではメインと言っていない」）で覆った＝**この枠を判定に使わない**（記録として残す）。
      - 文脈行つき2行バー案（語・機械の読み・高低・手直し済みを出す第2行）は**据え置き**＝候補2で1行に収める形が決まったので、足すなら別スライスで。
    - **経緯＝オーナー不在中に推しで進めた（2026-07-30b・オーナー指示「しばらく…何も見られないから進めちゃって」）→ 2026-08-02 に追認されて閉じた**：(b)(c)(d) は**私の推しをそのまま採って実装した**。根拠の表は材料 doc（`docs/archive/2026-07-30-lyric-editor-screen-decisions.md`）にそのまま残っている。
      - 戻す費用の見積り（2026-07-30b 監査の指摘2-2）＝(c) は localStorage 1キーとトグルの置き場・(d) は文字列とテスト2行で小さいが、**(b) は器ごと戻すなら `8c4e98b` の revert**（`PianoRoll.tsx` +166行の作り替え）＝という退路を用意していた。**追認により不要になった**（記録として残す）。
      - **併記すべき事実**：`5bc4a49` 時点の §31-9 には「**裁定が出るまで展開パネルの実装は入れない**」と私自身が書いていた。その門を「進めちゃって」で越えている＝**結果は当たったが、越えたこと自体は当たり外れと別に記録に残す**。

17. ~~**(い-c) 遅延生成の粒度**~~ → **✅ 決着（2026-08-02 オーナー実機追認）＝この項目は閉じた**（2026-07-30b 追加・上の §31-9 の ⚠ から起票）。裁定材料＝実機スクショ3枚（通しで読む面の既定の姿／[表示]全部オン／「＋ ここに歌詞」）＝オーナー「B1-3 は良い」。
    - ①**書いた粒度**＝**「確定」の操作を置く**（1文字目では生まない）。②**書きかけでやめたら残さない**（下書きはクライアント state・保存されるのは確定した句だけ＝§31-7 と衝突しない）。③**確定の合図＝Enter（Shift+Enter は改行）／確定ボタン・Esc で取り消し**（`apps/web/src/components/LyricOverview.tsx` の `editorBar`）。
    - あわせて追認＝**□の穴は (い-c) の対象外**（「＋ ここに歌詞」は曲に穴を開けるだけでメロを作らない・メロ化は「メロにする」の別操作）。＝有力仮説で先行実装した形がそのまま正＝**作り直しなし**。

### 31-12 本節が訂正する本書の既存記述（実コードで確かめた）

| 本書の場所 | 旧記述 | 実態と訂正 |
|---|---|---|
| #13 データモデル決定（2026-07-15） | 「歌詞は `Note.syllable` としてメロに埋め込みが正典」 | 確定「歌詞の正データは漢字仮名交じりの普通の文章」と衝突＝**歌詞の本体は句（`content.lyric`）・`Note.syllable` は仮歌のための写し**へ。**根拠（同じ音符列に別歌詞は成立しない／同じ音・違う歌詞は別のメロディネタ）はそのまま生きる**＝句もメロの中に持つ・2番は複製（§31-1・スライス6） |
| #13b「入力源」 | 「本スライスは `syllable`（かな）で完結」 | 実測でかなを渡すと高低が **48.9% 反転**（`accent.ts:76` `accentsFromSyllables`）＝**破棄**。読みは表記から取る（§31-3） |
| 「歌詞をメロディに流し込む」決定L1 | 「歌詞は `{ lines:[{text, reading}] }` を行ごとに持つ」 | **一度も実装されていない**（`kind:"lyric"` の content は素通しで、`lines` を読む/書くコードは無い）＝**破棄**し `content.lyric.phrases`（§31-1）へ |
| 同・土台の記述 | 「モーラ分割は web `lyrics.ts`/worker `split_mora`」 | **worker に `split_mora` は無い**（`apps/worker/src/cm_worker/` は `search.py`/`db.py`/`serve.py` のみ＝ジョブ消化は 2026-07-05 に撤去）。TS 実装2本（`apps/web/src/lyrics.ts:8`・`apps/api/src/lyric.ts:16`）だけが実体＝§31-4 で music-core 1本へ |
| #16「(C') condition 解決層」 | 「`fit_to` は worker が解決し `params.fit_context` に展開」 | **`fit_context` は実装が1つも無い**（リポジトリ全体で 0 ヒット）。`fit_to` も `mcp.ts:647` の説明文に `params.condition={fit_to,by}` と書かれているだけで解決器が無い。**worker 自体がジョブ消化から撤去済み**＝この段は**未実装として棚上げ**（歌詞の音数合わせ `by:"syllable"` は §31-3/§31-4 が置き換える） |
| #13「歌詞流し込み」 | 「text→(漢字あれば読み)→モーラ列」 | 読みの解決経路が実装に無かった（`splitMora` が漢字を1字1音で数える）＝§31-3 の `POST /music/reading` で埋める |
| #13d 受け入れ「モーラ⇔音数 100%一致」 | ー | 「モーラ」＝`isOnsetMora` が っ/ー を外した**オンセット数**。2026-08-02 オーナー裁定（旧裁定「立てる」の撤回）で**この数え方が正で確定**＝#13d の受け入れ条件はそのまま生きる（§31-8） |
| content の厳密スキーマ「歌詞の流し込み」 | 「歌詞neta をメロのノートに音節割り当てしてタイムラインに流せる」 | 流し込み（音符を割る）は**人が「合わせる」を押したときだけ**へ（§31-5）。歌詞の本体は `melody.content.lyric`（§31-1） |

**先行の研究文書に見つけた誤りの記録**：`docs/research/2026-07-29-upstream-tasks.md` の監査対応が「オーナーの原文『最終の良し悪しは人が耳で決める』」と書くが、**この文は一次資料のどちらにも存在しない**（grep 0件）。実在するのは `docs/requirements.md:215` の要件文（別アーク由来の道具全般の原則）で、オーナーの原文は「作る努力の部分を人間から奪うと自作感がなくなるので、最後に選ぶのは人間にさせるべき」。要件文としては引けるが「オーナーの原文」と呼ぶのは誤り。

## ジグの主旋律の様式＝段1（関数だけ・2026-09-17・正典＝`docs/drafts/2026-09-16-celtic-jig-reel-design.md` §8 S1 と末尾の耳判定 2026-09-17）

### 根拠（上位から下ろす）
- 要件＝`docs/requirements.md` 達成度の線引き「ポップス中心の例外として、アイリッシュのジグとリール…」（粗い寄せ・統計のみ・装飾と揺れは DAW）。
- 耳判定（2026-09-17）で芯が確定：①中心の音（部ごとに1つ）の近くに留まる ②区切りの音（半小節ごとの要所の音）へ同じ音か順次進行で着く ③同じ音の連打を含む均等な8分。既存のメロディ生成に 6/8・旋法を指定したものは「もっともダメ」＝既存の表面に分岐を足さず**別の純関数**で作る（裁定5＝置き場は既存の「骨格→表面」の上に様式として載せる・既定の出音は不変）。

### 関数（`packages/music-core/src/jigMelody.ts`・api/web 共有の純関数）
- `generateJigMelody({ tonic, mode, seed, tempo? })` → `{ notes, parts, meter:"6/8", bars:16, tempoQuarterBpm }`。
  - `tonic`＝主音の音名クラス 0〜11／`mode`＝`ionian|dorian|mixolydian|aeolian`／`tempo`＝付点4分の BPM（既定 110）／`seed`＝整数。
  - 形＝A部8小節＋B部8小節（計16小節＝48拍・4分音符単位の start/dur）。音域＝主音（4オクターブ目）から旋法の12段上まで。
  - 骨格＝部ごとに中心の音（主音／5度／3度を重み 33:21:19・B部は1オクターブ上）を1つ決め、半小節ごとの16点をその上下1〜2段で交互に振る（小節5〜6は上へ2〜4段）。4小節目末＝5度・8小節目末＝主音。
  - 表面＝半小節（8分3つ）ずつ1音ずつ決める。次の要所へ残りの音数で届く範囲だけから、順次（重み5）／3度（2）／同音（1）／向きの転換（1）を引く。4小節目末は4分＋8分、8小節目末は付点4分、他は均等な8分。強弱＝半小節の頭 96・他 78。
  - **固定の旋律の表を持たない**（その場で1音ずつ）。他者の曲の旋律は持たない。
  - 乱数＝`PyRandom`（MT19937）を `stableSeed("<seed>-jig-<部>")` 等の文字列から種付け。`Math.random`・時刻は使わない。
- `diagnoseJigMelody(melody)` → 診断値（合否にしない）：中心の音±2段に居る音の割合／隣り合う音の音程（同音・2度・3度・それ以上）の割合／要所の直前の着き方（同音・順次・跳躍）の割合／均等な8分の割合。
- 試作（scratch の Python・オーナーが「ジグらしい」と判定した作り方）と**同じ手順を移植**するが、**乱数列は一致させない**（Python の `random.Random(文字列)` は SHA-512 種付け・`Random(浮動小数)` はハッシュ種付け・`choices`/`choice` は `getrandbits` 経由＝全部を移植する価値が無い）。同じ性質であることは診断値の比較で担保する。

### ゲート（機械の合格条件は最低限）
- 拍子どおりの長さ（音が隙間なく連なり合計 48 拍・小節の区切りをまたいでも総長一致）／旋法の音だけ／決定論（同じ種で同じ出力）／種が違えば出力が違う。
- 既存のメロディ生成は**触らない**（新しいファイルを足すだけ・到達口は作らない＝段2以降）。
- 診断値（中心の近さ・音程・着き方・均等な8分）は**テストの合否にしない**（耳の観察を機械の条件へ硬化させない）。
