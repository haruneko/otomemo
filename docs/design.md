...
- **6/8（compound）の走句拡張（2026-07-10・full＝2026-07-10-68-sixteenth-runs-plan.md）**：4/4と違い6/8は**グリッド(8分6枠)と語彙(RHYTHM68_DATA)のレベルで16分が不在**＋run旗が compound で全false固定＝runsノブが完全no-op。正準＝**runs>0 指定時のみ compound の `mkMotif` を16分12枠(t=bar*3+s*0.25)へ切替**（未指定＝従来6枠 pick68 verbatim＝bit一致）。効き：(a)12枠設計語彙 `RHYTHM68X_DATA`（既存6枠語の機械展開 x→x. を基底＋16分ペア割り/付点8分+16分/走句の設計語）を `runW`×`densW` で再重み付け (b) run旗の compound 全false固定を撤去し4/4同型「隣接≤0.26」判定へ（既存 run方向保持・gap-fill に乗る＝**新ピッチ論理なし**） (c) score の n16計上を8分格子外(.25/.75)で数え・受入音数帯 lo68/hi68 に runs 拡張 (d) ジグ跳ね dur と16分ペアの共存はテスト担保。表面パス案（生成後に8分を割る加飾）は不採用＝runsの効く層が拍子で変わるのは設計として歪。スコープ外＝swing/push/ドラム3ノブの6/8対応・コーパス12枠統計抽出(後続任意)。**耳確認必須**（6/8 × runs 0/0.4/0.8）。
