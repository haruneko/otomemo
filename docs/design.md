...
- **メロ化の橋＝実装済み（2026-07-31）**＝曲の句（□/詞先の下書き）の行に「メロにする」＝**メロ作成（notes空・content.lyric に句）＋配置を1操作((い-c) 遅延生成)＋曲側から句を外す**＝詞先→曲先。失敗で `deleteNeta`＝孤児なし。実機検証＝「詞だけ先に」→メロにする→曲の lyric 句数0・セクションに melody「詞だけ先に」が生成配置。□の削除口も（曲の句を消す）。web 1234緑。**＝スライス5は 読む＋推敲＋インライン編集＋□プレイスホルダ＋詞先↔曲先の橋 が一通り動く。**
