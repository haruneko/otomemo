...
- **⚠ 訂正（2026-07-29・オーナー未レビュー）**：WP-L0 の「オンセットロール」は**長音ーと促音っに音符を立てない**（`apps/api/src/music/lyricsPlan.ts:30-32` `isOnsetMora`）。これはコンセプトの確定「『っ』『ー』にも音符を立てる」に反する。`isOnsetMora` を「全モーラが音符を持つ」に変える＝**`planLyricMelody` の音数＝総モーラ数**になる（§31-8）。**生成される音符数が変わる＝耳で確かめてから入れる**（#31 スライス7）。あわせて `lyrics` に渡す入り口を**表記（漢字仮名交じり）**にそろえ、読みは呼び側が先に解いて渡す（§31-3(f)）。`mcp.ts:964`（「漢字は先に読み(かな)へ」＝指示は正しいが実装が数え直す）・`mcp.ts:1250`（「漢字はpyopenjtalkが読む」＝実装がそうなっていない）も、そのとき実装ごと直す。
- **受け入れ（機械）**：モーラ⇔音数（＝計画オンセット数）100%一致（全候補・property）／句頭A-01赤の件数を候補メタに表示（ゼロ強制なし）／整合score降順ソート／E-rule ガード維持（V2既存）／`lyrics` 無し bit一致。※この「モーラ」の数え方は上の訂正で **っ/ー 込み**へ更新する（§31-8）。**残り＝リズム配分ヒューリスティックの質と耳較正約30%（A/C 指摘）＝M-1 は「音数一致＋句割り＋整合レポート」まで**。
...
- 従うコンセプトの確定：正データは漢字仮名交じり／かな書きは仮歌のため／「っ」「ー」にも音符を立てる（Q7「立てる」）／字余りとメロが途中を分ける／割付の手直しは最初は作らないが、手でずらす・自動符割りを見据えた作りにする。
...
- **「っ」「ー」は音符を立てる（オーナー裁定）**。`splitCandidates` の特殊拍判定はこれに従う。既存の `prosody.ts roleOf`・`lyricsPlan.ts isOnsetMora` が裁定と食い違う件は別途直す（研究 §4-2 穴3）。
...
裁定は**立てる**（Q7）。実装は2流儀に割れている。
- **立てる側（裁定どおり）**：`splitMora`（web/api）・`flowLyric`＝ー/っ/ん も1モーラ1音符。仮歌も `syllable:"ー"`＝母音継続（`sing.ts:141,148`）で音符が立っている。**こちらが正＝触らない。**
- **立てない側（裁定に反する）**：詞先メロの `isOnsetMora`（`apps/api/src/music/lyricsPlan.ts:30-32`）＝長音=延長・促音=休符として音符を立てない。`prosody.ts:97` `roleOf`（`suggestLyricRhythm` の候補の role）も同じ考えの提案を出す。

**直し方（2026-08-02 実装＝旗で切り替える形にした）**：`isOnsetMora` を消して「全モーラが音符を持つ」に置き換えるのではなく、`planLyricMelody` の `standSpecialMoras`（既定 false）で切り替える＝true なら音数＝総モーラ数（#13d の受け入れ条件「モーラ⇔音数 100%一致」の数え方も っ/ー 込みになる）。**生成される音符数が変わる＝既定は従来のまま・耳で確かめてから既定を反転する（スライス7）**。`suggestLyricRhythm` の候補は「提案」であって保存物ではないので今回は触らない。
実装時の確認1点：促音「っ」の音符を VOICEVOX にどの lyric で送るか（`sing.ts:148` は syllable をそのまま送る）。「っ」が歌えない場合の音の扱いはスライス7の耳確認で決める＝**設計はデータ（音符は立つ）だけを決める**。
...
- **実装済み（2026-08-02）＝ただし音符数が変わる2つは既定OFFの opt-in**（「音符数が変わるので耳で確かめてから入れる」を、入れずに待つのではなく**入口を作って既定は従来のまま**という形で満たした＝耳確認の前に本物の音を出せる）。
  ...
  - `isOnsetMora`（裁定に反する側）は**残したまま `opts.standSpecialMoras` で裁定側に切り替えられる**＝true で「っ」「ー」にも音符を立てる（全モーラが音符を持つ＝`onsetCount === moraCount`）。**既定 false＝従来と bit 一致**。#13d の受け入れ条件「モーラ⇔音数 100%一致」は、この旗を立てたときだけ っ/ー 込みの数え方になる。
  ...
  - MCP `gen_melody` に `readLyrics`（表記から読みを取る・pyopenjtalk）と `standSpecialMoras`（っ/ー に音符を立てる）を追加＝**どちらも既定OFF**。読みが取れなければ黙って従来の数え方へ落ちる（生成を止めない）＝返りの `lyricPlan.readingSource`（`none`/`pyopenjtalk`/`failed`）と `lyricPlan.standSpecialMoras` で**どの数え方で音数が決まったか**が分かる。
  - 説明文2箇所も実装に合わせた＝`gen_melody` の `lyrics`（「かな推奨(漢字はpyopenjtalkが読む)」は**嘘だった**＝読みを解く経路が無かった。今は「既定はかなで渡す／読みを取らせるなら `readLyrics:true`」＋句が載ることを明記）／`suggest_lyric_rhythm`（**候補は提案であって保存物ではないので実装は据え置き**＝§31-8 どおり。説明文だけ「この型候補では ー/っ に音符を立てない・裁定は立てる」と正直に書いた）。
  - テスト＝`apps/api/test/lyrics-plan.test.ts`（読み・裁定の数え方・**既定＝従来と JSON 一致**・計画→句）＋`lyrics-first-melody.test.ts`（content に句が載る・既定の音数7／`standSpecialMoras` で9・`readLyrics` で7＝pyopenjtalk 未導入でも落ちない）。api 1522緑。
- **残＝耳で確かめて既定を反転する**（オーナー手番）＝①読みを取る側を既定にするか（表記のまま渡せるようになる）②「っ」「ー」に音符を立てるのを既定にするか＋**促音「っ」を VOICEVOX にどの lyric で送るか**（`sing.ts` は syllable をそのまま送る＝§31-8 の実装時の確認1点）。backlog「作詞補助アークの耳確認リスト」に同じ2点。
...
| #13d 受け入れ「モーラ⇔音数 100%一致」 | ー | 「モーラ」の数え方が `isOnsetMora`（`lyricsPlan.ts:30-32`）で っ/ー を外している＝裁定「立てる」に反する。スライス7で数え方ごと更新（§31-8） |
