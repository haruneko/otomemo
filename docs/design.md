...
- **編集面の出し方（決定・実装）**：軽い編集（歌詞・テーマ等のテキスト＋タグ）は**中央ダイアログ**。音楽の編集（melody＝ピアノロール、将来 chord/rhythm/配置）は面が要るので**全画面オーバーレイ**（上バー＝戻る/調/テンポ/再生/書き出し/保存、本体＝編集面いっぱい）。出先のスマホでの編集を優先。router は当面入れず、オーバーレイ表示で実現（専用ルートは後で検討可）。
