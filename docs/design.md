...
- **つなぎ＝計画 verb の結線（S3・未実装＝確定設計）**：`suggest_form`→空 section の足場をストリップに並べる／`suggest_key_plan`→各 section の key/mode 更新（転調する共有子は分家を自動提案）＋境界の準備和音候補（gen_chords transition）／`suggest_energy_plan`→カードのΔチップ＋生成既定。全て「提案→人がワンタップ適用」。**遷移試聴**（前セクション末→次セクション頭の連結再生・縫い目E＝backlog 昇格）。
