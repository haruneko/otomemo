...
- **［耳/手］未消化（D1・オーナー実機）**：①レンズ無停止切替の滑らかさ（ループ中に畳み⇄実音・音途切れ/位置飛びなし・畳みで音程が読め実音で座りが聞こえるか＝この器の核） ②骨格ロールの打点でスクロール誤爆しないか ③クリック音量/音色がベッドとして邪魔でないか。**フィール既定（クリック vel 90/55・SAVE_DEBOUNCE 500ms 等）は暫定＝耳較正で見直し可**。
- **残スライス**：D1.5（ループ範囲ブレース＝skelPosition≠0 の playhead ずれも解消）／D2 接点ストリップ＋ダイアッド／D3 ②コード前景／D4 ④出口トレイ／D5 ①ビート前景＋レンズ一般化／D6 B-lite。
