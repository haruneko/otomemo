...
- **［耳/手］未消化（D1〜D4・オーナー実機）**：D1＝①レンズ無停止切替の滑らかさ（核）②打点スクロール誤爆③クリック音量。D2＝④指摘文が「禁止でなく味」に読めるか⑤ダイアッド持続/音量で不協和が聴き取れるか。D3＝⑥コード試着で③の響き変化がループを止めず耳で追え採用で鳴り直すのが妥当か⑦導出ベース線が「②が下声を決める」と読めるか。D4＝⑧候補の試着がループを止めず次周から鳴り現メロと比較できるか（試着メロ音量/音色がベッドに座るか）⑨「書く[①②③]＋出口[④]」レールと「→吹いたメロ N」で「戻って再度吹いても手直し済みメロは消えない」が直感に落ちるか。**フィール既定（クリック vel90/55・SAVE_DEBOUNCE500ms・ダイアッド0.8拍・試着 reloop 発火点/ゴースト扱い等）は暫定＝耳較正で見直し可**。
- **残スライス**：D5 ①ビート前景＋レンズのステージ相対一般化／D6 B-lite。D7 パーキング＝③叩き台の句単位再抽選（「句2だけ引き直す」）等。
