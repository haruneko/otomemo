...
- **(f) 物理フィルの置き方＝源流 `apply_fills` 準拠へ修正（2026-08-29・オーナー耳判定の差し戻し）**：step2 の初版は「**フィル小節を丸ごと空けて**、`placeFill(fillBar, 0.0, "bar", …)` の裸のフィルを1小節鳴らす」形だった。これは phrase_maker に**存在しない極端形**で、耳判定で「退屈・音楽的に成立していない」と却下された（比較対象の grid 経路 `FILL_TYPES` は、たとえば `fill.snare.1beat` が HiHat/Kick を3拍鳴らしたまま4拍目だけスネアという形＝**グルーヴ継続が前提**）。修正3点：
  ...
  - **(g) M3＝身体シミュレータの移植（2026-08-29・`bodyFill.ts`）＝辞書ではなく生成器**：耳判定「型の種類が少ない」の根本解。phrase_maker `bodyfill.py`（676行・fills/ で最大）を忠実移植した。**型を表から引かず、毎回 DP で解く**：グルーヴが手を置いていった位置（右手=ハット・左手=スネア）から、キットの座標・打面の高さ・リバウンド係数・手の連打レート上限・移動速度の制約を通って、crash+kick の着地まで、**両手が辿れるいちばん安い経路**をスロット単位のビーム探索で求める。骨（skeleton）＝いま鳴っているグルーヴ自身を引用するので、フィルが曲から浮かない。到達口＝`fillStyle:"body"`（`gen_drums`）。
    ...
    - **限界（黙って歪めない）**：スロットは 0.25qb 固定＝**三連格子（shuffle 型・beatsPerStep=1/3）には付けない**（付与せず返る）。源流も第一増分では三連スロットを持たない。実フィルの約35%が三連という調査があるので、三連スロット化は次の増分。
