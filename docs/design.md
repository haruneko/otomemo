...
**決定N1（操作モデル＝案A）**：ロールに **[描く]/[選ぶ] モードトグル**（既存 ロール/パッド の隣）。
...
- **決定①＝フィルタをアイコンチップ行に**：`<select>{生key}` を廃止し、`FILTER_KINDS` を **`KindIcon`＋`KIND_LABEL` のチップ**（`.filter-kinds`/`.filter-kind`）に。タップで選択/再タップで解除・検索中は無効。作成グリッドと同じ絵で絞れる。
...
- **① 修正＝ブロック tap で編集/長押しで外す**：旧 tap=配置解除 を入れ替え＝**tap→`onOpenNeta(子)` で子ネタの編集画面へ潜る**／**長押し(500ms)→配置解除**（ネタ自体は残す＝低リスク）。指8px動いたら長押し中止（スクロール誤爆防止）。右端グリップ(③ループ)とは別ジェスチャで共存。
