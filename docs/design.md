...
- **`flowLyric` の呼び出し元を人の明示操作だけに限定する**：現在の呼び元は PianoRoll の流し込みボタン・`set_lyric`（`mcp.ts:953`）・詞先メロ（`generate.ts:762`）。**改訂（2026-07-30）＝句の面が配線された画面では、常設の「流し込む」ボタンとかな欄は退役**（既定で音符を割る先例はゼロ＝調査の定石3）。**退役の実施は歌詞パネルの実装と同時**（§31-9）＝それまで現状のまま常設で置く。理由＝いま外すと音符を割る唯一の口が先に消える（パネルは裁定待ちで着手できない）。音符を割って合わせる操作を残すかどうかは、**「音楽的に成立する割り方を候補として出せるか」の研究の結論待ち**＝出せる見込みが立たないなら操作ごと作らず余りは切り捨て（研究文書は 2026-07-30 着手済み）。残す場合の置き場は歌詞パネルの奥・明示操作＋確認つき・名前は音符が変わることが読める言葉（仮称のまま＝§31-11 裁定待ち16）。`set_lyric`・詞先メロ・句の面が配線されていない呼び側は従来どおり（後退ゼロ）。＝architecture「黙って実体を作らない」の受け＝**歌詞を打っただけで音符が割れることはもう起こらない**。
...
- **決まったこと2つ**（この2つは分けられない＝下の理由）：
  1. **(い-c) 遅延生成**：詞先の枠は**通しで読む面の中の下書き**（クライアント state）。**句を確定した瞬間にメロを作って同時に配置する**。書きかけで離れたら下書きは消える＝空のメロは1件も保存されない。
  2. **カードは句を出す**：音符0個のメロのカードに**句の表記**を出す（`MiniRoll.tsx:51` は音符0個で `null` を返し、カード見出しは `title`／`text` しか見ない＝いまは中身があっても空白のカードになる）。**遅延生成でも「句はあるが音符が無いメロ」は必ず生まれる**（それが詞先の主役）＝この表示は遅延生成の有無と無関係に要る。
...
16. **メロ編集画面の歌詞まわりの画面裁定（2026-07-30 追加・入口＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §5・材料＝`docs/archive/2026-07-30-lyric-editor-screen-decisions.md`）**
    - **(b)(c)(d) は決着（2026-07-30b）＝下の「⚠ オーナー不在中に推しで進めた」参照**。(b)＝案C（未反映のときだけ「読みを反映」を1行に出す）／(c)＝案B（韻律トグルはパネルの奥へ＋OFF を覚える）／(d)＝文言一式（`あとN音`→**字足らずN**・未反映チップ新設**まだ反映していません**・`読みを取ります`→**まだ読みを取っていません**・`クリア`→**かなを消す**）。
    - **(a) 詞モード（1音符ずつの直し）の見せ方＝据え置き（裁定待ちのまま）**。理由＝推し（案B＝ボタンに言葉を出す）の前提が**オーナーが実機で3分触ること**（押す前に用が分かるか／1音直したいときここを探しに来るか）で、これは代われない。**モード行の言葉「詞」も触らない**（(d) の「詞→1音ずつ」は (a) で案B を採る場合だけ効く＝いまは適用しない）。文脈行つき2行バー案も据え置き（(a) が決まる前にバーを太らせると作り直しになる）。
    - **⚠ オーナー不在中に推しで進めた（2026-07-30b・オーナー指示「しばらく…何も見られないから進めちゃって」）**：(b)(c)(d) は**私の推しをそのまま採って実装した＝オーナー未レビュー**。3件とも**戻す費用が小さい**ことを条件に進めている＝(b) は表示条件1つ・(c) はトグルの置き場と localStorage 1キー・(d) は文字列（テストが握るのは `PianoRoll-lyric.test.tsx` の2行）。**気に入らなければ1件ずつ戻せる**。根拠の表は archive の材料 doc にそのまま残っている。
