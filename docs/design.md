...
最終更新: 2026-07-29（**「#31 作詞補助＝歌詞を書く」を新設**＝歌詞の正データは句（漢字仮名交じりの普通の文章）でメロの `content.lyric` に持つ・`Note.syllable` は仮歌のための写し・読みは pyopenjtalk からまとめて取る・音符を変えずに写す `placeMoras`・字余りとメロが途中の言い分け・人の直しの置き場・自動保存で句を落とさないこと。あわせて #13／#13b／「歌詞をメロディに流し込む」／#16(C')／content スキーマの、実態や確定と食い違う記述を訂正（一覧＝§31-12）。**歌詞の置き場は案(い) を仮置き＝確定ではない**（§31-9）。**⚠ この 2026-07-29 分はオーナーのレビューを受けていない**＝オーナーの了解のうえ留保つきで先行している。上位の根拠＝requirements「歌詞を書く」節・architecture 同日追記（これらも未レビュー）／2026-07-18 表題を Otomemo に統一。GitHub リポジトリも otomemo。技術識別子＝ローカルdir/ルートpackage `creative_manager`・`cm-*`・MCP `creative-manager` は据置＝下「UI/ブランド刷新」に権威記述。本書は随時追記の決定ログ）
...
- 合成(section/song)：content でなく `compose_edge`（子＋position）で表す。section/song ＝エディタのメインペーン。
  - ※歌詞の置き場（メロがまだ無い場所＝詞先の句をどこが持つか）は**未裁定**で、案(あ)＝曲が持つ、が選ばれた場合だけ**ここに例外が1つ増える**（`song.content.lyric`）。**現在は案(い)＝人が合図して空のメロを置きそこに書く、を仮置き**で採っており、その場合この行に例外は増えない。裁定はスライス5の着手前に要る＝§31-9（2026-07-29 追記・⚠オーナー未レビュー・確定ではない）。
...
## #31 作詞補助＝歌詞を書く（設計 2026-07-29・スライス1〜4を確定／5〜7は置き場の裁定で分岐）
...
> **歌詞の置き場（メロがまだ無い場所＝詞先の句をどこが持つか）は未裁定**。本節は案(い)＝人が合図して空のメロを置きそこに書く、を**仮置き**で採るが**確定ではない**（§31-9）。スライス1〜4は4案のどれでも壊れない形で確定してある（§31-0 の検算）。
...
3. 範囲内の音符（start 昇順）へ moras を頭から1対1で `syllable` に書く。音符が余れば `"ー"`（メリスマ。`sing.ts:141` と `PianoRoll.tsx:398` がこの文字を読む＝当面変えない）。**モーラが余れば書かない＝音符は増えない**。余りの数はここでは言わない（言い分け §31-5 が別途言う）。
4. **導出（表記→モーラ・高低）／割付（モーラ→音符＝この関数）／音符の作り替え（`flowLyric`）は別の段**。`flowLyric` は**人が「合わせる」を押したときだけ**走る（当面の形＝§31-11 の裁定待ち2）。
...
- **控えを引き直す契機**：句の表記が変わった状態で保存されたとき。`reading.forText !== text` なら控えを使わない＝引き直すまで印も高低も出さない。
...
- **`flowLyric` の呼び出し元を「合わせる」経路だけに限定する**：現在の呼び元は PianoRoll の流し込みボタン（`PianoRoll.tsx:280`）・`set_lyric`（`mcp.ts:953`）・詞先メロ（`generate.ts:762`）。PianoRoll のボタンは**「合わせる」に改名して残す**（音符が変わることを明記した確認つき）。`set_lyric`・詞先メロは句に覆われていない音符が対象のままなので従来どおり（後退ゼロ）。＝architecture「黙って実体を作らない」の受け＝**歌詞を打っただけで音符が割れることはもう起こらない**。
...
**決め**
- **スライス1〜4は新しい画面を足さない。** メロを開いて詞を書く面＝既存のメロ編集画面（NetaDialog→KindEditorBody→PianoRoll）の中。PianoRoll の歌詞行（`PianoRoll.tsx:265-289`）を句の面に育てる：表記の欄（複数行・句ごと）＋「読みを取り直す」＋言い分けの表示（§31-5）＋既存「流し込む」ボタンは「合わせる」へ（§31-5）。読み・高低・かなの直し（スライス4）もこの中（語のふりがな行＋高低の2段表示＋タップ反転）。**App.tsx の3箇所に触らない**＝戻る対応の不具合の再発面を増やさない。
...
**歌詞の置き場＝案(い) を仮置きで採用（⚠ 確定ではない・裁定が要る）**

メロがまだ無い場所（詞先）の句をどこが持つかは4案あり未裁定（`docs/research/2026-07-29-lyric-storage-decision-material.md`／requirements「作詞まわりの未決」1）。オーナーの最後の傾き（原文「空のメロディを作る方のが無難な気がするな、その代わりコピーとかを充実させるとか」）に沿って**(い)＝人が合図して空のメロを置き、そこに詞を書く**を**仮置き**で採る。乗り換えが安いことは実コードで裏取り済み＝**句のデータの形は4案とも同一**で、違いは句がどのネタの content に入るかだけ（content は素通しの JSON＝`schemas.ts:16`）。**スライス1〜4はどの案でも壊れないので、この仮置きは実装にほぼ影響しない。**
**スライス5（通しで読む面と詞先の句）の着手前に、オーナーの裁定が要る。** それまでスライス5は着手しない。
...
**スライス5＝詞先の句と、通しで読む面（⚠ 着手前に置き場の裁定が要る）**
...
1. **歌詞の置き場4案（最優先・スライス5を止める唯一の1問）**＝判断材料は `docs/research/2026-07-29-lyric-storage-decision-material.md`。現在 (い) 仮置き（§31-9）。
