...
**音（原曲なし・著30-4）**：検算＝**合成クリック(小節線)＋実 onset コード**を同期再生＋区間ループ。相手は原曲でなく
**"実測 onset が自分の置いた小節グリッドに噛むか"**＝内部整合で耳確認＝**音源を保持せずに小節合わせが成立**（設計の解錠）。
...
2. **メロ再生の著作権**：検算でメロを"鳴らす"と採譜メロの再生＝複製懸念。→**クリック＋コードのみ鳴らしメロは視覚のみ**が
   無難だが検算しづらい。要判断。
