...
- **メロ単体系＝「ツール ▾」1つに集約（✅2026-07-03）**：旧UI＝[崩す]ボタンと[道具 ▾]メニューが別置き＝「崩すもツールの一種なのに分かれている／"道具"の名が謎／スタイル不揃い」（オーナー指摘）。→ ✨wandアイコンの**単一「ツール ▾」**へ統合＝縦メニュー＝**崩す（別メロ候補）**を主役色(primary)で先頭、続けて調推定・似たメロ、`移調`小見出し下に ＋半音/−半音/＋8va/−8va(2×2)。＝崩すも移調も"メロを変える道具"として1箇所に並ぶ。名前は「道具」→「ツール」（"道具"は用途不明との指摘）。
