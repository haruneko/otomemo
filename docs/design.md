...
- **歌詞まわりの姿（2026-07-30 改訂・根拠＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §3）**：現状の常設6段（歌詞欄・読み行・読みを取り直す・かな欄・流し込む/クリア・韻律トグル＝実測約220px。描画は `PianoRoll.tsx:402` 以降）を畳み、**常時は「歌詞行」1段（約40px）だけ**＝句のテキスト1行（あふれは省略・空なら「歌詞を書く」）＋状態チップ1個。書く/直すときは歌詞行タップで**歌詞パネル**に展開＝表記の入力欄（複数行）＋読み表示＋**「読みを反映」**（§31-3(d)）＋奥の段（読みを取り直す・かなをクリア・韻律チェックON/OFF＋凡例。音符を割る操作は研究の結論待ち＝§31-5）。かな欄＋「流し込む」は句の面が配線された画面から退役（§31-5）。**2026-07-30b に確定＝実装済み**（§31-11 の16。オーナー不在中に推しで進めた＝未レビュー・戻せる）。確定した姿を以下に書く。
  ...
  - **詞モードの見せ方＝(a) 案B で確定（2026-07-30c オーナー裁定）**＝モード行の「詞」ボタンに**言葉を出す**（「詞」→「1音ずつ」）。理由＝説明が `title` 属性だけ＝スマホでは一度も出ず、存在に気づけない（`KindEditorBody.tsx:188`。オーナー自身も存在を認識していなかった）＝発見性の唯一の穴に直接効く。入口はモード行のボタンのまま（2026-07-30b 裁定「ボタンでいいのでは？」・ダブルタップ/長押しは却下）。**送りの形（確定→次の音符へ）は既存のままで足りる**（同裁定）。**文脈行（バー1行→2行）は今は足さない**＝詞モードの形が固まる前に太らせると作り直しになる（次スライスで安く足せる）。裁定の重み＝オーナー「文句あったら後で言う」＝**推しで先行・未レビュー・戻せる**。（2026-07-30b にオーナーが挙げた「他にもっといいアイディアの検討がほしい」「スマホにタブキーはない＝デスクトップ作法の扱い未議論」は、実物を触ってからの反応で拾う＝実機で不足なら発散し直す。）
...
16. **メロ編集画面の歌詞まわりの画面裁定（2026-07-30 追加・入口＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §5・材料＝`docs/drafts/2026-07-30-lyric-editor-screen-decisions.md`（件1が未裁定なので drafts に残置））**
    ...
    - **(a) 詞モード（1音符ずつの入力）＝2026-07-30b に一部だけ決着。残りは裁定待ち。**
      - **決まった（オーナー発言）**：**廃止しない・入口を消さない**＝「**メインではないが入力としては必ずあるべき**」（原文）。入口は**ボタンでよい**＝「ダブルタップとか長押しとかはだるいというか他の機能とかブルからボタンでいいのでは？？」＝**ダブルタップ／長押しは却下**。**入力ごとに次の音符へ遷移する今の形は悪くない**＝「現状直す前の入力ごとに遷移するインターフェースは悪くないしもうあるでしょうに」「音符一つに入力ごと次の音符に遷移、ね」（＝既存の `確定→次`／Enter で実装済み・触っていない）。
      - **決まっていない**：①**デスクトップの作法とスマホの両立**（オーナーが名指しした論点＝「デスクトップアプリのお作法をスマホも両立させるところで使うの？っていうのも議論してないでしょ。そりゃパソコン操作のときはそれがありがたいけど**スマホにタブキーはないんだわ**」）②**メインではない**なら一等地（モード行の4つ目）に置き続けるか ③ボタンの言葉（「詞」のまま／言葉を足す） ④発見性の穴（説明が `title` 属性だけ＝スマホでは出ない）を埋めるか ⑤オーナー要望「**他にもっといいアイディアないかの検討はほしい**」への材料。
      - **⚠ 硬化させた記述の訂正（監査の指摘1-B）**：07-29 の「個別の音符の修正がメインでは。」は**打診**だったのに、`docs/research/2026-07-29-lyric-editor-screen-design.md` と drafts の材料で「オーナーの見立て／主動線」と書いて**未裁定の判定基準に使っていた**。2026-07-30b の発言（「このツールではメインと言っていない」）で覆った＝**この枠を判定に使わない**。
      - 文脈行つき2行バー案も据え置き（(a) が決まる前にバーを太らせると作り直しになる）。**モード行の言葉「詞」も触っていない。**
    - **⚠ オーナー不在中に推しで進めた（2026-07-30b・オーナー指示「しばらく…何も見られないから進めちゃって」）**：(b)(c)(d) は**私の推しをそのまま採って実装した＝オーナー未レビュー**。根拠の表は材料 doc（`docs/drafts/2026-07-30-lyric-editor-screen-decisions.md`）にそのまま残っている。
