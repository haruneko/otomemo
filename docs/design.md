...
- `copyNeta`（`apps/api/src/core.ts:88`）が元 id への繋がりを残す（`vary` の `variant_of` とは別の種類で）。既存の複製の振る舞いは変えない。2番を作る操作は既にある＝曲フォームの「複製する」（`apps/web/src/components/FormStrip.tsx:240` `duplicateUnit`）。
- 共有されたメロへの書き込み確認：`useCowGuard.ts:70`（`parentId` 無し＝ネタ帳から直接開くと何も聞かずに原本へ書く。`App.tsx` で親から潜ったときだけ parentId が付く）。詞を持つメロではこの経路が正面玄関になる。「この曲だけ変える」が分家（vary）を呼ぶことと、2番の作り方が複製であることの食い違いも、ここで扱う。範囲は §31-11 の裁定待ち3・4に依存。
