...
- **＋パートを足す＝実装済み（2026-08-01）**＝オーナー「歌詞は各パートに入れられるが、パート（Aメロ/Bメロ）を追加する口が無い＝Cメロしかない曲にA,Bメロの置き場がほしい」。→ 通しの面の末尾に「＋パートを足す」＋役割メニュー（`ROLE_KEYS`＝Intro/Aメロ/Bメロ/サビ/落ちサビ/大サビ/Cメロ/間奏/Outro）＝**新セクションを曲の末尾に開く（作成＋配置1操作・失敗で deleteNeta 孤児なし）**＝歌詞を入れる前に置き場を作る（穴の粒度 曲＞セクション＞句）。`collectLyricRows` は `songNextBeat`（曲の末尾拍・次小節頭へ丸め）を返す。**位置は当面末尾**（Aメロを Cメロの前に、等の並べ替えは編成エディタ側／後段で精緻化）。web 1234緑。**＝スライス5は 読む＋推敲＋インライン編集＋□プレイスホルダ＋パート追加＋詞先↔曲先の橋 が一通り動く。**
