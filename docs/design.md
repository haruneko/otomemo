...
- **決定 LV2（情報優先度＋並べ替え・後続）**：カード内は **タイトル最優先／id(1ae4ffd3…)は薄く小さく**（価値低いのに現状目立つ）／副アクションは主要2つ＋「…」畳み。並べ替え＝更新/種別/タイトル順のセレクト（本来欲しい・LV1の次）。
- **決定 LV3（一覧画面の作り込み・後続）**：フィルタ強化＋**プロジェクトの関係（曲/ファイル/チャット/ジョブ）一覧**（既出「プロジェクトをホーム化」S1-S3＝line 405-409 と合流）。ProjectScreen をタブ一望に育てる。
