...
- **⚠ この「退役」はオーナー裁定ではない（2026-07-30b 監査の指摘1-C で訂正）**：この行自体を私が `5bc4a49` で書いた＝根拠は先例調査で、オーナー帰属は無い。**オーナーの実際の言葉は 07-29 の「流し込みも歌詞入力もあまり使わないから奥でいい」＝残して奥に置く**であって、口をゼロにする許可ではない（一次資料＝`docs/research/2026-07-29-lyric-editor-screen-design.md` §1 の(1)(3)）。
- **現況の帰結**：**配線された画面には、いま音符を割る口が1つも無い。** 割る操作の存否は研究の結論後のオーナー裁定待ち（§31-11 の2）なのに、事実上「無し」側で運用が始まっている。**監査の推しは「研究の結論が出るまでは奥に残す」**（オーナー発言に忠実）＝この扱いはオーナーに確認中。戻すなら `PianoRoll.tsx` の条件式1つ（`!wired` を外し、置き場をパネルの奥にする）。音符を割って合わせる操作を残すかどうかは、**「音楽的に成立する割り方を候補として出せるか」の研究の結論待ち**＝出せる見込みが立たないなら操作ごと作らず余りは切り捨て（研究文書は 2026-07-30 着手済み）。残す場合の置き場は歌詞パネルの奥・明示操作＋確認つき・名前は音符が変わることが読める言葉（仮称のまま＝§31-11 裁定待ち16）。`set_lyric`・詞先メロ・句の面が配線されていない呼び側は従来どおり（後退ゼロ）。＝architecture「黙って実体を作らない」の受け＝**歌詞を打っただけで音符が割れることはもう起こらない**。
