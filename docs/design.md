...
**段階**：✅CP1 `MetaPanel` 抽出（折りたたみ・要約を内製）→ ✅CP2 `EditorHeader`/`RelationsPanel` 抽出（NetaDialog ~470→323行・web221緑・Playwrightで melody/chord/rhythm 回帰なし確認・2026-07-01）→ ⬜CP3 `useNetaEditor` へ state/logic 移設（NetaDialog を薄く＝**コールドで一気に割らず・state を触る機能追加のついでに**進める・S5後続）。各段階でテスト緑・aria-label 不変。
