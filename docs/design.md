...
- **決定 LV3（一覧画面の作り込み・🚧実装中 2026-07-02）**：フィルタ強化＋**プロジェクトの関係一覧**（既出「ホーム化」S1-S3＝line 405-409 と合流）。
  ...
  - **ProjectScreen 磨き**：ヘッダ/タイトルの**絵文字□化→SVG**（backlog「絵文字→SVG」と合流）、空状態の充実、`ファイル`セクションのボックス不揃い是正、縦詰め。
  - **段階**：✅Slice A＝P3＋P4（入れる＋未仕分け・2026-07-02）→ ✅Slice B＝P1＋P2（チップ＋作成・2026-07-02）→ Slice C＝ProjectScreen磨き。各スライスTDD/Playwright/コミット。
